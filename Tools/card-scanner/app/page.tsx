...
import { contactCount, downloadCSV, loadContacts, saveContact, splitPair } from '@/lib/storage';
import { feedbackCount, loadFeedback, saveFeedback } from '@/lib/feedback';
import { addToBridge, mapCardToCapture, outboxCount } from '@/lib/bridge';
import type { StepLog } from '@/app/api/scan/route';
...
const DEFAULT_SETTINGS: AppSettings = {
  // Capture plane = local by default (Bridge two-plane gate): private captures
  // stay on-device. Groq/Gemini are cloud = egress, opt-in.
  source:    'ollama',
        ...
        {d.source === 'groq' && (
          <div className="space-y-3">
            ...
            <div className="bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2.5 space-y-1 text-[11px] text-emerald-800">
              <p className="font-semibold">✓ Pre-configured — no setup needed</p>
              <p>Or get your own free key at <span className="font-mono underline">console.groq.com</span></p>
              <p>Model: Llama 4 Scout 17B (vision) · 14,400 req/day</p>
            </div>
          </div>
        )}
...
export default function CardScanner() {
  ...
  async function processJob(id: string) {
    ...
    try {
      ...
      const data = await res.json() as Partial<CardData> & { error?: string; _model?: string; _source?: string; steps?: StepLog[] };
      if (!res.ok || data.error) { upd(id, { status: 'error', error: data.error ?? `HTTP ${res.status}`, steps: data.steps }); return; }

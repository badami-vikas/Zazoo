...
- **RESOLVED 2026-09-15 (opened 2026-09-11, user report repeated 2026-09-15) — `relationship` and `devpilot` can never be added: the trifecta gate refuses them at publish AND at entry verification (attach: TASK-033, P0).**
  ...
  **Exit (one of, user's call):** (a) first-party built-ins publish with the trifecta verdict recorded and `verifyCommonsEntryContent` accepts a recorded-verdict first-party entry, leaning on `modules.install`'s existing risk-`external` human approval as the real gate; (b) split the egress capabilities (`web-research`, `google-sources`, DevPilot's push/comment) into separately-installed capabilities so the base Modules publish clean; (c) keep the gate and have the dialog say these two are withheld and why, instead of omitting them silently. Whichever is chosen, `apps/api/test/commons-embedded.test.ts` currently pins the broken number (13 of 15) and must flip to 15.  **Resolved 2026-09-15 (ADR-282, AP-187):** the user chose the split. The egress capabilities left both Modules for four separately-installed Commons entries, so neither base manifest carries egress and the trifecta union cannot form. Catalog 15 → 19, and `apps/api/test/commons-embedded.test.ts` now asserts zero refusals and every curated built-in seeded — it was pinning 13-of-15 and was seen failing at that number before the split. Oracle over every Commons built-in: `OK — all 19 pass the scan`. Relationship and DevPilot are offered in New again, without reach until it is added.
- **OPEN 2026-09-15 — DevPilot's egress split is manifest-level only: nothing enforces it at runtime (attach: TASK-033, P2).**
  ADR-282 moved `devpilot.syncGithub`, `devpilot.reviewPr`, `devpilot.suggestPractice` and `devpilot.analyzeIssue` out of the DevPilot manifest into two Commons entries, which is what lets DevPilot publish. Relationship got a matching runtime gate (`assertWebResearchModuleBinding` now requires the Commons attachment under the Learning Agent). DevPilot got none: `packages/db/src/governance-stores.ts` still seeds its Agents with `allowedSkills: ["devpilot.syncGithub"]` and the reviewer trio unconditionally, and `apps/api/src/agent-role-templates.ts` allows them by id, so an install without the GitHub capability still reaches GitHub. The publish shape improved; the trust shape did not.
  Exit: a gate in `apps/api/src/routers/devpilot.ts` (beside `assertDevpilotFlightEnabled`, which all three draft procedures already call) asserting the Commons attachment for the invoking Agent, with `governance-stores.ts` granting the skills only when it is present; a test that an install without the attachment is refused, seen failing first. Open questions this gate must answer: which Agent identity must hold each attachment, and what an existing DevPilot install does on upgrade when it loses reach.
- **RESOLVED 2026-09-16 — `apps/web` ui-conformance ratchet was red: `AgentDetailPage.tsx` bypasses `ModuleSurfaceLayout`/`DataViews` (attach: TASK-073, P2).** Exempted with a written reason during the 2026-09-15 strain merge and confirmed under TASK-114: the page is a Record Detail shape (§3b) with the ADR-250 room rig and no Database rows, not a landing view the shell was built for. `ModuleIntelligenceEntryPage.tsx` is exempted on the same ground. Original evidence below.
- **(was OPEN 2026-09-11) — `apps/web` ui-conformance ratchet is red on this branch: `AgentDetailPage.tsx` bypasses `ModuleSurfaceLayout`/`DataViews` (attach: TASK-073, P2).**
...
## OPEN 2026-09-20 — `jobpilot-culture-research` socket-close tests time out on CI (TASK-115 PR #97; attach: the 2026-09-03 concurrency evidence above)
- Verbatim signature, CI run 35496414537 on PR #97, twice: first `agentOrchestration.childRun.cancel … routes a culture-research child Run through its OWN durable cancellation mechanism` then, on rerun, `cancelCultureSourceFetch aborts a real in-flight fetch and leaves the final record cancelled` — both `Error: timed out after 120s waiting for the server to observe the socket close`. 94/95 turbo tasks green; nothing in the PR touches `apps/api`.
- Standalone on this Mac the same file passes 57/57 (`node --test dist/test/jobpilot-culture-research.test.js`, 2026-09-20). Main's last two CI runs were already red on `security audit` and `cargo check windows-latest`; `platform` was green there, so this signature is new on CI.
- The 2026-09-03 prescription ("widen the socket-close wait") is not the fix: the wait is already 120 s and the server never observes the close on the runner. Next step: make the assertion observe the abort through the run's own state (cancelled record + aborted controller) instead of a real socket close event, or gate the two real-socket tests behind a runner capability check. Merged with this recorded rather than re-pinning the suite (AP-019).

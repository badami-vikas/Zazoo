...
## OPEN 2026-07-20 — broad GraphStore suite still writes the read-only VOCAB4 Signal view
While validating VOCAB6, nine pre-existing `platform/packages/db/test/graph-store.test.ts` cases failed before reaching their assertions because they still call Drizzle INSERT against `signals`, now a security-invoker read view after migration `0023`; Postgres correctly reports that the derived `type` column is not updatable. The changed Automation Run and full-Graph projections pass targeted tests, so this is not a VOCAB6 regression. TASK-012 owns updating those historical fixtures to seed canonical Events plus participant Relations during final compatibility deletion; no production dual-write or writable-view workaround should be added.

## OPEN 2026-07-20 — two Onboarding tests still assert pre-VOCAB3/pre-VOCAB4 vocabulary
The broad `platform/apps/web/test/onboarding-learning.test.mjs` file still expects a support `touchpoint`/`Ticket` Blueprint node and rejects current source merely because historical `log_touchpoints` compatibility input appears. Current Onboarding compiles canonical Records/Events and the exact rendered-copy regression passes; these two assertions were already stale on the VOCAB5 baseline and are unrelated to the VOCAB6 Settings/Sources change. TASK-012 final compatibility deletion should rewrite them against the canonical Blueprint/Event contract, then remove the legacy compatibility input rather than weakening the vocabulary check.

...
FIX: OnboardingDialog.submit now chains workspace.blueprint.propose -> workspace.blueprint.activate (activation is itself the governed pipeline proposal, so governance is not skipped); final step reports the real outcome (activated vs pending in Approvals). Live-verified: ETA persona run now ends with the Workspace page rendering the active "Deal" blueprint. Remaining cosmetic: parent refresh closes the dialog before the success message is readable. The preview-discrepancy + kanban groupBy items from this row are being handled separately (Agent H).

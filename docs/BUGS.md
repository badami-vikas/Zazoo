...
## OPEN 2026-08-30 — three red tests on `main` that nobody's change caused: a missing avatar module, an unawaited nested test, and a timezone-dependent assertion (attach: TASK-088, P2)
...
- **(1) `apps/web/test/avatar-liveness.test.mjs`** — `ERR_MODULE_NOT_FOUND: Cannot find module '.../app/avatar/zazoo/parts' imported from '.../app/avatar/zazoo/director.ts'`. An extensionless directory import that the node test runner cannot resolve; last touched by `0aa6c191` (companion chat convergence). Web suite is otherwise 187/188.

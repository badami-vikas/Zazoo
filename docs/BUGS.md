...
## OPEN — docs/raw/execution-plan-2026-07.md (Track F3 source doc) does not exist yet (2026-07-07)
This session was tasked with building the F3 foreign-capability importer against `docs/raw/execution-plan-2026-07.md`'s "Pi-primitive-mapping section" and `platform/packages/core/src/capability/foreign-import.ts` (both described as already landed/existing). Neither exists in this worktree — `find`/`grep` across `docs/` turned up no execution-plan doc and no prior `foreign-import.ts`. Proceeded by authoring `foreign-import.ts` fresh from first principles (Capability Trust Model origin/risk axes in `capability/types.ts` + the package format's "no ranges, exact pin" discipline) rather than blocking. Next step: when/if `docs/raw/execution-plan-2026-07.md` lands, reconcile its Pi-primitive-mapping vocabulary against the mapping actually implemented in `platform/packages/core/src/capability/importer.ts` (extension->tool, skill->skill, prompt->skill, theme->view) — the mapping is defensible from the existing docs but was not checked against a doc that doesn't yet exist.

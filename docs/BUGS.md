...
## RESOLVED 2026-08-22 — DevPilot's runtime Agents were never registered in the in-memory `seedGovernance` organization list (TASK-078)

- **What was wrong**: `seedGovernance`'s `agents.organizations.set`/`statuses.set` loop (`apps/api/src/wiring.ts`) covered only the eight original agents. `DEVPILOT_TRACKER_AGENT_ID`/`DEVPILOT_REVIEWER_AGENT_ID` were seeded with roles/scopes/skills but no organization, so `resolveSkillForTask`'s organization-mismatch gate rejected every in-memory-mode invocation of their governed Skills ("Goal, Task, Agent, and Skill manifest must belong to one organization"). Persistent mode was unaffected (`ensureDevpilotTrackerGovernance` writes the agent row). Found because the new Academics Study Steward failed identically in the TASK-078 live harness.
- **Fix**: the loop now carries the module agents (DevPilot pair + Academics Steward) with a comment stating the requirement.

## OPEN 2026-08-22 — two pre-existing failures on `main`, observed unchanged before and after TASK-078 (attach: TASK-073)

- `@bridge/web` `ui-conformance.test.mjs`: `AgentDetailPage.tsx: missing <ModuleSurfaceLayout> missing <DataViews>` — a NEW page bypassing the standard shell with neither an EXEMPT reason nor the shared components. Verified present on clean HEAD (`git stash` → same failure).
- `@bridge/api` `modules.test.ts` "modules.promote: auto-demotes the prior available version": inner subtest "Commons attachments for different Module Agents remain available together" is `cancelledByParent` (async subtest not awaited before its parent finished). Verified present on clean HEAD.
- `@bridge/api` `agent-orchestration.test.ts` "action.propose: a Human directly invoking the governed skill fails closed even with a valid goalTaskRef": inner subtest "action.propose handles null inputs without crashing policy evaluation" is `cancelledByParent`, same unawaited-subtest shape as the `modules.test.ts` case above. Found running the full `apps/api` suite for TASK-079 (Canvas document sync + Ox Alpha summarization); verified present on clean HEAD (`git stash -u` → same failure, `git diff main -- apps/api/test/agent-orchestration.test.ts` empty).
- `@bridge/api` `procedure-classification.test.ts`: 16 procedures from the Accounting, D2C, and JobPilot-onboarding modules (`accounting.*`, `d2c.*`, `d2cNotes.*`, `d2cResearch.*`, `jobpilot.onboarding.*`) are unclassified in `deployment-boundary.ts` — outside this branch's Academics/Canvas scope (`deployment-boundary.ts` last touched by an unrelated dealpilot commit before this branch started); the Academics/Canvas procedures this branch added (`academics.canvas.*`, `academics.listDocuments`) are now classified.
- `pnpm verify`'s `check:vocabulary` stage (`scripts/check-retired-vocabulary.mjs`) fails on clean `main` with no code changes: `apps/web/src/app/avatar/zazoo/ZazooAvatar.tsx` (8 fingerprints) and `apps/web/src/app/components/shared/ModuleGovernanceSection.tsx` (1 fingerprint) report as "new retired product/code vocabulary … changed from 0 to 1" against `scripts/retired-vocabulary-baseline.json`, though neither file nor the baseline has any diff against `main` on this branch. Verified with `git stash -u` → identical output on clean HEAD. Root cause not yet investigated (baseline drift vs. a real un-reviewed retired-vocabulary hit); this currently makes `pnpm verify` fail on `main` itself, independent of any feature branch.
- `@bridge/db` `local-store.test.ts:563`, a THIRD instance of the same unawaited-subtest bug as the `modules.test.ts` and `agent-orchestration.test.ts` entries above: the outer test ("persistent governance aligns Egress and Intake authority with their governed Skill manifests") fires an inner `test("persistent governance provisions DealPilot's Human Module permissions without widening Agent roles", …)` at line 563 without `await`ing it, then proceeds straight to `ensureIntakeAgentGovernance` and `close()`. Reproduces deterministically under `pnpm run test:coverage` (`--experimental-test-coverage`'s per-line instrumentation overhead shifts timing enough to lose the race that plain `node --test` usually wins) but not always under the plain suite — confirmed pre-existing and unrelated to TASK-079's `ensureAcademicsStewardGovernance` grant additions by reading the failing subtest's own content (it is entirely about DealPilot governance) and confirming `local-store.test.ts` carries zero diff against `main`.

...
## OPEN 2026-07-14 — USER REPORT: Intelligence tabs violate the standard table/page toolbar rule; Workflows label regressed from Automations
`apps/web/src/app/pages/IntelligencePage.tsx` renders Modules, Tools, Integrations, Agents, Workflows, and Skills as bespoke cards/lists/empty states directly below `Header`. It never mounts the existing `StandardToolbar` (List → view → search → filter → Add/custom action → 3-dots) or a standard landing-section view, despite the canonical UI architecture grouping Automations/Agents/Skills/Integrations as sibling toggle pages and requiring that toolbar order. The same file hardcodes user-visible “Workflows” plus “Create/Run a workflow”, contradicting the canonical user-facing primitive “Automation” (`docs/wiki/ontology.md`) while the kernel identifier may remain `ritual`. “Apps” is also absent as a user-facing sibling despite the report, and must be classified in UI-RULES-1 rather than silently conflated with Integrations. Source requirement: `docs/raw/requirement-bugs-2026-07-14-onboarding-shell-intelligence.md`; root cause: UI-RULES-1 was scheduled as the first Egg+Commons task but has not been executed.
...
## OPEN 2026-07-07 — DealPilot/JobPilot/Helpdesk hardcoded into apps/web, not gated by package installation state
`platform/apps/web/src/app/routes.tsx` and `IntelligencePage.tsx`'s `TOOLS` array statically register DealPilot/JobPilot/Helpdesk as always-present routes/tools. This contradicts the "mix-and-match add-on capability packages" architecture (CLAUDE.md) — they should be gated by whether a corresponding row exists in `packages.list` (state=available) for the workspace, not hardcoded into the shell. The real package store/lifecycle (`packages.register`/`install`/`promote`) already exists and is wired (see the new Packages tab, `IntelligencePage.tsx`); what's missing is routes.tsx/Layout.tsx actually consulting it before rendering these tool entries. Not fixed here — flagged per user's 2026-07-07 "are these kept as add-on packages?" question; a real fix means routes.tsx conditionally registering these three based on `packages.list` state, which is a incremental follow-up, not a rewrite.

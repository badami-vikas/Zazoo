...
## NOW — current batch: Batch 9 — Month-6 (PKG-1 / PKG-2 / BLUEPRINT-1 / CONSOLIDATE) → `docs/raw/roadmap-6month-2026-h2.md` §M6 (code-complete + 59/59; awaiting AP-017)
...
- [ ] AGENTS-1 — the foundational agents invocable as first-class peers via a shared prompt-assembly seam (kernel-invariants + agent-identity layers over the ADR-027 RunContextAssembler); `invokeAgent` returns a governed union (information|draft) with NO executed variant (structural "no independent write")
- [ ] AGENTS-2 — onboarding-profile → Chief-of-Staff persona: Memory-family `OnboardingProfile` view + `buildChiefOfStaffPersona`; spirit-animal tone + persona resolved SERVER-SIDE from the stored profile (stored animal wins over client `input.animal`)

> **Status (2026-07-14)**: both items code-complete + verified green (full `turbo run typecheck test build --force` **59/59**; `@bridge/core` → 317 @ 93.04%; apps/api converse tests 7→9). Boxes above deliberately NOT ticked pending **AP-016** approval (governance). Built sequentially by me (both edges live in `@bridge/core` — shared `dist`/`index.ts` — so parallel subagents would race on build state; the genuine parallel opportunity is Batch 8's independent surfaces). Key reconciliations: the "PromptAssembler" (undefined-elements #6) IS the shipped `RunContextAssembler` (ADR-027), extended with layers 1–2 (ADR-072), not a duplicate; roster stays ADR-046's **4 agents + Communications skill** (roadmap's "5 agents" wording superseded — Communications NOT re-promoted, ADR-074); `invokeAgent`'s "no independent write" is made structural via a union with no executed variant (ADR-073). DONE-WHEN met for both (`core/test/agents-invoke.test.ts`, `core/test/agents-persona.test.ts`, + 2 new `apps/api/test/chief-of-staff.test.ts` cases). Details: `docs/log.md` 2026-07-14 Batch 7 entry; ADR-072–075. **Deferred by design**: the richer four-axis SpiritAnimal tone card (Commons-authored), a model-backed CoS voice.
...
- [ ] PKG-1 — sandbox before executable logic: a pure `evaluateSandboxRequirement` gate (`core/src/capability/sandbox-policy.ts`) wired as a HARD REJECT in `packages.install`; an `execution` spec's PRESENCE marks a capability executable; `isolation:"none"` and network/fs-under-`process` are refused; declarative caps pass
- [ ] PKG-2 — Commons supply-chain trust: manifest signing + verify-on-install + TLS-by-default + a community-origin trust floor. Pure policy in `@bridge/core` (canonicalize/verify/TLS, zero-dep, injected verifier); ed25519 bound at the seam (signer in `services/commons`, verifier in apps/api `HttpCommonsClient`); community pinned == user_code (untrusted → no auto-activation)
- [ ] BLUEPRINT-1 — freeze `WorkspaceBlueprint` as a versioned, Commons-publishable manifest (`kind:"workspace_definition"`, `manifest.blueprint`, `schemaVersion`); `parseWorkspaceBlueprint` closed-key gate makes it declarative-by-construction; `workspaceBlueprintTo/FromPackageManifest` bridge; caps composed by reference
- [ ] CONSOLIDATE — testing debt: codify the real setup (`node --test` + per-package coverage floors + turbo, NOT vitest); fix core-barrel coverage-dilution by recalibrating downstream floors

> **Status (2026-07-14)**: all 4 items code-complete + verified green (full `turbo run typecheck test build --force --concurrency=1` **59/59** — serial to avoid the documented pglite/db parallel-resource-pressure flake). Boxes above deliberately NOT ticked pending **AP-017** approval (governance). Built the coupled `@bridge/core` foundations directly (`signing.ts` → `blueprint.ts` parse+bridge → `approvals.ts` origin floor → `manifest.ts`/`types.ts` wiring → `index.ts`), then fanned two genuinely-independent edges to **parallel subagents** — `services/commons` ed25519 sign-seam (10/10) and `integrations-google` behavioral tests (CONSOLIDATE; 35/35, floor 50→53) — owning all apps/api wiring + tests myself (`router.ts` sandbox gate + community floor; `commons-client.ts` verify-on-install + TLS). Core → 347 (built); apps/api 75→90 (3 new test files: pkg1-sandbox-install, pkg2-commons-signing, blueprint-commons-roundtrip); commons 10; integrations-google 35. **Key calls**: ADR-076 (execution-presence = executable, MCP carve-out removed), ADR-077 (crypto at the seam, v1 registry-held signing key), ADR-078 (closed-allowlist parse = declarative enforcement), ADR-079 (no vitest; two floors recalibrated `sensors` 40→39 + `db` 55→54 for core-barrel dilution — no package's OWN coverage dropped). DONE-WHEN met for all 4. Details: `docs/log.md` 2026-07-14 Batch 9 entry; ADR-076–079. New BUGS row: `blueprintFieldSchema` enum omits `"location"` (store 10 kinds vs router/blueprint 11 — pre-existing, out of scope). **Deferred by design**: store-layer `resolvedTrustGrants` for the origin floor (defense-in-depth only today), per-publisher author-key signing (post-v1), a `docs/wiki/testing.md` rewrite.

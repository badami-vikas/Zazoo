...
- Implementation evidence: [core prototype](../outputs/2026-07-16-task-006-dealpilot-core-prototype.md); [durable Local Plane closure](../outputs/2026-07-18-task-006-dealpilot-local-durability.md). DealPilot Records, Relations, captures, candidate profiles, Gmail continuation/checkpoint/receipt state, spend settlement, and credential audit now persist atomically per Organization through the file-backed Local Plane; the desktop sidecar supplies app-data storage and an explicit OS-keyring provider, while non-test server/web-only boot fails closed without durable storage and an approved credential provider. Credential reveal/copy/revoke requires server-derived password-AMR proof, owner scope, and a short-lived single-current session; revoke removes the vault entry and durable Source projection. Capture reads are Source-scoped, bounded, and deterministic; provider payload samples no longer enter governed output. The desktop loopback API requires a 256-bit per-launch capability, explicit loopback binding, authenticated graceful shutdown, and closed Tauri-origin CORS; that capability authenticates the server-owned client but cannot prove Human re-authentication. Google OAuth uses an authenticated, hashed-at-rest, short-lived, single-use state, rechecks the initiating Human's membership before and after provider exchange, opens consent in the system browser, and never navigates a token-bearing webview externally. A real macOS keyring round-trip, native 1280x800 Tauri DealPilot window, and real-Chrome 375x812 viewport are evidenced. Status stays `in_progress` pending live Google/BizBuySell and verified OS/application re-authentication; signing and physical-mobile certification remain unclaimed.

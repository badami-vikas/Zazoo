...
- Evidence: docs/TASKS.md + Task Manager UI already prove the ledger model in production (AP-024/025); Bridge implementation checkpoint `c708017` adds one recursive Task Database, migration `0027`, governed pipeline-linked create/restructure/reopen proposals, atomic path/version updates, required-Skill no-default routing, calibrated approval bands, deterministic projection/reconciliation, guard evaluation, Graph/Files/Calendar View integration, signed built-in Module packaging, Queue/Form/Tree/Record Detail, and real self-hosting evidence. Targeted Core/DB/API/Graph/RLS/parser/manifest/web checks passed; migration fresh/upgrade/replay preserves legacy Goal/Task IDs and assignment. Real local workspace proved paths `2`→`2.1`→`2.1.1`; desktop `1280×800` and exact `375×812` Tree rendered with `scrollWidth===clientWidth`; screenshots are session artifacts. GitHub Actions remain payment-blocked and no CI success is claimed. Exact remaining gate: a separate strictly sequential session must certify `badami-vikas/Corporate-training-sims` against the contract in `outputs/2026-07-21-task021-task-manager-bridge.md`; this session did not change that repository.
...
- Approval: AP-033 applied (plan+roadmap addition); AP-034 applied (revision 1); AP-035 applied (revision 2); AP-062 applied (Bridge implementation milestone and Corporate-training-sims remaining gate); ADR-106 records the Goal-collapse vocabulary call; ADR-107 records no-default CoS routing; ADR-136 records the landed storage/governance/projection contract.
...
- Approval: AP-023, AP-029, AP-044, AP-045, AP-054, and AP-060 applied
- Dependencies: TASK-001

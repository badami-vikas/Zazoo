...
- Evidence: user feedback-mechanism correction 2026-07-15; [TASK-010 output](../outputs/2026-07-17-task010-red-flag-feedback.md) — shared `RedFlagControl` primitive wired into TableView.tsx (data cells) and JobPilotApplicationDetail.tsx (rendered bullets), plus a Settings > Learning "Red flags" audit section; DealPilot/JobPilot green/yellow/red flag semantics removed (AP-023). **Independent review found this commit not merge-ready** (auth/IDOR, ledger-privacy leak, no saga/idempotency, no lineage concurrency control, weak anchor identity, no server-side pagination, per-cell N+1 queries, viewport- instead of pointer-based touch detection, unbacked persisted-column rename) — fully remediated in [TASK-010 review remediation output](../outputs/2026-07-17-task010-review-remediation.md): every `redFlag` procedure is now authenticated+membership-gated with owner always `ctx.identity.id`; the governed proposal's ledger row carries only an opaque Memory reference (no anchor/renderedValue/reason); `create` is a client-`operationId`-driven idempotent two-step saga; `MemoryStore` gained a real `casSupersede`/`currentForLineage` CAS primitive (in-memory synchronous, persistent via a genuine Postgres SERIALIZABLE transaction verified against real concurrent pglite transactions); the anchor is a discriminated cell/bullet union with a canonical-string-derived lineage key; `listAll`/`history` add cursor pagination and full lineage; a new `RedFlagProvider` batches one query per table/record instead of one per cell; touch detection now uses `[@media(hover:none)]`/`[@media(pointer:coarse)]`. Re-verified: core 426, DB 111, API 187 (30 red-flag tests total across core+db+api+web), JobPilot 94, DealPilot 72, Commons 22, integrations-google 35, web 57 tests, full monorepo typecheck/build (40/40), lint, and no-dummy-runtime all pass clean. Item 9 (the JobPilot flag column's persisted-data migration) is explicitly PENDING, not fabricated — TASK-008 RM4 owns migration 0015 and has not landed; a `normalizeLegacyFitFlag` interim safety net + test are in place, with the exact backfill/CHECK-constraint migration plan documented for once RM4 lands. Outstanding before this task can close: live desktop+375px browser evidence of the actual hover/focus/click/touch interaction, the pending migration once RM4 lands, and a fresh independent review of the remediated commit.

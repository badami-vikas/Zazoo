...
- Evidence: docs/TASKS.md + Task Manager UI already prove the ledger model in production (AP-024/025); Bridge implementation checkpoint `c708017` adds one recursive Task Database, migration `0027`, governed pipeline-linked create/restructure/reopen proposals, atomic path/version updates, required-Skill no-default routing, calibrated approval bands, deterministic projection/reconciliation, guard evaluation, Graph/Files/Calendar View integration, signed built-in Module packaging, Queue/Form/Tree/Record Detail, and real self-hosting evidence. Corporate-training-sims merge `d24e76ba9413e2ca757768af5f61d4f413714a69` and PR #102 completed a real queue-integrity Task with Task/Agent Run/Result/Event/File evidence, routing proof, one-queue proof, and signed-manifest verification, then found three Bridge blockers. Source checkpoint `a0da415` resolves those three; signed package checkpoint `c32bc20` versions the immutable bytes as `task-manager@1.0.1`; migration `0028` has fresh/upgrade/replay preservation evidence. Stricter file-backed two-process recertification at Corporate merge `4d6ae1c`/PR #103 proved same-process projection, sweep, signed normalization/install, trust-negative controls, and routing, but found four Bridge blockers now attached in BUGS: durable-local composition loses Automation Runs and signed Module installation/`commonsSource` across restart; legacy `pkginst_*` installation IDs violate the durable ledger UUID contract; and projection reconciliation refreshes unchanged completed Task versions/timestamps, delaying age sweep. GitHub Actions remain payment-blocked and no CI success is claimed. Exact remaining gate: fix those four Bridge defects, pass the exact two-instance Local Plane certifier, then rerun the updated external contract in `outputs/2026-07-21-task021-task-manager-bridge.md`; do not mark done from Bridge-only evidence.
...
- Approval: AP-033 applied (plan+roadmap addition); AP-034 applied (revision 1); AP-035 applied (revision 2); AP-062 applied (Bridge implementation milestone and Corporate-training-sims remaining gate); AP-064 applied (external-blocker remediation and recertification landing); ADR-106 records the Goal-collapse vocabulary call; ADR-107 records no-default CoS routing; ADR-136 records the landed storage/governance/projection contract; ADR-138 records durable recertification effects and signed root Module reconciliation.

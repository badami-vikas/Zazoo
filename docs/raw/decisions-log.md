...
- **Merge note**: this ADR originated on branch `claude/ai-assistant-brain-design-07d52a` (commit 62bd530) as "ADR-035", which collided with a duplicate ADR-035 already present on that branch (Docling DocumentProvider decision) — the same dup-ADR bug class this branch fixed 2026-07-09 by renumbering 035/036/037→042/043/044. Renumbered to ADR-046 on merge into this branch; content otherwise verbatim.

## ADR-047 — Communications Agent demoted to a skill; permanent roster is 4 agents, not 5 (2026-07-10)
...
## ADR-048 — Multi-monitor companion + click-through annotate window; AX-tree walking deferred (2026-07-10)

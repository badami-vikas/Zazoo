...
## ADR-115 — Installed Commons Runs require an exact current signed contract and preserve private-owner isolation (2026-07-18)
- **Decision**: `cited-role-model-practice@1.0.0` receives a runtime binding only when both the stored installation manifest and a freshly fetched, signature-verified Commons entry match the supported contract exactly: one built-in private `stageLearningRecommendation@1.0.0` Skill, one private Signal-write permission, no egress, and no connectors, package dependencies, context providers, execution block, Module, or Blueprint. The active installation must still attach that exact hash and Skill beneath the exact current built-in Relationship Module manifest/version and its declared Learning Agent. Each Run records the Module installation ID/version/manifest hash as well as package/hash/Agent provenance. The resulting Action request is private. In-memory and persistent ledger projections expose every private proposal only to `requestedBy`; resolution, history, and decision paths enforce the same owner check. Migration `0017` backfills legacy Learning proposals and linked rows; future blocked-attempt rows inherit scope/owner. Human edits may change recommendation content but never Commons invocation provenance.
- **Why**: a trusted package identity or recognized Skill ID does not authorize a broader runtime contract. Revalidating only the installation-time hash misses later signed-registry or stored-manifest drift, while private data scope without owner-aware reads and decisions is not private between Organization members.
- **Alternatives rejected**: check only package name/version/Skill ID; hardcode one static content hash despite envelope provenance and scan evidence changing valid hashes; trust installation-time verification without refetch; isolate only Relation proposals; hide private rows in the UI while leaving direct resolution/decision APIs open.
- **Consequences**: any Skill or owning-Module contract drift removes the visible runtime binding and blocks execution fail-closed. Current signed bytes, content hash, exact Module identity, need, attachment, and Agent are re-proven on every Run. Another Organization member cannot list, inspect, decide, or history-read current or legacy private recommendation rows. Adding another executable Commons Skill requires a new explicit adapter and exact-contract predicate rather than widening this one.

## ADR-116 — Organization rename coordinates canonical DB identity and local Files with fail-closed compensation (2026-07-18)
- **Decision**: Organization names are trimmed, bounded to 120 characters, and rejected when empty or unsafe as a local Files path segment. Onboarding performs the authorized rename before proposing or activating its Blueprint. A database `FOR UPDATE` row lock serializes renames across API processes sharing that database. While holding it, the API preflights `~/Documents/Bridge/<Organization>` (or the injected test root), refuses an existing conflicting destination even when the source is absent, moves the old local root, registers rollback, and persists the Organization name. Callback, update, or transaction-commit failure invokes the rollback move. Startup migrates the legacy `Pilot workspace` directory and DB name through this same coordinator rather than changing only the database. A typed missing-workspace error maps to `NOT_FOUND`; membership and non-member denial remain server-side.
- **Why**: the Organization name is both persisted identity and the canonical local Files-root segment. Updating only one side splits visible state, while activating the Blueprint before rename can leave a partially completed onboarding flow whose promised name and Files location never took effect.
- **Alternatives rejected**: DB-only rename; silently merge or overwrite an existing target directory; copy-and-delete Files; activate first and rename best-effort afterward; swallow Files or persistence failures; test against the user's real Documents directory.
- **Consequences**: ordinary failures leave the old DB name and Files path intact, conflicts fail before mutation, concurrent requests advance from the latest locked name, and successful onboarding immediately projects one name through shell, Settings, and Module Files. The filesystem root is injectable for deterministic legacy-migration/concurrency/conflict/rollback tests. This is a database-serialized compensating transaction, not a distributed crash-recovery journal; a future multi-host Files service would need durable intent/reconciliation before claiming crash atomicity.

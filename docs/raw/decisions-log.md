...
## ADR-113 — Model calls declare cost/capability tier; cached prefixes and usage receipts stay provider-neutral (2026-07-18)
- **Decision**: Every `ModelProvider.complete` request declares `cheap`, `default`, or `reasoning`; every configured provider declares the tiers it can satisfy. `createModelRouter` filters by Plane first and tier second, then applies explicit provider hints, so a Local-Plane binding can never fall through to cloud and no call chooses `providers[0]`. Anthropic maps cheap to Haiku 4.5 and default/reasoning to Fable 5 unless environment overrides pin another model. An explicit `cache_control` breakpoint marks only the stable system block; the changing user turn remains an uncached suffix. Every completion returns model, tier, uncached input, output, cache-write, and cache-read token counts. A normalized receipt computes dated USD estimates only where a known price catalog exists and otherwise says `pricing_unavailable`. Before any configured Chief-of-Staff provider call, one wrapper resolves Authority and pre-policy against the provider Plane: cloud completion is a public-scope `external:fetch` by the governed Egress Agent on behalf of the authenticated member; local completion is a local principal `tool:read`. Deny, approval-required, tainted cloud input, or absent principal authority stops before provider access. Cloud prompts contain only static instructions and the explicit user turn; profile-derived tone stays local. Every successful classification, Communications, and foundational-Agent completion appends its prompt-free receipt with the actual Authority basis and policy results.
- **Why**: Registration order is not a cost or capability policy. Provider-native usage fields are the only honest basis for spend attribution, and Anthropic cache reads require the caller to put a breakpoint at the end of an identical prefix. Separating system from the volatile turn preserves the prefix while keeping receipts low-risk and inspectable.
- **Alternatives rejected**: keep first-registered-provider selection; infer tier from token limit or caller name; accept a completion tagged with a different tier; automatically cache the final message block (which changes each turn and misses); expose Anthropic/Groq/Ollama usage shapes directly to callers; default missing usage to zero; silently route a Local request to cloud; treat workspace membership as model-egress authority; call providers before Authority/policy; retain arbitrary provider error bodies; store full prompts in the ledger; persist non-finite cost; claim a dollar estimate for models without a pinned price source.
- **Consequences**: Chief-of-Staff classification is cheap-tier work; foundational Agent and judge calls are reasoning-tier work; Communications uses default. A workspace member without model-egress authority cannot spend against a configured cloud provider, and the Egress Agent remains the attributable physical cloud actor rather than inventing a Chief-of-Staff database identity. Only Anthropic's five-minute cache TTL is exposed until normalized receipts can distinguish and price the one-hour write tier. Cache savings remain conditional on the provider/model minimum prefix length and TTL. The protocol-faithful deterministic adapter proves cache-write then cache-read behavior, but live external-provider evidence remains pending an approved sandbox key and spend budget. Future providers must expose authoritative usage or fail loud and must declare tier capabilities before registration.

...
## ADR-239 — Events speaker-extraction follow-on: own staging tables (not `LocalEntityRecord`/UAP), `commitEntity` for the Event→Person edge, `@bridge/dedupe` reused directly, single-source OpenAlex identity (2026-08-14; TASK-070 follow-on)

**Context.** ADR-236 already locked the strategic shape (drafts-only, existing People database, 3-tier match gate reused, outreach queue not send). This pass is the actual implementation: fetch → extract → OpenAlex resolve → 3-tier gate → draft rows, plus the approve/reject review queue and the outreach-note draft. Three real wiring questions had no prior answer: where do draft rows live, how does an "Event→Person edge" get represented given Events' capabilities only declare private `person`/`record` permissions (no cloud-canonical `edges`/`nodeTypes` graph access), and which of the repo's several dedupe entry points to call.

**Decision.**
1. **Two dedicated Drizzle tables (`event_speaker_drafts`, `event_outreach_drafts`), not `LocalGraphStore.commitEntity`'s `LocalEntityRecord`.** `LocalEntityRecord.kind` is a closed union (`"event" | "memory" | "signal"`) with no field for tier/score/matchedPersonId/decision-status — extending it would touch every `LocalGraphStore` implementation for one Module. A plain organization-scoped table following the exact `DrizzleEventsStore`/`DrizzleAcademicsStore` shape already used throughout this repo needed no new mechanism, and the "shared records/TableSpec pattern" ground rule is exactly this: every other Database in this app IS a Drizzle table, not a `LocalEntityRecord` side-channel.
2. **Not the Google-intake `IntakeService`/`UniversalActionPipeline` Proposal pipeline either**, despite it being the closer name-match to "draft-then-approve." TASK-070's Outcome names the WhatsApp Contact Extractor as the template, not Google intake — and UAP's `Proposal`/taint-label/`RunCtx` machinery is real weight for a Module with two manual-trigger Skills and no cross-plane materialization needs. Revisit if Events ever needs cloud-canonical materialization (Google intake's actual reason for existing).
3. **The Event→Person "edge" is a `LocalGraphStore.commitEntity` row with `kind: "event"`** (an interaction/participation record, personId-linked, payload carries `eventId`/`role`/`talkTitle`) — the SAME shape Google intake already uses for "counterparty attended/participated in X". Not the cloud `edges`/`nodeTypes` graph: Events' manifest capabilities declare only `readPrivate`/`writePrivate` on `person`/`record`, never `edges`/`nodeTypes` access, and that graph's `assertRelationInput`/node-type-owner checks are built for the cross-Module Second Brain, not a single Module's own private participation fact.
4. **`@bridge/dedupe`'s `matchOne` is called directly, not `@bridge/people-sourcing`'s `matchPerson` wrapper.** `matchPerson` is one line (`matchOne(candidate, targets, ["company", "title"])`) but importing `@bridge/people-sourcing` pulls in `@bridge/sourcing`/`@bridge/facts`, machinery for people-sourcing's multi-connector waterfall search that Events never uses (Events extracts from ONE page it already fetched). `LocalPerson` also carries no `company`/`title` field to corroborate against, so `matchPerson`'s specific corroborating-fields choice does not even apply here; `["affiliation"]` is what Events' own data supports.
5. **`decide` enforces never-auto-merge for EVERY tier, including `strong`**, and refuses (`CONFLICT`) a second decision on an already-decided draft BEFORE any side effect runs (not just at the DB row's own `WHERE status='pending'` update, which alone would still have let a duplicate Person/edge/outreach-row get created ahead of the no-op update). A tier is advisory triage text for the reviewer, never an auto-commit signal — this is stricter than WhatsApp's own Contact Extractor, which DOES auto-commit strong/corroborated matches; Events chose the stricter reading of ADR-236's "strong = auto-STAGE" (staged, not committed) since a speaker's identity carries higher externally-visible stakes (an outreach note gets drafted about them) than a WhatsApp contact roster entry does.
6. **Identity resolution calls OpenAlex only, not separate ORCID + Crossref clients.** OpenAlex's author record already folds in ORCID (`orcid` field) and institution affiliation (`last_known_institutions`) — the two facts ORCID/Crossref would separately supply. Three parsers for the same underlying fact was rejected as pure surface area; noted as an explicit upgrade path (`openalex.ts`'s doc comment) if OpenAlex coverage proves thin for a real speaker roster.

**Alternatives rejected.** Extending `LocalEntityRecord.kind` with a new `"event_speaker_draft"` variant (touches every `LocalGraphStore` implementation, and still has no field for reviewer decision-status without a second schema change). Routing through Google intake's UAP Proposal pipeline (real weight this Module's two manual Skills don't need). Writing the Event→Person edge into the cloud `edges`/`nodeTypes` graph (Events declares no capability for it, and the graph's cross-Module `nodeTypes.owningModule` registration is a real "changes Commons vocabulary" surface this Task never asked to touch). Auto-committing `strong`-tier matches like WhatsApp does (rejected — a drafted-and-visible outreach note is a higher-stakes artifact than a silently-added roster contact, so Events holds every tier for review). A second ORCID/Crossref API client (rejected — OpenAlex already aggregates both facts this Task needs).

**Verified.** `@bridge/events-extraction` 10/10 (parse.ts JSON-LD + text-heuristic, openalex.ts incl. affiliation tie-break/failure/blank-name paths); `@bridge/api events-extraction.test.ts` 5/5 over a real `buildWiring` local-plane instance (strong-match reuse never duplicates the Person, unmatched approval creates exactly one new Person, reject leaves zero trace, re-deciding an approved draft is refused with `CONFLICT` and does not re-materialize, per-event/per-status listing); `@bridge/module-manifests` 26/26 incl. the new no-`external:send`-anywhere-in-Events and Agent-owns-both-Skills-zero-Automations tests; `@bridge/db` migration-metadata/migration-journal green through `0043`; `pnpm exec turbo run build` green across all four touched packages (24/24 tasks); `apps/web` `ui-conformance.test.mjs` 13/13 unaffected.

**Consequences.** The review queue is fully self-contained under `@bridge/db`'s `DrizzleEventExtractionStore` — no change to `LocalGraphStore`'s port interface, no new UAP proposal kind, no cloud-canonical `edges` row. The Event→Person participation fact rides the same `commitEntity` shape Google intake already established, so a future cross-Module "what did this Person attend" view can query `LocalEntityRecord` kind `"event"` uniformly regardless of source. If Events ever needs the fact to be queryable from the cloud-canonical Second Brain graph, that is a new, separately-governed decision (declaring `edges`/`nodeTypes` capabilities), not an extension of this pass.
...
## ADR-236 — Academics Module ships three sibling toggles, not nested Pages; Events sub-module stages speakers into existing People, never a second store; LinkedIn send stays human-per-invite (2026-08-13; AP-154; TASK-069/TASK-070)

**Context.** User directive, verbatim: *"I want to create a new module called Academics. The main module should act like a voult where I have a toggle for Subjects inside which I can add and manage lecture sessions, then a toggle for assignments. Then add a sub-module to Relationships module called Events where I can add conference land events links. Similar to dice conference process, I need a workflow automation to extract speaker names and details, an automation to send linkedin connection request."*

Two premises in the request didn't hold up under investigation, both surfaced before any code was written:

1. **"Similar to dice conference process"** — exhaustive repo-wide grep (`platform/`, `docs/`, `outputs/`, `Tools/`, `git log --all --grep`) found no such process. "Dice" only ever means the Dice string-similarity coefficient (`platform/packages/dedupe/src/scoring.ts:1`); "conference" only appears as an unbuilt future Tool (`platform/tools/recorder/package.json:6`, `docs/raw/calendar-plan.md:270`). The extraction pipeline in TASK-070 is designed from scratch, built on governance the repo already has (the 3-tier match rule, WhatsApp's draft-then-approve staging).
2. **"An automation to send linkedin connection request"** — no LinkedIn API exposes invitations, their User Agreement prohibits automated access, and this repo already codified that position: `Tools/recon-salvage-2026-08-03/README.md:192`, "LinkedIn is never fetched directly (ToS / anti-scrape)." Every outbound-send path in `platform/` today (Gmail egress, WhatsApp send) is draft-then-approve with a human on the send; there is no unattended external-send capability anywhere in the codebase to extend.

**Decision.**

1. **Academics: three sibling toggles (Subjects · Lecture Sessions · Assignments), not Lecture Sessions nested inside Subjects.** The UI canon (`docs/wiki/ui-architecture.md`) decides this on its own terms: toggles are one level deep, and "different columns, same strong sibling cluster → TOGGLE"; nesting is reserved for sub-modules, and a grandchild re-attaches to the root by construction. Subjects/Lecture Sessions/Assignments have genuinely different columns, so they are three toggles; a Subject's Record Detail carries its own Lecture Sessions and Assignments as related Sections, which is where the "inside Subjects" experience actually lives.
2. **Events resolves speakers into NetworkManager's existing `people` database — there is no separate Speakers table.** A second people store inside a relationship product defeats the product. Consequence: Events must declare its own `readPrivate("person")`/`writePrivate("person")` permissions on its capability — `parentModule` nesting grants nothing (ADR-178 restated, not revised).
3. **Speaker extraction writes drafts only, gated by the existing 3-tier match rule** (strong = name + ≥1 corroborating point → auto-stage; moderate = name-only → pending; weak → flagged, no write) — reusing the WhatsApp Contact Extractor's draft-then-approve UI rather than building a second staging mechanism. Identity resolution is designed against OpenAlex/ORCID/Crossref, whose own published disambiguation approach (name similarity, co-author overlap, institutional affiliation, topical coherence, citation pattern, ORCID override, with a refusal sentinel rather than a guess) independently confirms the repo's existing ambiguous-duplicates-as-Signals rule.
4. **LinkedIn gets an outreach QUEUE, not a send automation.** The Skill drafts a personalized note per resolved speaker from their extracted bio plus existing Signals; a human clicks through and sends manually; "marked sent" writes a Signal. No code path sends a LinkedIn request without a human action on that specific invite. This is the same shape as every other outbound path already in the codebase.
5. **Both Modules ship their skeleton (manifest + toggle Page(s) + routes) as TASK-069/TASK-070 now; Skills, Automations, and the extraction/outreach pipelines are follow-on implementation under the same Task IDs**, not separate Tasks — consistent with how Helpdesk and WhatsApp's own Tools were phased in on their existing Module rows rather than minting new ones per capability.

**Rejected alternatives.** (a) Nesting Lecture Sessions as a second-level page under Subjects — rejected, no second nav level exists in the grammar and the nav builder would flatten it to root, which is worse than never building it. (b) A bespoke Speakers table under Events — rejected, duplicates the People database this Module already has access to and creates two identities for the same person. (c) An assistive "auto-fill the note field and let a script click Send" LinkedIn automation — rejected even as an upgrade path; it is still unattended action against a platform whose terms prohibit it, and this repo's own recon tooling already drew that line for a different feature.

**Consequences.** Both Modules are greenfield with no prior Bridge academic or LinkedIn-outbound surface to reconcile against. TASK-069/TASK-070 own the manifest/page skeletons; the FSRS-backed recall scheduler, lecture-capture pipeline, syllabus intake, and LMS sync (Canvas confirmed as the target LMS) are scoped as later phases of TASK-069, not new rows, matching the "Skills/Automations declared on an existing Module row" precedent set by every Module in the registry to date.

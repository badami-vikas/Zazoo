...
## ADR-113 — TASK-023 Phase 1 ships only rights-verified Parallel Search MCP and fails closed on provider-policy drift (2026-07-18)
...
- **Consequences / follow-ups:** Phase 1 coverage depends on Parallel and fails explicitly when unavailable/degraded. Every result is cited, provider-attributed, and `untrusted_external` before any future Memory/prompt sink. Reconsider Jina, DuckDuckGo, Tier 2, or Tier 3 only through fresh rights intake; paid sources also require the existing cost/ROI approval gate. AP-040 records the user's explicit implementation/rights directive.

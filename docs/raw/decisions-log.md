...
## ADR-111 — DealPilot durable state shares one exclusively owned Local Plane database; Source credentials live only in an explicit OS vault (2026-07-18)
...
- **Consequences**: only one Bridge process may own a file-backed Local Plane directory at a time, and shutdown must close wiring to release ownership. Equivalent filesystem spellings cannot bypass that ownership. Gmail pending receipts, cursors, checkpoints, visited tokens, stable message IDs, and settlements recover after restart; relation backfill and capture materialization are atomic/idempotent. Failed concurrent Source creation can delete only its own keyring entry, never another request's winning credential. Web/server-only deployments must inject an approved vault or fail at boot. The adapter-owned `CREATE TABLE IF NOT EXISTS local_state` needs no numbered migration metadata, so RM4 `0015` and TASK-010 sequencing are untouched. Live Google, physical-device, OS-keychain, and cryptographically verified OS re-authentication evidence remain external gates rather than inferred claims.

...
## ADR-099 — Commons content identity excludes recursive fields; signatures bind registry ordering (2026-07-16)
- **Decision**: Commons SHA-256 identity covers normalized manifest metadata, tags, provenance, and deterministic security-scan evidence, excluding integrity, signature, and publication time. Ed25519 signs `{content, integrity, publishedAt}`. Publish/fetch/install independently verify the hash, signature, trusted key, requested name/version, provenance, and passed scan.
...
## ADR-100 — Commons artifact bytes stay immutable; Module ownership is installation-local (2026-07-16)
...
## ADR-101 — Automation ownership is singular, stored, and server-derived (2026-07-16)
- **Decision**: New Ritual definitions persist exactly one owning `agentId` plus execution Plane. `runById` ignores client authority claims, derives the Agent actor from storage, and rejects missing or mismatched ownership. Legacy `agent_ids` is accepted only when it contains exactly one Agent. Signed Module manifests carry generalized Agent/Ritual keys; the server resolves those keys to Local-Plane UUIDs. Only Automations with an explicit runtime binding expose Run.
- **Why**: Caller-selected actors make attribution and authority forgeable. Multi-owner execution has no deterministic principal. Runtime UUIDs cannot enter Commons because its privacy gate correctly treats UUID-shaped identifiers as local/personal data.
- **Alternatives rejected**: trust caller actor; choose the first of several Agents; place runtime UUIDs in Commons; render Run buttons for inventory-only Automations.
- **Consequences**: proposals and Run records carry the declared Agent and Plane; ambiguous legacy rows fail closed; one DealPilot Automation has an honest governed path in both in-memory and persistent modes; broader orchestration remains TASK-007 and full TASK-005 remains blocked.

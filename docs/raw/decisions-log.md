...
## ADR-237 — DevPilot D2: engineering-assist Skills draft PR reviews, best-practice suggestions, and Issue triage — one model call, spotlit not quarantined, always a proposal a Human approves (2026-08-14; AP-155; TASK-071)

**Context.** The user directed "start on the next task" after D0+D1 merged; asked to disambiguate against the general queue, they chose DevPilot D2 — the PR-review/issue-analysis/best-practice skills named in the original module plan and ADR-235's design decision #2. D1 stored only PR/Issue metadata (title, state, counts) — never body text or diffs — so D2's Skills fetch fresh content from GitHub at draft time rather than reading anything D1 persisted.

**Decision.** (1) Three Skills — `devpilot.reviewPr`, `devpilot.suggestPractice` (both operate on a Pull Request's live diff + description), `devpilot.analyzeIssue` (an Issue's live body + labels) — each fetch fresh content via new `GithubGateway.fetchPull`/`fetchPullFiles`/`fetchIssue` methods (D1's gateway had no single-item or diff fetch) and make exactly ONE model call per draft, bounded to ~6k chars of diff + ~2k chars of description/body. (2) Untrusted GitHub content (PR/Issue text is page-authored by whoever opened it) is `spotlightUntrusted()`-delimited inline in the SAME model call rather than run through a separate quarantine-inspection call: the output is unconditionally a DRAFT requiring a Human's separate approve/edit/veto decision before anything is applied or sent, so a smuggled instruction has no path to act — the two-call quarantine pattern (`ContentGuard`) is reserved for skills whose output could otherwise cross into privileged context unreviewed. A new `github_intake` `TaintSourceId` (untrusted, mirroring `email_google_intake`) tags this content; each skill's own output `taintLabel` carries that provenance into the ledger. (3) All three Skills are `executionClass: "authority_bearing"` (they call a model) and reuse the EXISTING generic `pipeline.propose()`/`action.decide` ledger — no new tables, no new approval UI. Each is wrapped in a Human-triggered (unscheduled) governed Automation, invoked via `automationExecutor.runById` exactly like Task Manager's `runPlanningPlaybook`, so the draft gets an attributable Agent Run and halts for review by construction. (4) A dedicated `role-devpilot-reviewer` Agent, separate from D1's tracker: the tracker's authority is `external:fetch:read` alone; the reviewer additionally needs `record:write` to create a proposal — least-privilege keeps those scopes on separate identities rather than widening the tracker's. (5) `SkillManifest.budget.maxCallsPerDay: 25` bounds real spend per the saved plan's envelope — the first production use of that mechanism (previously declared but unexercised). (6) Posting a drafted review back to GitHub remains explicitly OUT of scope — no `external:send` capability anywhere in D2; the UI links out to the existing Approvals page rather than adding a second review surface.

**A real bug caught by the live verification, not by inspection.** The devpilot Module manifest's `version` stayed `"0.1.0"` while D2 added two capabilities, a second Agent, and three Automations to it — violating the platform's own "manifest content is immutable per version" rule (stated in the original plan and enforced at the DB layer: `module_installations: normalization would change immutable Module content`). A demo instance with devpilot@0.1.0 already installed failed to boot against the changed manifest. Fixed by bumping to `version: "0.2.0"`; re-verified boot succeeds and the three new devpilot tRPC calls (`definitions`, `pulls.list`, `issues.list`) return 200 against the live server.

**Rejected.** A full `ContentGuard` quarantine (separate tool-less model call per draft) — doubles cost for a case where the injection blast radius is already structurally bounded to "the draft text says something odd," caught by the same Human review the draft always requires anyway. Storing PR diffs/Issue bodies in `devpilot_pulls`/`devpilot_issues` at sync time — bloats D1's tables with content that is only ever needed transiently at draft time, and blurs D1's "metadata sync" scope with D2's "quarantine content when a model first reads it" scope that ADR-235 explicitly deferred. One shared Automation with a `skill` enum param (Task Manager's bundling pattern) — rejected in favor of three separate Automations, since `ModuleAutomationBinding` is declaratively one-skill-per-automation and three distinct manifest entries make the catalog's capability list (and its own tests) directly inspectable per skill. A generic per-row "action" UI primitive (a click-through Record Detail panel) — no such primitive exists anywhere in the platform yet (confirmed by search); building one is its own cross-module task, not D2's. Used the existing `DataViews` `actions` slot (documented for exactly "a governed action button") plus a page-scoped Select instead.

**Verified.** `@bridge/integrations-github` 16/16 (3 new tests: `fetchPull`/`fetchPullFiles`/`fetchIssue`, including a binary-file-has-no-patch case). `@bridge/db`, `@bridge/module-manifests` (25/25, updated to assert 2 Agents each with a Plane, the poll Automation still scheduled, the 3 new Automations manual-but-runtime-opted-in, still zero `external:send`), and `@bridge/api` full suite green (one pre-existing, unrelated flake in `automation-scheduler.test.ts`'s anti-storm fixture reproduced identically on the unmodified D1 baseline — confirmed by stashing all D2 changes and re-running before restoring them; not a D2 regression). `apps/web` typecheck clean; `ui-conformance.test.mjs` 13/13 unaffected. **Live walk**: rebuilt + restarted an isolated local-plane demo server (`BRIDGE_DEVPILOT=1`, no real GitHub PAT or model key available in this environment), caught and fixed the manifest-immutability bug above, then confirmed in a real browser: Pull Requests page renders the Select + "Draft review"/"Suggest practices" buttons correctly disabled with nothing selected; Issues page renders the Select + "Analyze" button; all three `devpilot.*` data calls returned 200 OK; no console errors post-fix. The actual model-drafting path (a real PAT + a real Anthropic/Groq key) was not exercised — no such credentials exist in this environment; D1's own live walk hit the identical limit for the sync path.

**Consequences.** DevPilot now drafts engineering-assist content end-to-end, still fully within the existing governed-proposal system — zero new persistence, zero new approval UI, zero egress capability. D3 (Jira), D4 (comms triage), D5 (news radar), D6 (Work Brief) remain future TASK rows, unstarted. Actually posting a drafted review/comment back to GitHub is the natural next `external:send`-gated slice, not started here.

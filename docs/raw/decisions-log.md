...
## ADR-181 — The capability-build chain: Internal Strategist decides, Capability Builder drafts, Governance computes the verdict, a Human approves; the three junctions are structural (2026-08-06; owner directive)

- **Context**: the owner asked to "ensure builder agent has capability of building capabilities … internal strategist helps decide what capability needs to be built and governance agent intervenes at right junctions". All four agents already existed on paper (ADR-033/046, AP-023) and all four had real governance rows. The wiring stopped at identity. `agents.skills` had **no entry at all** for `CAPABILITY_BUILDER_AGENT`, and `ensureCapabilityBuilderGovernance` provisioned it with no `allowedSkills`, so the Agent held a role, a scope, and nothing it could be asked to do. Its only reachable path was an `@builder` chat mention that returned prose and staged it through the kernel passthrough. Internal Strategist had no way to say a capability *should* exist. Governance was never consulted before a draft reached a human. Meanwhile the Capability Trust Model — `computeRisk`, `moduleHasLethalTrifecta`, `evaluateSandboxRequirement`, `requiredApproval` — was fully built, tested, and simply never called on the build path. `capability/builder-primitives.ts` (file:read/write/edit, shell:execute) was likewise complete, tested, and granted to nobody.
- **Decision**: (1) A pure `capability/build-chain.ts` in `@bridge/core` with three junctions — `recommendBuild` (Internal Strategist) → `draftCapability` (Capability Builder) → `reviewDraft` (Governance) — and **no fourth**. (2) Three governed Skills, one per junction, each granted to exactly one Agent in both the in-memory and the durable path. (3) One router procedure, `capabilityBuild.run`, that walks all three in order. (4) A cleared draft lands as a `draft` row in the *existing* `capability_manifests` table, so activation stays on the pre-existing Human-only `capability.approve` surface.
- **What makes the junctions structural rather than conventional** (the same discipline `AgentInvocationResult` uses — no "executed" variant — and `RoutingDecision` — no `peers` field): `draftCapability` accepts **only** a `RecommendedState`, so there is no entry point by which the Builder builds something nobody asked for. `reviewDraft` accepts **only** a `DraftedState` and **computes** its verdict; a verdict is not a parameter anywhere in the file, so Governance's opinion is not an input the pipeline could act on. There is **no `approved` phase and no `approve()`** — the chain's terminal machine state is `reviewed`, meaning "fit to ASK a person", and `humanApprovalRequired` is typed as the literal `true` so no branch can set it false. Every junction takes the acting `FoundationalAgentId` and throws on a mismatch, so the Builder cannot review its own draft even by calling the function directly.
- **One door, deliberately**: the router exposes `capabilityBuild.run` and nothing else. Separate `recommend`/`draft`/`review` endpoints would let a caller POST straight to `draft` (building with no stated reason) or skip `review` (reaching a human with no computed risk band). The bypass is closed by there being no door, and a test asserts against the router *definition* — a caller proxy answers to any property name and can never prove absence.
- **Six blockers, each mechanically checkable**: type substitution (a draft of a different Capability type than was recommended answers a question nobody asked); provenance (an agent-written manifest claiming `built_in`/`template` would inherit a trust tier it did not earn — a real escalation under `ORIGIN_TRUST_TIER`, not a cosmetic mislabel); the sandbox floor; a declared `shell:execute` on a manifest with no execution spec; and a **retrospective recommendation with zero citations** — the Strategist's standing "missing evidence stays explicit, never invented" boundary enforced at a junction instead of asked for in a prompt. A *proactive* recommendation with no evidence stays legal: it is openly a hypothesis.
- **Blocking is not rejection**: a blocked state keeps the recommendation, the draft, and the reasons, so the Builder can be asked to answer a named list rather than regenerate blind — and, critically, **writes no capability row**. A blocked draft that still landed in the table would appear in Approvals and a human would be asked about something Governance had already refused to forward.
- **The model is bounded, not trusted**: the Builder is asked for a JSON manifest, and the parser refuses on any doubt rather than accepting a partially-understood one (the parts that silently drop are the parts nobody reviews). It never reads `origin`, `id`, `version`, or `capabilityType` from the model — provenance and identity are the Engine's to assign. With no model configured, a least-privilege skeleton is used and labelled `offline_skeleton`, because "a model wrote this" and "the Engine filled in a safe shape" are different things to ask a person to approve.
- **No Agent gained authority**: all three Skills declare `signal:write`, which all three Agents already held. Wiring a governed capability path must not be a way to quietly widen a scope, and a test asserts the Builder's capability scope is still exactly `["signal:write"]`.
- **Star topology is preserved**: no agent calls another. The Engine calls three agents in sequence, each attributed separately in the ledger under its own identity and its own Goal/Task, which is what a workflow calling three Skills has always been. An agent *routing to* another agent would be the peer handoff the topology forbids.
- **Alternatives rejected**: letting Internal Strategist invoke the Builder directly (that *is* a peer handoff, and it puts the decision to build and the act of building in one uninterruptible step); a Governance "assessment" free-text field (a field the model can fill is a field the model can soften — every verdict field is computed); auto-approving low-risk drafts on Governance's say-so (Governance is already the sole exception to agent-floor's governance-resource DENY; giving it activation authority over machine-written capabilities is the one place that exception must not extend); renaming the Agent to "Module Builder" per the owner's question (ADR-180 settled Capability as the governed atom and Module as the shipping unit — the Agent drafts atoms, so the existing name is the correct one); granting the builder primitives now (file/shell access is a much larger authority question than manifest drafting and deserves its own decision).
- **A near-miss the completeness gate caught**: `capabilityBuild.run` does NOT start with `"capability."` (the dot), so the pre-existing capability-authoring deny prefix in `deployment-boundary.ts` did not cover it, and the new namespace landed **unclassified** against the public-cloud boundary. `procedure-classification.test.ts` failed it by name — which is precisely the AP-082/AP-085 failure class that test was written to end, working as designed. It now carries its own `capabilityBuild.` prefix: the chain drives three local-plane Agent Runs and writes governed capability state, so it is strictly more Local-Plane than `capability.` authoring, never less.
- **Consequences**: the Capability Builder can now produce a governed manifest for any of the five Capability types (skill/automation/agent/integration/database) — that is the whole of what "builds capabilities" means today, stated plainly. It **cannot** write files or run code: the builder primitives remain implemented, tested, and granted to nobody, so a draft that declares `shell:execute` is blocked rather than honoured. The `@builder` chat path is unchanged and still returns prose staged through the passthrough; folding it onto this chain is the next slice. `evidence` on a proactive recommendation is unvalidated free text — it is a citation the human reads, not a resolvable reference, and calling it more than that would overclaim.

...
## ADR-117 — Installed Commons Runs require exact Skill and owning-Module contracts with private-owner isolation (2026-07-18)
...
- **Consequences**: any Skill or owning-Module contract drift removes the visible runtime binding and blocks execution fail-closed. Current signed bytes, content hash, exact Module identity, need, attachment, and Agent are re-proven on every Run. Another Organization member cannot list, inspect, decide, or history-read current, legacy, or linked private recommendation rows. Adding another executable Commons Skill requires a new explicit adapter and exact-contract predicate rather than widening this one.

## ADR-118 — Organization rename serializes canonical DB identity and local Files with fail-closed compensation (2026-07-18)
...
- **Consequences**: ordinary, commit, process-crash, and host-crash failures converge to the committed DB name on the next row-locked recovery; conflicts fail before mutation; stale completion cannot consume a newer intent; concurrent and case-only requests advance from the latest locked name. Symlinked roots fail closed. The filesystem root is injectable for deterministic legacy, conflict, concurrency, generation, crash-boundary, casing, and symlink tests. This is single-host Local Plane reconciliation, not a distributed multi-host Files transaction.

...
## ADR-231 — Academics Module ships three sibling toggles, not nested Pages; Events sub-module stages speakers into existing People, never a second store; LinkedIn send stays human-per-invite (2026-08-13; AP-149; TASK-067/TASK-068)
...
1. **"Similar to dice conference process"** — exhaustive repo-wide grep (`platform/`, `docs/`, `outputs/`, `Tools/`, `git log --all --grep`) found no such process. "Dice" only ever means the Dice string-similarity coefficient (`platform/packages/dedupe/src/scoring.ts:1`); "conference" only appears as an unbuilt future Tool (`platform/tools/recorder/package.json:6`, `docs/raw/calendar-plan.md:270`). The extraction pipeline in TASK-068 is designed from scratch, built on governance the repo already has (the 3-tier match rule, WhatsApp's draft-then-approve staging).
...
**Consequences.** Both Modules are greenfield with no prior Bridge academic or LinkedIn-outbound surface to reconcile against. TASK-067/TASK-068 own the manifest/page skeletons; the FSRS-backed recall scheduler, lecture-capture pipeline, syllabus intake, and LMS sync (Canvas confirmed as the target LMS) are scoped as later phases of TASK-067, not new rows, matching the "Skills/Automations declared on an existing Module row" precedent set by every Module in the registry to date.

...
## ADR-026 — Foreign-capability importer: Pi package / MCP server / Activepieces piece / OSS integration -> CapabilityManifest (2026-07-07)

**Decision**: new `platform/packages/core/src/capability/foreign-import.ts` (types: `ForeignCapabilityImport`, `ForeignCapabilitySource` union `pi-package|mcp-server|activepieces-piece|oss-integration`, `ForeignVersionPin` (exact-pinned upstream ref+version, no ranges — mirrors `PackageDependency`'s discipline), `ForeignPermissionDeclaration`, `SandboxPolicy`) plus `platform/packages/core/src/capability/importer.ts`'s single `translateForeignCapability(input) -> ForeignImportResult` (a `{ok:true,manifest}|{ok:false,error}` union, not a bare `CapabilityManifest`). Per-source mapping: `pi-package` reads `descriptor.primitive` (`extension|skill|prompt|theme`) and maps extension->`tool`+connector, skill/prompt->`skill`, theme->`view`; `mcp-server` maps `descriptor.tools[]` to connectors and `descriptor.resources[]` to read permissions, capabilityType always `tool`; `activepieces-piece` maps `descriptor.actions[]`+`descriptor.triggers[]` to one `skill` capability with a connector per step; `oss-integration` is a pure passthrough — `capabilityType: "integration"`, zero connectors, and critically NO permissions are read from `descriptor` at all, only from the caller-supplied `permissionDeclarations[]` (no auto-inference, per the task's explicit requirement). Every import forces `origin: "community"` via a `FOREIGN_IMPORT_ORIGIN` const, regardless of source. Sandbox-required guard: `requiresSandbox()` returns true for `activepieces-piece` unconditionally and for `pi-package` only when `descriptor.primitive === "extension"`; if true and the input's `sandboxPolicy` is absent or `{kind:"none"}`, `translateForeignCapability` returns `{ok:false, error: ForeignImportSandboxRequiredError}` instead of a manifest — never silently allows unsandboxed execution. Other malformed-input cases (missing targetId/targetName/versionPin, `auditRequired !== true`, unknown `descriptor.primitive`) still throw `ForeignImportValidationError` synchronously, matching `package/manifest.ts`'s "loud validation error, never silent default" convention — the result-union is reserved specifically for the ONE well-known/expected governance gate (sandbox), not genuinely malformed input.

**Why**: `docs/raw/execution-plan-2026-07.md` (the task's cited source doc, including its "Pi-primitive-mapping" section) and a pre-existing `foreign-import.ts` do NOT exist in this worktree (confirmed via `find`/`grep` across `docs/` and `platform/packages/core/src/capability/` before starting — see the matching docs/BUGS.md row). Built the type+translator from first principles against what DOES exist: `capability/types.ts`'s `CapabilityManifest`/`CapabilityOrigin`/`RiskBand`, and `package/types.ts` + `package/manifest.ts`'s established conventions (exact-pin dependencies, defensive snake_case-tolerant-but-here-just-plain-object parsing, throw-on-malformed). The `oss-integration` "no auto-inference" rule and the sandbox-required guard are both direct, unambiguous requirements from the task brief and are the two properties the test suite (`test/importer.test.ts`, 18 new tests) most heavily guards against regression.

**Alternatives rejected**: throwing `ForeignImportSandboxRequiredError` like the other validation errors instead of returning a result union (rejected — the task brief explicitly said "throws OR returns a typed error"; a result union lets a caller branch on `.ok` before ever touching a manifest, which is more ergonomic for an installer pipeline than a try/catch specifically for the one expected-in-normal-operation gate, while every genuinely-malformed-input case still throws); inferring `SandboxPolicy` requirement from `permissionDeclarations` (e.g. "any write implies executable") (rejected — conflates the risk axis, which `computeRisk`/`baseRiskForManifest` already own, with the sandbox-required axis, which is really "can this artifact run arbitrary code," a property of the SOURCE TYPE + descriptor shape, not of what permissions it declares); letting `mcp-server` also require a sandbox policy (rejected — an MCP tool call already runs inside the MCP transport/protocol boundary, which is a different isolation layer than "this platform must sandbox arbitrary executable code," so treating it as always-safe-here is consistent with the task's explicit executable list "activepieces, some pi extensions"); adding real `@modelcontextprotocol/sdk`/Activepieces SDK types to read `descriptor` typed (rejected per the task's explicit "no new dependencies... translation-logic layer, not a live connector" instruction — `descriptor` stays `unknown`, read via narrow defensive property access only).

**Consequences**: new files `platform/packages/core/src/capability/foreign-import.ts`, `platform/packages/core/src/capability/importer.ts`, `platform/packages/core/test/importer.test.ts` (18 tests, `test_fixture_` naming per the task's convention). `platform/packages/core/src/index.ts` gained two additive export blocks (`foreign-import.js` re-export + `importer.js`'s named exports) — no existing export touched. `pnpm --filter @bridge/core build` (tsc -b, `exactOptionalPropertyTypes` on) and `pnpm --filter @bridge/core test` both green: 181/181 passing (163 pre-existing + 18 new), 0 failures. docs/BUGS.md: new OPEN row flagging that the task's cited source doc (`docs/raw/execution-plan-2026-07.md`) doesn't exist yet, so the Pi-primitive-mapping vocabulary implemented here should be reconciled against it once/if it lands.

...
## ADR-230 — Avilo becomes the Accounting Module and CV Naturals the D2C Module; the donor repositories are never touched (2026-08-16; attach: TASK-067; AP-148)

**Context.** Three repositories carry the same `platform/{apps,modules,packages}` skeleton at very different depths. Bridge has the full Engine — `ModuleManifest`, Commons, risk bands, Postgres, Tauri, and eight governance ledgers. Avilo is a shipping Electron 43 desktop app at v1.8.0 with a live beta user, better-sqlite3, and exactly two ledgers (`decisions.md`, `bugs.md`) with no execution queue and no approval gate. CV Naturals sits between them: Bridge's ledger shape, Tauri, sqlite, two modules. The user directed a merge of the latter two into Bridge as **Accounting** and **D2C**. Avilo's own `module-host/src/contract.ts` had already been written for this merge — its header says "What a Bridge module is, and what a host owes it," and explains that Avilo's application-level decisions about ports, storage and router mounting "do not survive being merged with a second module." So the target shape was designed before the merge was ordered.

**Four conflicts were surfaced explicitly and resolved by the user rather than assumed.** (1) *AI autonomy*: Avilo's "no model call without a button press / AI never load-bearing" is the direct negation of Bridge's "adapt before asking; learn before acting" and its Automation-driven Agent Runs. (2) *Storage*: Avilo's "no account, no cloud, no network required" against Bridge Cloud, Supabase and Commons. (3) *D2C's shape*: CV Naturals is two modules today and plans eight, so a single flat `d2c` module would strand its roadmap. (4) *The `whatsapp` id collision* between Bridge's module and CV Naturals'.

**Decision.** (1) **Accounting** and **D2C** as canonical Module names. D2C is a Module whose Orders/Inventory/Research/Production declare `parent: d2c` — sub-module is an **existing** Bridge primitive (ADR-178, `core/src/module/types.ts:143`, navigation-only by explicit declaration: "no shared credentials, no inherited permissions, no plane relaxation"), so this costs no new kernel vocabulary and needs no glossary change. (2) Both Modules run **better-sqlite3 on the Local Plane**, cloud sync optional and off. (3) **`@bridge/whatsapp` survives; CVN's clone is not imported.** (4) **The donor repositories are modified by no stage of this merge.** (5) Subtree merge with `--allow-unrelated-histories`. (6) Both Modules gain a `module.yaml` and keep the `module-host` runtime contract.

**The whatsapp conflict dissolved instead of being decided, and that is the point.** Before choosing which implementation survived, the two `src/` trees were diffed: `diff -rq` returned nothing — all sixteen files byte-identical — and CVN's own `package.json` description says outright that it is a clone of `@bridge/whatsapp` with the manifest/catalog integration dropped. Only `package.json` differs. A conflict that looked like it needed a product judgement needed a two-second command. Recorded because the reflex it rewards is worth repeating: **check whether the conflict is real before paying to resolve it.**

**Why the donors stay untouched — this resolves the desktop-shell problem for free.** The obvious reading of "merge" is that Avilo's Electron shell must either be ported to Tauri or maintained in parallel inside Bridge, and both are expensive: the first orphans a live beta user's installer during the gap, the second puts two packaging pipelines on one maintainer's critical path. Leaving the donors as read-only sources removes the choice. Avilo's Electron beta keeps shipping from its own repository at its own cadence; Bridge grows an Accounting Module in its Tauri shell. Two shells exist, but not in one repository. It also means Avilo's Electron-specific working rules (asar `extract-file` grep, the cross-built-installer warning, `build.mjs` web-bundle ordering) stay true exactly where they are true and are **not** carried into Bridge canon, where every one of them would be false at its point of use.

**Rejected.** *Porting Avilo to Tauri as part of the merge* — breaks a shipping beta for a gain that is not on the critical path, and BUG-016 already records that no Avilo Windows build has ever been launched on real hardware, so the Electron pipeline's risk is unretired and should not be inherited. *Porting both Modules to Postgres* — would require rewriting Avilo's sqlite schema, formulas, overrides and every parser assertion, then migrating existing advisor databases; the parser assertions are validated against real client exports, so this trades a working guarantee for uniformity nobody asked for. *A flat single `d2c` module* — the literal reading of the directive, rejected because CV Naturals' own `ecosystem-proposal.md` plans eight modules and a flat module gives them nowhere to live. *A new "module group" primitive above Module* — fits the directive's wording most literally but adds governed vocabulary when `parent` already does the job. *Copying files in with fresh commits* — cheaper graph, but Avilo's ~53 ADRs and 47 bug records cite commit-era context and would stop being verifiable against history.

**Consequences.** Bridge becomes the only repository under active architectural development; the donors continue as independent shipping products and are not deprecated by this ADR. Id renumbering is owed: ADR, TASK and BUG series collide three ways (Bridge ADR 006–229 / TASK 001–066 / AP 001–147; Avilo ADR 001–053 / BUG 001–047; CVN ADR 001–016 / TASK 001–040), and the later-arriving side renumbers, following the precedent `docs/TASKS.md` already set for the TASK-029, TASK-032 and TASK-033 collisions. Two open items are recorded honestly in the plan doc §10 rather than assumed away: CV Naturals' `packages/module-host` checkout has `dist/` but no `src/`, so Stage 2 must confirm it is not a divergent copy; and `packages/db` is Postgres-only, so Stage 2 must confirm a Local-Plane registration path exists for a sqlite-dialect Module or record a blocker.

## ADR-231 — Ten rules from Avilo and CV Naturals promoted to all-Module Bridge canon; the Electron rules deliberately left behind (2026-08-16; attach: TASK-067; AP-149)

**Context.** The directive was that "all rules in avilo and CVN are added as rules within all modules and beyond of relationship OS and adhered to." Taken as written that is a request to merge three rule sets, and rule sets do not merge cleanly: Avilo's are prose in `docs/decisions.md` earned against 47 logged bugs, CVN's are a compact tier model close to Bridge's own, and Bridge's live in an always-loaded `CLAUDE.md` whose own §Context section flags every added instruction as "a reviewed recurring cost." Adding fifty-three ADRs' worth of prose to an always-loaded file would degrade every future session to pay for rules most of them never touch.

**Decision — promote ten, and only ten.** Encode-knowledge-as-data (Avilo's house rule, and the one most of its ADRs are applications of); never fabricate a figure, with "unknown" as a first-class result; generated UI **binds, never carries** (ADR-041); an author declares which levers it changes and is held to it (ADR-045); where the server knows the truth the server speaks first (ADR-047); "already configured" is not a refusal (ADR-052); a fresh database is not a test; blast-radius before finishing; present-not-absent (CVN ADR-001); and a task closes only when its outcome is true in the running app. They are written into `CLAUDE.md` compressed to three bullets rather than reproduced at source length.

**Rationale.** The selection test was not "is this rule good" but "does this rule change what an agent does on a Module that is not the one it came from." Each of the ten passes: they are all about the relationship between what a system claims and what is true, which is the failure mode Bridge's own ADR-229 was — a false liveness verdict tearing down a live window after a missed affected-neighbour check. Two are worth naming specifically. **Generated UI binds, never carries** and **declare-your-levers** are both structural: they make a guarantee hold by construction rather than by policy, and ADR-045 exists precisely because prompt text failed *twice* on the same defect (BUG-031, BUG-033) before someone stopped writing prompt text and found two model outputs that could be checked against each other. That is a lesson about how to enforce rules at all, and it applies far outside accounting.

**Rejected.** *Promoting everything* — degrades every session's context budget for rules most will never reach; `CLAUDE.md` §Context makes this an explicit governance constraint, not a preference. *Promoting Avilo's Electron packaging rules* — Bridge is Tauri. These rules are correct in Avilo's repository and false at every point of use in Bridge, and a rule that is false everywhere it is read is worse than an absent rule: it trains readers to skip the list it lives in. *Promoting Avilo's strict AI posture globally* — this was the sharpest conflict, and the user resolved it by generalizing Bridge's AI canon to Avilo instead, with Avilo's posture moved to Module-scoped policy (ADR-232). *Importing Avilo's ledger shape* — Avilo has no execution queue and no approval gate; that is a gap, not a rule, and Accounting adopts Bridge's tier model instead.

**Consequences.** `CLAUDE.md` grows by roughly twelve lines, a cost accepted under AP-149 against ten defect-earned rules. Avilo's full rule set is not lost: it stays in its own repository, and its stricter half is seeded as Accounting's declared Module policy under ADR-232. Everything promoted here already had a home in Bridge's existing canon — "never fabricate a figure" strengthens the existing "real connected data or honest empty states" by naming the positive obligation instead of only the prohibition — so nothing promoted contradicts what Bridge already held.

## ADR-232 — Governance is a per-Module declared policy under Intelligence, not a global constant or a line in a prompt (2026-08-16; attach: TASK-068; AP-150)

**Context.** Avilo's AI canon and Bridge's are irreconcilable as global constants. Avilo: "The app works fully with no model configured. AI is additive, never load-bearing. No model call without a button press." Bridge: Automations that start Agent Runs, a learning agent, an Avatar that acts ahead of the user, and the stated principles "adapt before asking; learn before acting." Whichever is adopted globally destroys the other product. Presented with that conflict the user rejected both framings and directed a third: generalize Bridge's AI rules to Avilo, and give **every** Module a governance surface, below its Intelligence Section, where the user declares what is and is not allowed — with Avilo's specific rules seeded there for Accounting.

**Decision.** A `governance` block in `module.yaml` / `ModuleManifest` carrying user-editable `allow` and `deny` entries, and a new `ModuleGovernanceSection.tsx` rendered from `ModuleSurfaceLayout`'s `below` slot immediately after `ModuleIntelligenceSection`, on every Module. Accounting's block is seeded with Avilo's posture: no model call without an explicit user action; one key and one reader; a model may choose but never invent; only a formula may introduce a new id; client-scoped levers carry no client id; an external agent reaches configuration and never the books; every configuration change leaves a state you can return to; the deterministic narrative is the source of truth.

**Rationale.** This is Avilo's own house rule applied to Avilo's own rules. Avilo's four founding inversions all took knowledge that had been compiled into JavaScript — row-label regexes, hard-coded column indices, inline metric arithmetic — and made it a persisted, correctable row. Its AI posture was the one piece of important knowledge it left in prose, and prose is exactly where it failed: BUG-029 through BUG-045 are all the assistant claiming work it had not done, and ADR-045 records that prompt text failed twice on the same defect before the team stopped writing prompt text. So the strictest rule in the codebase was the least enforceable one. Moving it into a declared block the engine reads is the same move, one level up. **When you are about to encode knowledge in code — or in a prompt — check whether it should be data.**

Placement under Intelligence is not decoration. `ModuleIntelligenceSection` already answers *what this Module can do*, manifest-sourced and read-only, linking to Module Detail for the interactive surface. Governance answers *what it is allowed to do*, from the same manifest, in the same slot, with the same honest-empty-state discipline. A capability list a reader cannot see the limits of is half an answer, and the two halves should not be on different screens.

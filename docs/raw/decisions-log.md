...
## ADR-054 — Security P0 API request-boundary hardening: mutation-gated auth middleware + verifier-tied fail-closed CORS + rate limiting (2026-07-13)
...
- **Consequences**: `ApiContext` gains required `authenticated`/`verifying` fields (type-honest for a security boundary), forcing all 6 `createCaller` test harnesses to declare trusted in-process auth (`authenticated:true, verifying:false`). New dep `@fastify/rate-limit ^10.3.0`. Default rate-limit store is in-memory per-process; a shared Redis store is the multi-instance follow-up (noted in code). api test suite 53→58 green. BUGS.md H1/H1a/H4 → RESOLVED. Marking the roadmap Batch DONE is deferred to `docs/APPROVALS.md` AP-010 (not self-approved).

## ADR-055 — drizzle-orm 0.45 upgrade (SEC-3): handle the DrizzleQueryError `.cause` wrapping so 23505 ledger-concurrency detection survives (2026-07-13)
...
## ADR-056 — XP-1 cross-OS Tauri shell compile: gate objc2 to macOS, keep macos-private-api inert-unconditional, native crate stays outside the turbo graph (2026-07-13)
...
## ADR-057 — Agent-quality eval: pure AQV reducer + in-memory EvalStore behind a port, implementing the existing eval-model doc (2026-07-14)
...
## ADR-058 — RLS-as-code (SEC-5): FORCE RLS + tenant/visibility policies, prod boot guard in persistent ports, pglite-superuser caveat (2026-07-14)
...
## ADR-059 — Membership checks (SEC-6): enforce caller∈members on the membership-surface procedures, seed the pilot member, defer blanket enforcement (2026-07-14)
...
## ADR-060 — SEC-7 medium-severity closes: Recon SSRF denylist, pino log redaction, drop client-asserted linkedin from the trust path (2026-07-14)
...
## ADR-061 — PI-1 provenance/taint tagging: additive `trustOrigin` from ingest edge → Memory + ledger, untrusted-by-default at the intake seam (2026-07-14)
...
## ADR-062 — MEM-1 the Memory table: `MemoryStore` port + `memories` table, authority-scoped at the store boundary, capture→Memory wired at the sensors hub (not apps/api) (2026-07-14)
...
## ADR-063 — PI-2 tainted-context egress gate: a STRUCTURAL pipeline gate (not a wired policy), `external:send` only, require_approval not block (2026-07-14)
...
## ADR-064 — PI-3 dual-LLM quarantine + spotlighting: a tool-less `ContentGuard` returning only typed extraction, local-plane adapter in @bridge/models (2026-07-14)
...
## ADR-065 — Batch-6 foundations: the `policy_params` typed tunable space + the `capability_manifests.kind` registry discriminator (2026-07-14)
...
## ADR-066 — EVAL-3 baseline-vs-candidate two-gate promotion, wired into `capability.approve` (2026-07-14)
...
## ADR-068 — REG-1 Component Registry: two-tier (structural pure → semantic pgvector) overlap detection (2026-07-14)
- **Decision**: `@bridge/core/src/capability/registry.ts` — `structuralSimilarity(candidate, existing)` (pure Tier 1: weighted `kind` 0.4 / `name` 0.4 / `permissions` 0.2); `findOverlaps(candidate, manifests, opts, model?)` runs Tier 1 first and escalates to Tier 2 semantic (`cosineSimilarity` over `ModelProvider.embed` vectors) ONLY when Tier 1 is inconclusive (a mid-band score between the low cut and the near-duplicate threshold `0.82`). Reuses `capability_manifests` + the ADR-065 `kind` discriminator as the registry. The Learning Agent runs `findOverlaps` BEFORE proposing a new capability.
...
## ADR-069 — VAR-1 Variance Adjuster: a bounded single-parameter nudge proposed as a governed diff (2026-07-14)
...
- **Consequences**: **59/59** green. `core/test/variance-adjuster.test.ts` proves 3× "too casual" VETTED vetoes propose a governed `tone_threshold` increase AND that the nudge cannot cross the ceiling (`clampedAtCeiling`). No parameter persists until the proposal is approved (ADR-065 in-memory store, Drizzle deferred).

## ADR-070 — GOV-1 org-health rollup + Governance auto-approve enacted as a SYSTEM gate (minor only) (2026-07-14)
...
## ADR-071 — Batch-6 integration + verification: in-memory EvalStore/PolicyParamStore wiring + the db coverage-leak fixed with real tests, not a lowered floor (2026-07-14)
...
## ADR-072 — AGENTS-1 "PromptAssembler" = the ADR-027 RunContextAssembler, extended with layers 1–2 (not a second assembler) (2026-07-14)
...
## ADR-074 — AGENTS-2 onboarding-profile → Chief-of-Staff persona, resolved SERVER-SIDE; roster stays 4 agents + Communications skill (2026-07-14)
...
## ADR-075 — XP-2 (native installers/capture) + XP-3 (mobile Expo rebase) deferred to an infra-gated Batch 8 (2026-07-14)
...
## ADR-076 — PKG-1 sandbox floor: an executable capability is gated at package install, declared via a manifest `execution` spec whose PRESENCE marks it executable (2026-07-14)
...
## ADR-077 — PKG-2 Commons supply-chain trust: manifest signing + verify-on-install + TLS-by-default + a community-origin trust floor, with the crypto primitive bound at the seam (2026-07-14)
...
## ADR-078 — BLUEPRINT-1: a WorkspaceBlueprint is frozen as a versioned, Commons-publishable manifest whose declarative-ness is enforced by a closed-key parse gate (2026-07-14)
...
## ADR-079 — CONSOLIDATE: the house test runner is `node --test` + per-package coverage floors (NOT vitest), and core-barrel coverage-dilution is fixed by recalibrating downstream floors (2026-07-14)
...
## ADR-080 — AP-007 applied: the long-term optimizations / Memory-lifecycle / VM-isolation plan is ingested into the roadmap as additive per-phase bullets, with NO batch reorder (2026-07-14)
...
## ADR-081 — XP-2 honest partial: the unsigned native-installer path is delivered and proven (3-OS bundle CI + Tauri bundling + generated icon set + a real local macOS build); only signing/notarization stays cert-gated and open (2026-07-14)
...
## ADR-082 — XP-3 confirmed fully BLOCKED and deliberately NOT scaffolded: no mobile (Expo) app exists on any accessible ref and Expo/RN deps cannot be installed here (2026-07-14)
...
- **Why**: a DONE-WHEN that cannot be proven must not be claimed, and fabricating a mobile scaffold would either be dummy code or break the build — both violate the real-data / no-fabrication rules (ADR-075 already held that fabricating infra-gated DONE-WHENs is unacceptable). The honest outcome is to document the exact blocker so a future session doesn't re-hunt for a non-existent app.

...
## ADR-113 — DealPilot durable state shares one exclusively owned Local Plane database; Source credentials live only in an explicit OS vault (2026-07-18)
- **Decision**: DealPilot runtime state is one versioned, Organization-scoped aggregate behind the generic atomic Local Plane state port. The file-backed adapter stores it in an adapter-owned `local_state` table, uses revision compare-and-swap for shared-client contention, and takes a heartbeat-backed cross-process lock before opening the embedded database. Ownership canonicalizes absolute paths and existing symlinks before locking. Drizzle and Local Plane adapters share that one PGlite client and the existing `BRIDGE_LOCAL_DIR`; they never open competing clients on one directory. Record/Relation/capture/Gmail/spend updates that must agree commit in one reducer. Initialization failures close partially opened clients and release ownership before propagating the original and cleanup errors. Desktop supplies its Tauri app-data directory, `BRIDGE_DEALPILOT_CREDENTIAL_VAULT=os-keyring`, and its parent PID; shutdown requests bounded graceful API cleanup, while parent-loss supervision prevents a crashed shell from orphaning the database owner. Source secrets use MIT `@napi-rs/keyring` behind `SourceCredentialVault`; every write gets a unique keyring account and every opaque reference is rebound to the requested Organization and Source on metadata/read/delete. Runtime boot without durable Local Plane storage or an explicitly approved secure credential provider fails. The in-memory vault and state adapter are test-only.
...
- **Alternatives rejected**: retain process-local maps with startup warnings; open a second PGlite client or a second directory for DealPilot; persist credential ciphertext or plaintext inside `local_state`; use stale `keytar`; silently fall back to memory on servers; add a numbered Drizzle migration that would collide with RM4 `0015` or TASK-010's next migration.
- **Consequences**: only one Bridge process may own a file-backed Local Plane directory at a time, and shutdown must close wiring to release ownership. Equivalent filesystem spellings cannot bypass that ownership. Gmail pending receipts, cursors, checkpoints, visited tokens, stable message IDs, and settlements recover after restart; relation backfill and capture materialization are atomic/idempotent. Failed concurrent Source creation can delete only its own keyring entry, never another request's winning credential. Web/server-only deployments must inject an approved vault or fail at boot. The adapter-owned `CREATE TABLE IF NOT EXISTS local_state` needs no numbered migration metadata, so RM4 `0015` and TASK-010 sequencing are untouched. Live macOS keychain interaction is proven; live Google, physical-mobile/signing certification, and cryptographically verified OS re-authentication remain external gates rather than inferred claims.

## ADR-114 — The durable desktop sidecar uses a per-launch capability, not loopback as an authentication boundary (2026-07-18)
- **Decision**: Every release-sidecar launch generates 256 random bits in Rust, passes the capability to the Node child through `BRIDGE_SIDECAR_TOKEN`, and injects it into Tauri webviews only as an initialization-script value. The web client sends it in `X-Bridge-Sidecar-Token`; both the typed client and legacy Google helpers share the injected URL and bearer/capability headers. Fastify constant-time verifies the capability before every sidecar request except the external Google OAuth callback and CORS preflight. The launcher sets `API_HOST=127.0.0.1`, and Fastify independently forces loopback whenever sidecar mode is active. Sidecar exit first uses an authenticated loopback shutdown request on every OS, then a bounded signal/kill fallback. The sidecar sets a closed Tauri-origin CORS allowlist, file-backed Local Plane wiring counts as persistent, and the capability authenticates only the server-pinned pilot identity. It never supplies password AMR or credential re-authentication evidence. The header is log-redacted and never stored or placed in a URL.
...
- **Alternatives rejected**: rely on loopback/random ports; reuse a Supabase bearer as the desktop capability; put the capability in a URL; persist it across launches; exempt health/private reads; treat sidecar possession as Human re-authentication.
- **Consequences**: stale or cross-launch callers fail 401, while the Tauri client remains offline-capable without inventing a Human identity proof. Google OAuth's provider callback remains separately reachable by design and is protected by ADR-115's single-use state.

## ADR-115 — Google OAuth state is random, hashed at rest, short-lived, and atomically consumed (2026-07-18)
- **Decision**: `google.connectUrl` issues 256 random bits only after authenticated membership checks. The raw OAuth state goes only to Google's authorization URL; Bridge persists its SHA-256 hash with Integration, initiating Human, and a ten-minute expiry inside the atomic Organization-scoped Local Plane. Callback handling consumes the state before denial handling or code exchange, derives the Integration from the server record rather than callback input, validates the initiating Human before provider access, and rechecks membership at the token-persistence boundary after exchange. Unknown, malformed, expired, replayed, legacy predictable, wrong-Integration, and no-longer-authorized states fail closed.
...
- **Alternatives rejected**: exempt the callback and trust CORS/loopback; use Integration ID as state; store the raw state; use a process-local map that loses valid flows on restart; exchange first and validate later.
- **Consequences**: authorized OAuth flows survive API restart, while account-substitution and membership-revocation races cannot reach Local Plane token persistence. The opaque random state necessarily transits the OAuth URL but carries no secret, identity, or private payload.

## ADR-116 — Privileged Tauri webviews never navigate externally; OAuth uses the system browser (2026-07-18)

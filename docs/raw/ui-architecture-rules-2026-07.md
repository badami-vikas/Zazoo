...
## 5h. Toggle options always live in the header (user directive 2026-08-30, AP-168)

**Rule, generic and unconditional: a toggle renders inside its surface's header row, never in the
surface's body.** This is the same rule §5's Shell paragraph already states for the Module page
toggle ("Toggle strip at the very top of the module header"), restated here as its own numbered rule
because that phrasing read as one layout note about one strip, and every other toggle in the product
was left to decide for itself. It is not about the Module toggle. It binds *every* segmented
switch/tab strip that swaps what a surface shows, at every level:
...
- Section-level tab strips (Intelligence Section's Agents · Automations · Integrations, the
  `/intelligence` route's Agents · Automations · Skills · Integrations, Second Brain's tabs) → that
  Section's or route's own header row, not a free-floating strip above its content.
- Anything generated by the compiler that swaps sibling content the same way. The compiler has no
  separate allowance.

Consequences, all of them things a reader could otherwise get wrong:

- **The header is where a user looks for "what am I switching between".** One resident position
  across every surface is the whole point (Jakob's Law, C-28); a toggle that floats in the body
  moves the answer per-page.
- **A toggle is never a second row of its own.** If it does not fit at label width, it goes
  icon-only inside the same `h-14` (existing `Header.tsx` behaviour) — it never wraps and never
  earns a new row. This is the same "never a separate arrow row of its own" correction §5 already
  applies to the Insights chevron.
- **This does not promote non-toggles into the header.** The toolbar (List · View · Search · Filter
  · Custom actions · 3-dots · Insights chevron) stays exactly where §5 puts it: the row BELOW the
  header. The View dropdown switches render mode of one dataset and is a toolbar slot, not a toggle;
  the List dropdown likewise. A toggle swaps the Database/Page being shown; those two do not.
- **A single-item "toggle" is not a toggle** and renders as the plain centered Title-case name
  (existing rule, §5 / requests R-013) — still in the header.

**Enforcement is NOT claimed here.** `ui-conformance.test.mjs` (§10) does not yet assert toggle
placement, so today this rule is documentation the way §10 says rules used to be — and §10 is
explicit about what that produced. Adding the assertion (a page that renders a segmented
switch outside its header row fails) belongs to the TASK-061 burn-down, with the pre-existing
divergences in the same ratchet, rather than being asserted as done here.

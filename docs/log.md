...
What shipped: real HTTP fetchers on global `fetch`; a source catalog in code with per-Organization toggles in rows (`jobpilot_sources`, migration 0045); a `deadline` column (migration 0044) whose only producer is a curated target list, because no ATS feed carries one; a Sources page; and a 6-hourly Automation whose sweep body is the same function the manual button calls. ADR-265 and ADR-266, AP-172 and AP-173.

**Four things the build found that the plan did not.** The first-declared `trigger: "Every 6 hours"` **would never have fired** — JobPilot's Agent had no runtime id and no `plane`, so wiring.ts's Module-Automation materializer skipped it silently; that is precisely the defect ADR-179 built the scheduler to end, caught only by reading the materializer instead of trusting the manifest. A live sweep ranked *"Product Management Intern (Summer 2027)"* at a perfect **1.00** against a second-year MBA recruiting full-time — scoring cannot catch this because the feeds carry no employment-type field, so full-time filtering is title-level, and its test pins that `"Internal Audit Manager"` survives the word-boundary. The UI gate caught **five `text-[11px]`** additions below the rulebook's 12px accessibility floor, in my own new component. And a `@bridge/core` test that had been failing all along was a **timezone bug, not JobPilot's**: `timeOfDayBucket` buckets the LOCAL hour by design, but the test hardcoded `22:00Z` and so only passed on a UTC machine — now built from a local-hour helper and green under UTC, America/Chicago, Asia/Kolkata and Pacific/Auckland, three of which were red before.

**Two corrections of my own, both times the code was right and my test was wrong.** The repeat-sweep unique constraint fired correctly; my `assert.rejects` matcher checked the wrapper message instead of drizzle's cause chain. And `listJobs` with a deadline window returns `total: 2` for two matching rows — I had asserted 4, expecting `total` to count the Organization; counting the *window* is what makes the router's `hasMore` correct on a filtered page.

**Evidence, not assertion.** 45 sources after probing 150 candidate slugs — **66 failed**, either 404 (no ATS exposes a directory endpoint, so every slug is a guess) or resolving with zero relevant postings; both are excluded on the same rule. A full live sweep: **7,308 fetched → 460 kept** across 38 companies, 93 dropped as not-full-time, zero internships leaking. Persistence proven against real pglite Postgres, not asserted: toggle upsert, the deadline window's ordering and null-exclusion, and the duplicate-url rejection. Central: typecheck 57/57, build 31/31, jobpilot 137/137, ui-rules 4/4, vocabulary/agent-context/no-dummy all OK.

**Named ceilings rather than smoothed.** `scoreJobFit` resolves to few distinct values when a profile carries only categories, so most keepers tie at 1.00 and the sort degenerates to catalog order — one large board can fill the top of the feed; marked `ponytail:` in sweep.ts with its upgrade path. And the strategic ceiling the toggle page states outright: these ATS boards are overwhelmingly venture-backed startups, the population **least** likely to sponsor a visa, and the employers that matter for a sponsored MBA search — MBB, Big 4, Microsoft, Apple, Google — are on none of them (all three 404 on both Greenhouse and Lever, probed directly). Volume was never the constraint; relevance is.

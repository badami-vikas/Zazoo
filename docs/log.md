...
- **2026-09-07 — a Module can be deleted, and the confirmation asks which of the three things you meant (TASK-113)**: the rail could only HIDE, which is this browser's localStorage and never reached the server, so a Module the user was finished with stayed installed forever. Delete now sits beside Hide in the rail menu and opens a confirmation with three answers, each stating its own consequence: delete the Module and keep what it collected, delete the Module and its data, or hide it and delete nothing. `modules.uninstall` takes `deleteData` as data and does exactly that — human identity only (AP-182 names irreversible loss as a thing governance blocks on), refusing a Module another Module is attached to and naming the dependent, and returning the count of Records removed rather than a bare success. `LocalStateStore` gained `remove` and `ModuleStore` gained `deleteVersions`, because writing an empty document leaves a row that reads back as "this Database has no Records" — a different fact from "this Database is gone" (ADR-247) — and `deprecated` is a Module that still exists and can be rolled back to. ADR 2026-09-07 "Deleting a Module is a question with three answers". An existing rail assertion said the rail must never call `modules.uninstall`; it narrowed to "hiding never reaches the server", which is still true and is the fact it was protecting. Not landed: another member's saved View on a deleted Database is left behind (the `view_configs` delete policy is owner-only, and widening a security policy to tidy up is the wrong trade), a share grant pointing at it resolves to nothing, and the Module's folder under `~/Documents/Bridge/` is untouched because those are the user's own files and deserve their own question.
- **2026-09-06 — Notion database parity, measured from code and closed in four slices (TASK-108/109/110/111)**: the user asked for every Notion database feature to be present in our tables. Two read-only inventories were taken first, scoring a capability absent whenever it was only a type-union member, a disabled control or a hook with no caller, and the result is `docs/raw/notion-database-parity-2026-09-06.md`, which supersedes the 2026-08-10 audit's per-feature scores. The grammar landed first in `@bridge/tables` so four parallel slices could share it: nine column kinds (longText, email, phone, person, files, status, rollup, autoNumber, button), three view kinds (list, timeline, chart), the number/date/choice/checkbox filter operators with `filterOpsForKind` deciding which a column may offer, and the view mechanics Notion persists — sub-group, row height, cell wrap, column widths and order, a frozen column, per-column summaries, page size, card settings — every one optional so an older saved List resolves unchanged. Then, as PRs #92, #93, #94 and #95: List, Timeline and Chart Views plus a real Gallery (no charting dependency, three shapes in plain SVG); cells that render and edit as their kind, columns that resize, reorder, wrap and freeze, and table grouping, which retired the "grouping is not wired for this View yet" reason on every column; a filter builder with per-kind operators and the first control that ever wrote `filterMatch`, a multi-level sort editor, saved Lists that can be renamed, duplicated, deleted, shared and made the default (migration 0051), and one pagination control; and a server that queries — `moduleRecords.list` searches, filters, sorts, groups and pages over every stored Record before slicing, values are coerced and checked against their kind with `required` enforced, a relation target naming no Database is refused at registration, and rollups are computed on read. Follow-ups landed in the same run: the three new Views held their settings in component state so a saved List forgot them, `ROW_HEIGHT_PX` and the filter helpers were declared but never exported, and the server's new default window silently truncated both Module surfaces — the Record page could not open the 51st Record. ADR 2026-09-06 "One table grammar, four parity slices". Not landed and stated on the tasks: formula on a Module Database stays refused, `autoNumber` is validated but not assigned, a two-way relation is still absent, filters join with one all-or-any switch rather than nested groups, a button cell has no Action runner, grouped bodies are not windowed, and the seventeen Notion features the audit lists as untouched — row comments, row history, sub-items, row templates, bulk property edit, manual row order, row undo and trash, CSV import and export, cell-range paste, database automations, a public API, external sync, a shared-View route, web publishing, database- and column-level permissions, linked views and locked views.
- **2026-09-06 — the Module surface obeys: toolbar, right-click columns, grid lines, add/rename, Sections once, inline policy, an onboarding turn, a keychain named Bridge (TASK-104/105/106/107)**: the user's first Module Page fought them at every control (BUGS 2026-09-06, verbatim). Fixed across four slices and three subagent PRs (#89, #90, #91). The toolbar is now List/View/Search left and Filter/⋮ right with nothing else — a page's custom actions became a labelled group inside the ⋮. Column commands open on right-click of the header (Enter/Space/ContextMenu keyboard route) and the per-column ⋮ is gone; header and body cells draw vertical rules. `SCHEMA_MUTABLE_SPECS` was the reason "Add column" and rename were dead on Module Databases: the governed schema path now reaches them, `ColumnOverlay` gains `added[]`, adding is its own answer (`canAddColumn`/`addReason`, false with a server-authored reason for Accounting and D2C whose rows are real sqlite columns), and ModulePage asks for the capability like AccountingPage. Adding a Record opens `/module/:name/:pageId/new` instead of swapping the table inline, which is why Intelligence and Governance rendered twice; `RecordPage` no longer mounts `RecordSections` at all. Governance policy is edited in place through the existing governed overlay with no Edit mode, an incomplete rule is never sent and says so, and "Manage in Module Detail" is deleted — it linked the page to itself. Standing explainer prose became `title` tooltips or was cut. The Builder now spends ONE onboarding turn on its own session after installing a Module that declares outside software or has required columns. The OS keyring service is renamed `com.bridge.dealpilot` → `Bridge` with a read-through fallback, because the item holds this app's own secrets. ADR 2026-09-06 "The Module surface obeys". Not landed: Add List needed no code (`view.saved.*` accepts any Database id, proven end to end on a migrated Local Plane against unfixed source) — the control was reporting a real server error, most likely an installed plane migrated before 0047; hand-written surfaces keep the inline Add path; the delete preview now says it did not inspect Views rather than claiming none are persisted; macOS still re-prompts after every local rebuild because each build is ad-hoc signed.
- **2026-09-05 — the Builder finishes the job; Approvals have no page; the chat panel declutters (TASK-103, TASK-097 done, TASK-101)**: the user's first real build ended in `Could not register academics-module/module.yaml: … options must be an array of strings` after the agent told them to "go to Modules and install" (BUGS 2026-09-05, verbatim). The Builder's grammar never said how a relation names its target (`relation_target`), the chat lane stopped at registration, and the copy was written for developers. Now a rejected definition goes back to the agent's own session (≤2 rounds, verbatim reason), the accepted Module is installed through the governed `modules.install` path with a `user_pref` ask decided by the chatting user in the same turn as a recorded Human decision, and `PLAIN_LANGUAGE_RULES` (no internals vocabulary, first principles, executive style, never "go install it") ride in both lanes — ADR 2026-09-05 "The Builder installs what it builds and speaks plainly". Same report, two subagent PRs merged: #88 deletes the Approvals page (`/approvals` → Tasks, unanchored approvals as "Waiting for your yes" on the Task Manager, dead `relationship.outstandingMaterializations` call removed) and #87 removes the composer footer and Module dropdown and names sessions `<Module> · <date>` (multi-Module attach now has no UI affordance — noted on TASK-101/093). Follow-up the same evening (BUGS 2026-09-05 "the chatbot claims academics is in my side bar when it isnt"): the live repair loop worked but install left the row `promoted` while every surface lists only `available`; the lane now promotes, re-reads the row and only says "in your sidebar" when it is; the left nav re-reads `modules.list` on `bridge:modules-changed` after a chat turn; the user's row was promoted by hand on the live plane.
- **2026-09-05 — merged PRs #85 and #86, rebuilt and reinstalled the Egg (TASK-097 done)**: PR #85 (`@` addresses an Agent; Chief of Staff stays the face, TASK-101) merged with its migration renumbered 0049 → 0050 because the ledger `superseded` CHECK already held 0049; PR #86 (the Builder discovers before it designs) merged with its task renumbered TASK-101 → TASK-102 — two parallel subagents had both taken TASK-101. Installer rebuilt from main 8771081d (Tauri dmg step failed only because the previous Downloads dmg was still mounted at /Volumes/Bridge; the dmg was written with `hdiutil create` from the signed .app) and installed. Live evidence on the reinstalled Egg: migrations 0049+0050 applied, first tick wrote 255 `superseded` rows, `action.listPending` total 2 in plain language, Canvas sync parked.
- **2026-09-05 — the approvals pile survived the fix: migration 0049, fenced sweep, parking foreign-Module Automations, plain-language proposals (TASK-097 reopened)**: a read-only copy of the installed Egg's Local Plane showed 375 undecided rows, 0 `superseded`, and no Automation Run since the reinstall — migration 0015's `ledger_user_decision_check` refused `superseded`, the sweep threw, and the scheduler tick died every minute (the 2026-09-04 tests ran on in-memory stores with no constraint). Fixes: `0049_ledger_superseded_decision`; the sweep in its own try/catch; boot parks any active Automation whose Skills belong to a Module outside the profile (the stale Academics "Canvas coursework sync" row); replay carries the persisted Skill; `describeProposal` sentences on Home, Task Page and Approvals. BUGS 2026-09-05 (verbatim), ADR 2026-09-05 "Ledger admits `superseded`; approvals speak plain language; housekeeping never stops Automations". Evidence: `superseded` insert seen failing on a scratch copy of the real plane before 0049 and passing after; db 239/239 (metadata head moved to 0049, no snapshot — constraint-only like 0038); core 805/805 (replay test now expects the persisted Skill); web 242/242 + pending-work 6/6; vocabulary, ui-rules and agent-context gates OK; full `@bridge/api` suite 0 failures.
- **2026-09-05 — `@` in Chat addresses another Agent; the footer names Modules as Modules (TASK-101)**:
  the user read the Chat panel's "On this conversation: TaskManager" as Agents and asked why other
  Agents show at all when Chief of Staff is the face (BUGS 2026-09-05, verbatim there). The line was
  TASK-093's Module list; it now reads "Working in:" with the explanation on the element. Typing `@`
  in the composer opens a keyboard-driven picker over `chat.agents.list` — installed Modules' declared
  Agents resolved to runtime ids and kept only while active in the Organization, Chief of Staff never
  offered — and inserting one sends `mentions: [runtimeId]`; `turn.send` refuses a non-active id before
  writing and records each on the assistant turn as an `addressed_agent` ref (core kind, db CHECK,
  migration 0050), badged "Addressed to X · answered by Chief of Staff". Routing is unchanged and said
  so: the per-Agent lane handoff is NOT LANDED. ADR 2026-09-05 "`@` addresses an Agent; Chief of Staff
  stays the face". Evidence: `chat.test.mjs` 9/9 (2 seen failing first), `chat-mentions.test.ts` 2/2,
  api + web typecheck clean.
- **2026-09-05 — the Builder discovers before it designs (TASK-102)**: the user asked why "build me
...
- **2026-08-16 — Accounting and D2C become real, running, sqlite-backed Modules with enforced governance (TASK-074/TASK-072, ADR-246/247/248/249)**:
  ...
  the stock-costing engine stay explicitly out of scope; TASK-074/TASK-072 close on "real, running,
  enforced" per their Prototype tests, not on feature parity with either donor app.
- **2026-08-14 — DevPilot D2 shipped: engineering-assist Skills draft PR reviews, best-practice suggestions, and Issue triage (ADR-237/AP-155, TASK-071)**:
  User directive: *"start on the next task"*, disambiguated to "DevPilot D2" via a clarifying question against the general queue. Three new Skills (`devpilot.reviewPr`, `devpilot.suggestPractice`, `devpilot.analyzeIssue`) fetch fresh GitHub content per draft (D1 never stored PR/Issue bodies or diffs), spotlight it inline in ONE model call, and reuse the existing `pipeline.propose()`/Approvals ledger — no new tables, no new approval UI, still zero `external:send`. New `github_intake` untrusted taint source; dedicated `role-devpilot-reviewer` Agent (least-privilege split from D1's read-only tracker); 3 Human-triggered governed Automations; `SkillManifest.budget.maxCallsPerDay` exercised for the first time. Live verification caught a real bug — the devpilot Module manifest's version wasn't bumped despite the new capabilities, violating the platform's per-version immutability rule — fixed (`0.1.0`→`0.2.0`). Verified: `@bridge/integrations-github` 16/16, `@bridge/module-manifests` 25/25, full `@bridge/api` green (one pre-existing unrelated scheduler-test flake confirmed identical on the unmodified D1 baseline), `@bridge/web` typecheck + `ui-conformance.test.mjs` 13/13; live browser walk confirmed both pages render the new controls correctly and all `devpilot.*` calls return 200 — the actual model-drafting path wasn't exercised live, no GitHub PAT or model key available in this environment. D3 (Jira), D4 (comms triage), D5 (news radar), D6 (Work Brief) remain unstarted; posting a drafted review to GitHub is the natural next `external:send`-gated slice.
...
- **2026-08-03 — Unfinished-work audit: WhatsApp branch pushed, four production/canon fixes landed, five tasks queued**:
  ...
  **Queued (AP-095):** TASK-038 plaintext OAuth tokens + the ownerless 2026-07-08 SECURITY M1-M6 multi-tenancy
  block; TASK-034 production config (incl. `policyParams` bound to an in-memory store OUTSIDE the persistent/
  in-memory split, so governance gates always read defaults and every write dies at restart while a real pgTable
  sits unused); TASK-035 unreachable Pages, uncancellable child Runs, unmounted `ControlPanel`, 52 of 214
  client-less procedures; TASK-036 green CI + the emit-only `EventBus` (designated absorber for test/CI blockers);
  TASK-037 unlanded-work recovery. CI's `check:vocabulary` blocker was diagnosed and routed to TASK-036 per the
...
# 2026-08-02 — WhatsApp linked to the Relationship Module (TASK-030, ADR-159)
- User asked for the WhatsApp Module linked to the Relationship module, with chat data on People/Community page timelines. Shipped as a key-space lookup, never a matcher: chat id → `whatsapp:+E164`/`whatsapp-lid:<id>` → `local_people.dedupe_key` (`modules/whatsapp/src/link.ts`, pure, 100% line+branch coverage). Five explicit link states; a LID chat NEVER matches a phone Person even when the digits coincide — a test asserts exactly that case.
- **Fixed a silent drop found while wiring**: `stageExtraction` had computed `possible_duplicate` Signals since v1 and thrown them away after counting. They now commit to `local_entities` as `kind: "signal"` with deterministic ids (idempotent re-runs), same payload discipline as the Google intake path.
- New API: `whatsapp.relationshipLinks`, `whatsapp.chatLink` (read-only, Local Plane), `relationship.whatsappTimeline`. Person page Timeline gains a "WhatsApp activity" subsection: counts/timestamps only, joined at render time; no body, phone, or identity key crosses the wire, and nothing is written into cloud `events`.
- Honest limit stated in ADR-159 and on the surface: the cloud↔local person bridge is id equality (Google/Capture pattern); WhatsApp-origin local People have no cloud row, so on today's real data the subsection reports `no_local_record`/`no_whatsapp_identity` rather than fabricating activity. Promote path is future work. Community timelines report `community_unsupported` (no local Community store).
- Verified: `@bridge/whatsapp` 199 pass (link.js 100%), `@bridge/local` 37, `@bridge/web` 129, `turbo build --filter=@bridge/web` green, and `tsc -b apps/web --force` green — with the typecheck's meaningfulness itself verified by planting a fake procedure name and confirming it errors. `@bridge/api` full suite green (12m47s). NOT verified: live rendering against a real Person page (user runs `tauri dev`; no competing server started).
# 2026-08-02 (shell fixes) — session recovery affordances land (ADR-159), and the send-outcome audit seam is wired
- Today's splash-screen wedge (invalidated store, no escape but hand-moving `~/Library/WebKit` in a terminal) becomes two governed commands: `whatsapp_session_reload` (same store, fresh page — offered when not linked AND in the connected-but-chatless state hit live today) and `whatsapp_session_reset` (destroy window, `rename` the store dir to a timestamped sibling — never deleted, rename failure is a typed error with no deletion fallback — clear the store-id file, re-link via QR). Reset is confirm-gated in the Chats surface and appears only when not linked. Both are named ops; neither accepts caller-supplied code. ADR-159; BUGS entry filed and resolved.
- The send-outcome audit seam is wired at the one send site: `sendAutomatedMessage` in `engine.ts` now hands every `performAutomatedSend` outcome to `whatsapp.recordSendOutcome`, so the audit log the panels read holds what the gate actually decided. Recording only — an audit-write failure warns and never rethrows, because retrying a send over bookkeeping would be worse than a gap in the log.
- Fix 4 from the shell-defect list (dead `MESSAGE_WPP_DEPENDENCIES`) was already closed by the health-tripwire merge at this base; verified by a warning-free build rather than re-done.
- Verified: Rust 139 pass (was 133; +6 reset filesystem tests over temp dirs), 1 ignored; `@bridge/whatsapp` and `@bridge/web` (129) suites green; `turbo build --filter=@bridge/web` and `tsc -b apps/web` (after `apps/api`) clean. Proven by test: reset's rename/id-file/absent-dir behaviour. Proven by build/type only: UI wiring and the tRPC seam call. NOT verified: the commands against a live wedged session — today's wedge was cured manually before these existed.

# 2026-08-03 — WhatsApp extraction fidelity: the empty contact list and the undated chats (ADR-160)
- Both live defects were diagnosed against the bundle we actually ship (`vendor/wppconnect-wa.js`, wa-js v4.5.0, SHA-256 pinned) rather than against documentation or the standing hypothesis. The LID-migration explanation for the empty contact list was NOT confirmed and is not what the evidence shows.
- Contacts: `list({ onlyMyContacts: true })` is a client-side `filter(e => e.isMyContact)`, and `isMyContact` is a getter wa-js binds by DUCK-TYPING WhatsApp's webpack modules. A broken binding yields `undefined` for every contact and the filter returns an EMPTY ARRAY WITH NO ERROR. The filter moved into the op so its failure is visible: a non-boolean flag is UNREADABLE rather than `false`, and contacts-present-but-none-readable now throws instead of resolving as an empty success. What "contact" MEANS is unchanged — strangers who merely messaged the owner are still excluded, so the `contacts_and_messaged` staging volume is untouched.
- Chats: the sort in `syncedThreads` was never the bug. The prior fix listed three activity sources but built them EAGERLY inside one `try`, so `c.msgs.last()` ran before `c.t` was read and its throw discarded a perfectly good `c.t` — the lowest-priority candidate poisoning the highest. Candidates are now lazy, each in its own `catch`, `t` first (the only field declared on WhatsApp's ChatModel; `lastMsgTimestamp` and `msgs.last` have ZERO occurrences in the pinned bundle), with a `>1e11` milliseconds guard. Each summary carries `activitySource` naming the field that answered.
- Deliberately not done: dropping `onlyMyContacts` (would stage 35,298 People against ~998 known), and falling back to `WPP.whatsapp.functions.getIsMyContact` — that path could not be confirmed to exist in the vendored bundle, and guessing at a private path to patch a private path that already moved rebuilds the same silent-empty defect one build later.
- Verified: Rust 145 pass (was 139; +6 tests — lazy evaluation, ms normalisation, `activitySource`, and three on the contact op including one asserting the error names no one), 1 ignored; `@bridge/whatsapp` 248 pass; `tsc -b apps/api` then `tsc -b apps/web` clean; `turbo build --filter=@bridge/web` green. The fresh-worktree `@bridge/research` typecheck errors were a build-order artifact and cleared after building that package — no TypeScript was changed by this work. NOT verified: anything against the live account. The read-only console probe that settles which `activitySource` answers and whether the address-book flag is readable is recorded in BUGS.md with what each possible answer would mean.

# 2026-08-03 — TASK-032: learning observation loop v1 (generic Egg baseline) + Deal Copilot requirements handoff
- Deal Copilot freelancer requirements doc written and gap-patched against the verbatim roadmap-v2 requirement (outputs/2026-08-01-deal-copilot-agentic-freelancer-requirements.md): added generate-don't-configure onboarding pipeline, light-egg constraint, three knowledge layers, Commons cross-workspace archetype-mining deliverable, and the "never feels like training" / "every new workspace starts smarter" success criteria.
- LA2 observation loop landed as generic kernel machinery (`packages/core/src/learning/observation.ts`) composed over the existing MemoryStore — no new table, no migration. Signals are private Local-Plane episodic Memories; digest detects repeated (action, attribute) patterns and proposes suggestions through `casSupersede` lineages (annoyance-capped; any existing lineage — including rejected — suppresses re-proposal at the store level); only `acceptSuggestion` with a Human actor mints a `preference` Memory with provenance to the suggestion and evidence signals; `preferencesToMemorySnippets` feeds the run-context memory slot, making the LA1 exit criterion ("accepted Memory demonstrably changes agent output") a tested fact.
- DealPilot bound as first consumer via pure structural mapping (`modules/dealpilot/src/learning.ts`): Pursue/Review/Dismiss + DealProfile → generalizable attributes (industry, geo, deterministic SDE band, source); free-text reasons ride as data, never pattern attributes. No dependency edge added dealpilot→core.
- One defect found and fixed during TDD: a superseded suggestion row still claims `proposed` in its own content, so accept/reject state checks now consult the lineage HEAD (double-accept produced a confusing conflict error instead of "already accepted"; now typed).
- Verified: core 493/493 (observation.js 97.9% lines), dealpilot 96/96, api typecheck clean. Open follow-ups on TASK-032: API/tRPC surface, Avatar suggestion-review UI, digest Automation scheduling, retrieval fusion (LA5), promotion machinery.

# 2026-08-03 — TASK-032 second landing: learning.* tRPC surface behind a feature flight
- `learning.*` router group landed behind `Wiring.learningObservationEnabled` (env `BRIDGE_LEARNING_OBSERVATION=1|true`, default OFF; `BuildWiringOptions.learningObservationEnabled` override) per user directive to keep the loop toggleable. `learning.status` reports the flight state even when off; all other procedures fail closed with typed `PRECONDITION_FAILED`.
- Surface: recordDealDecision (server-resolved identity, `dealDecisionSignal` mapping, run-clock observedAt) · digest (proposes only, `deterministicUuid` lineage mapping for the uuid-typed persistent adapter — same convention as red-flag lineages; core gains an injectable `lineageIdFor` seam and stays browser-safe) · suggestions.list/accept/reject (accept is the sole preference-minting path; double-act = CONFLICT; cross-member = NOT_FOUND) · preferences.list. Signal rows no longer place non-uuid record ids into uuid columns (record id preserved in content) — found live against pglite (`string_to_uuid` 22P02), fixed at the core seam.
- Verified: core 493/493, api full suite 392 (391 pass + 1 pre-existing skip) with 3 new flight tests over real `buildWiring()` (off-fails-closed, full on-loop, cross-member authority), web typecheck clean. Open: Avatar suggestion-review UI, digest Automation, retrieval fusion (LA5), promotion machinery.

# 2026-08-03 — TASK-032 third landing: Settings → Learning observed-patterns review card
- `ObservedLearningCard` landed in Settings → Learning: flight-gated on `learning.status` (renders nothing when off/unresolved/unreachable — no dead controls), digest button, Remember/Dismiss per suggestion (accept remains the sole preference-minting path), accepted preferences listed with evidence counts and Delete (onboarding.forgetMemory; learning content parses as non-legacy so the red-flag guard does not apply). User copy states the suggested-then-accepted contract and private/Local Plane provenance.
- Verified: web 110/110 (4 new source-contract tests) + production build; live browser session against this session's own vite server showed the Learning section with the card honestly absent while the API refused tokenless requests (SEC-1 fail-closed — the API, started with the flight on, requires verified identity because BRIDGE_LOCAL_DIR makes stores persistent; sidecar-token mode refuses standalone start by design: "Managed sidecars require a parent-retained inherited loopback listener"). Positive live UI proof therefore rides on an authenticated session (Tauri shell or Supabase) — recorded as the user-at-keyboard item on TASK-032. Launch-config experiments were reverted.

# 2026-08-03 — TASK-032 fourth landing: scheduled observation-digest Automation (flight-gated)
- Governed Automation `platform.learning.observation-digest` landed: Learning Agent sole actor, Local Plane, one advisory `signal:write` step invoking new governed Skill `learning.observationDigest` (manifest in the catalog; Goal/Task pair ensured idempotently at wiring build). Seeded ONLY while the learning observation flight is on — with it off there is no Skill, no Automation row, nothing to trigger (tested). The Skill itself also fails closed if invoked with the flight off.
- Server schedule (`server.ts`): 15-minute unref'd interval plus one 30s post-boot run, reentrancy-guarded like the relationship-reconciliation timer, never on the public-cloud boundary. The scheduled RunCtx carries an explicit `system_generated`/verified_system taint label.
- Two genuine pipeline gates surfaced by TDD and fixed properly (not bypassed): (1) proposal rejected with `skill "learning.observationDigest" not in agent allow-list` → Skill added to the Learning Agent's static capability allow-list; (2) run halted at the `skill_execution` runtime taint sink with "unknown taint axis fails closed" because a bare RunCtx has no label → the schedule (and test) now label the run `system_generated`, mirroring `withHumanInputTaint` on the human-triggered path. Suggested-then-accepted survives the Automation path: the digest run proposes suggestions and mints zero preferences (asserted).
- `deterministicUuid` moved to `apps/api/src/deterministic-uuid.ts`; router re-exports unchanged, wiring imports it without a router↔wiring cycle.
- Verified: api 394 (393 pass + 1 pre-existing skip), server tests 28/28, core/dealpilot/web unchanged-green, web typecheck clean. Remaining on TASK-032: retrieval fusion (LA5) and promotion machinery; positive live UI proof stays the user-at-keyboard item.

# 2026-08-03 — TASK-032 fifth landing: accepted preferences reach chat agent output (LA1 exit on chat)
- Accepted learned preferences now project into the chat run-context memory slot (`assembleChatCompletion`): retrieved module-agnostically (`retrieveLearnedPreferences` moduleId now optional — light-egg, chat never learns which Modules exist), rendered ONLY as `preferencesToMemorySnippets` statements, per-preference taint labels joined into the model request. Flight-gated on `learningObservationEnabled` and Local Plane only — flight off means preferences are dormant data; the cloud consent boundary never sees them.
- New core classifier `isLearningObservationEntry` keeps raw learning machinery (signal/suggestion/preference JSON rows) out of the generic recent-memory slice — machinery JSON never reaches a prompt through any path.
- Closes the TASK-032 prototype-test clause "accepting mints one preference whose statement reaches `projectToSystemPrompt` output" on a real surface end to end (record → digest → accept → chat.turn.send → statement in the local model's system prompt, asserted).
- Verified: core 497/497 (new module-spanning retrieval + classifier test), api 396 (395 pass + 1 pre-existing skip) incl. 2 new e2e tests (flight ON: statement reaches the system prompt, machinery never does; flight OFF: pre-existing preference rows influence nothing). Remaining on TASK-032: retrieval fusion (LA5), promotion machinery, positive live UI proof (user at keyboard).

# 2026-08-04 — TASK-032 sixth landing: LA5 retrieval fusion v1 behind BRIDGE_RETRIEVAL_FUSION
- Three-lane retrieval fused into the chat memory slot: structured recency + vector similarity + graph traversal (Person name match → one-hop timeline, RLS/visibility enforced inside the graph store), combined with deterministic Reciprocal Rank Fusion in new core `learning/retrieval.ts`. Flight default OFF; off = byte-identical pre-fusion behavior (tested).
- Cross-plane leak is a fail-closed fusion invariant: a cloud requester never sees local or UNLABELED candidates (missing plane counts as local); proven at the core seam and end-to-end (cloud thread receives zero local memory with fusion on).
- Vector lane: `VectorIndex` port stores refs + vectors only — hydration goes back through authority-scoped `memoryStore.get`, the index is rebuildable (`clear` + one pass), and the graph/MemoryStore stays the single source of truth. `DrizzleVectorIndex` binds the port to the pre-existing 768-dim pgvector `embeddings` table (first reader/writer that table has ever had). v1 embeds with a deterministic hashing lexical embedder under its own embeddingModel id — honest token-overlap, not semantic; a real model swaps in behind the same port and the eval gate will prove the lift.
- Eval harness landed with a PINNED baseline (recall@3 1.0, precision@3 0.6, MRR 0.8667 over the held-out fixture set) — the LA5 exit criterion "regressions block ship from here on" is now literally a failing test.
- Scheduled memory-embedding indexer (15-min + 30s boot, reentrancy-guarded, unref'd, never public-cloud) runs as a plain maintenance loop like relationship reconciliation — derived-index upkeep, not an agent action, hence no governed Automation.
- Verified: core 504/504, db 218/218, api 400 (399 pass + 1 pre-existing skip; 4 new e2e), web 164/164 + typecheck. Follow-ups: semantic embedder behind the port, scheduled eval automation over real-usage pairs, team-scope retrieval evals.

# 2026-08-04 — TASK-033 first landing: Universal Commons capability archetypes (roadmap-v2 Phase 4) behind BRIDGE_COMMONS_ARCHETYPES
- Egg/Commons plan audit (roadmap-v2 vs implemented state): kernel, five agents, Commons module registry, blueprint compiler, onboarding, learning loop, and RAG v1 all real; the one wholly-unimplemented roadmap area was Phase 4 capability archetypes — the privacy-preserving network effect ("every new workspace starts smarter"). Implemented it end to end.
- CONTRIBUTE half: `generalizeLearnedPreferences` (core `learning/archetype.ts`) reduces accepted preferences to generalized fields only — no ids, no evidence refs, no user text, support counts banded — and screens every candidate through `findOrganizationDataPaths` AT THE SOURCE, so personal-shaped data never leaves the machine even toward a misconfigured registry. Publishing is `learning.archetypes.contribute` with an explicit per-archetype names[] (empty never means "all"), double-flight-gated (learning AND archetypes flights), with the Commons server's privacy gate as authoritative defense in depth.
- CONSUME half: `learning.archetypes.seed` fetches domain archetypes (trust-verified; unverified entries dropped) and `seedSuggestionsFromArchetypes` writes PROPOSED suggestions on the exact lineage a local digest would use — suggested-then-accepted, annoyance cap, rejection suppression, and local/seeded dedupe all inherited; accepting words the preference without a fabricated observation count. The existing Settings review card surfaces seeded suggestions with zero UI changes.
- Commons contract extended ADDITIVELY (ADR-163): optional `listArchetypes`/`publishArchetype` on `CommonsRegistry`; `/v1/archetypes` on services/commons with the same bearer auth, privacy gate, and Ed25519 signing as modules; deterministic pattern-derived names make same-pattern contributions from many workspaces dedupe idempotently (first writer wins, publish-race safe).
- Verified: core 509/509 (5 new archetype tests incl. privacy stripping and lineage sharing), commons service 25/25 (new HTTP roundtrip/dedupe/privacy/auth test), api 403 (402 pass + 1 pre-existing skip; 3 new e2e over real buildWiring with a registry double), web typecheck clean. Follow-ups on TASK-033: archetype review UI, support-band aggregation semantics, Bridge Cloud deployment of the archetype resource.

# 2026-08-04 — TASK-036: test execution model reworked (ADR-167)
- Researched what remains after the 2026-08-03 content cuts, then implemented rather than recommended. Three changes, each measured on this 8-core machine: (1) coverage split out of `test` into `test:coverage` — CI now runs `turbo run typecheck test:coverage build --force`, so the floors are bit-identical while local runs skip the ~2x instrumentation cost (`nodejs/node#55103`); `packages/core` 3.18s → 1.68s. (2) New `scripts/test-shards.mjs` runs N `--test-isolation=none` shards so a warm per-process cache is built per shard instead of per file — `packages/db` 137.5s/548s CPU → 74.8s/240s at 4 shards, 217/217 throughout, ~4x against the pre-audit ~300s. (3) Stryker configured for `packages/core`'s enforcement path as an out-of-band check, deliberately not in `test`.
- Rejected on evidence, not taste: `--test-isolation=none` for `apps/api` (hangs past 10 min in shared-process mode; stays per-file, and the shard runner documents "must pass under isolation=none" as its entry condition); Turborepo cross-worktree/remote caching (this session's own opening recommendation, withdrawn after `docs/BUGS.md` "turbo cache replays across worktrees" and `ci.yml`'s `--force` comment — the repo has untracked generated build inputs, so cache hits do not prove identical inputs, and speed is not worth a false green); ML predictive test selection (real at Meta/T-Bank scale, but needs a CI history to train — `turbo --affected` already covers a solo developer deterministically).
- Fixed the api socket-abort flake at the root rather than working around it, since raising parallelism is exactly what makes it fire: the fixed `setTimeout(100)` + boolean assertion in `jobpilot-culture-research.test.ts` was measuring scheduler latency, not the abort. Replaced with an awaited `close` event (10s deadline) at all three sites — the BUGS entry named two, the third had the same defect. Proven non-vacuous by sabotaging the compiled test (fails with the timeout message) and restoring it (passes). BUGS 2026-08-03 watch item → RESOLVED; `--test-concurrency=4` left in place.
- Verified: `packages/db` 217/217 under the shard runner; `apps/api` 393 pass + 1 pre-existing skip; full `turbo run test --force` green across all 48 tasks. Evidence and the rejected alternatives in `outputs/2026-08-04-test-speed-research.md`.
- Mutation testing paid for itself on its first run and produced a second, unplanned landing: mutating `sinkForRequest` (the ADR-161 taint sink map) left **31 of 61 mutants alive** — `req.action !== "read"` could be replaced with the constant `true` and all 485 core tests still passed, at 88% line coverage. The only rule with zero survivors was the `integration` rule, the one ADR-161 had given a dedicated regression pack. ADR-161's own consequences section had asked for "a table-driven conformance test over every `resourceType`"; that test now exists (`packages/core/test/taint-sink-map.test.ts`, 24 resource types x 6 actions, expectations restated independently of the implementation, `null` asserted explicitly), with `sinkForRequest` exported from `pipeline.ts` for it and `index.ts` left untouched. Same mutation range re-run: **49.18% → 100.00%, 0 survivors**; core 488/488 at 88.29%.

# 2026-08-04 — TASK-034: eval history persisted (ADR-168) + test output cut ~97%
- `DrizzleEvalStore` + migration `0036_task034_eval_persistence` (purely additive: three CREATE TABLEs, no ALTER) now back `eval_datasets`/`eval_runs`/`eval_comparisons`, bound in BOTH wiring modes — both already resolve a real Drizzle database, and keeping the in-memory fake in dev is what let the bug hide. This was filed as a persistence gap; it was a **silently disabled security control**: `capability.approve` only compares candidate vs baseline when it can load a run for both, so after any restart the `if` never entered and approve proceeded as "not applicable". The gate could not fail because it could not see. It can now reject — expect the first genuine `candidate does not beat baseline` to look like a new bug.
- Shape decisions against the obvious default, all in ADR-168: composite `(organization_id, id)` PKs (ids are caller-supplied slugs), the store bound to one Organization at construction rather than widening the port across core and every call site, and `started_at`/`finished_at` as `text` because the port types them as opaque ISO strings that must round-trip unchanged. Restart durability is pinned by a test that opens a genuinely FILE-backed PGlite twice — `memory://` would have proven nothing, since a memory-backed instance starts empty on every open.
- Token cost of a full verification run cut hard: `--test-reporter=dot` on the fast `test` scripts only (CI's `test:coverage` keeps full spec output). `packages/core` alone, measured directly: 48,868 bytes → 513. Verified against a deliberately failing test FIRST: the dot reporter still prints the full assertion, diff and stack, and still exits 1 — dots replace only the per-passing-test lines. Considered and REJECTED as poor value: compressing `docs/wiki/index.md` (already dense — it would trade real information for ~1k tokens) and `CLAUDE.md` (the instruction authority; compression risk is not worth ~1.8k).
- Two corrections to this session's own earlier claims, both caught by re-measuring rather than by a test: (1) the first coverage split covered only `packages/*` and `apps/*`, so seven `modules/*` and `tools/*` packages had NO `test:coverage` script — and since CI had just been switched to `turbo run test:coverage`, turbo would have listed them in the task graph and silently no-opped them. `@bridge/whatsapp`, `@bridge/jobpilot` and `@bridge/dealpilot` would have stopped being tested in CI with no red check anywhere. All workspace globs (`modules/*`, `tools/*`, `services/*`) are now covered and asserted. This also invalidates the earlier "identical 26-package task sets" check — turbo's dry run lists a task whether or not the script exists, so it could never have caught this; the real check is "every package with `test` also defines `test:coverage`". (2) The "whole-monorepo run: 1,449 bytes" figure was measured off a `tail -25`, not the full log — a truncation artifact, not a result. Per-package numbers above are direct measurements.
- Two real regressions the change surfaced, both fixed rather than muted: (1) `migration-metadata.test.ts` pinned the journal head at 0035 and asserted no `0036_*.sql` could be allocated — it caught the new migration exactly as designed; updated to head 0036 with the allocation guard moved to 0037, and the drizzle-kit no-op check confirms schema.ts and the committed migrations still agree. (2) the api socket-abort deadline was raised 10s → 30s: it is a LIVENESS assertion ("the socket is aborted at all"), never a latency one, and 10s proved too tight once db's 4 saturating shards run alongside api's 4-way concurrency on 8 cores. Confirmed load-only — the file passes 57/57 alone.
- Also installed and wired Codag (log compression for agent log reads) on the user's explicit approval; it registered its MCP server with Claude Code, Codex and Cursor and needs an app restart to load.

# 2026-08-04 — TASK-034: policy_params gets its reader (ADR-169)
- `DrizzlePolicyParamStore` binds the `PolicyParamStore` port to `policy_params` — a table that had sat in `schema.ts` since the VAR-1 batch with NO reader at all (the "roadmap batch without a consumer" pattern from the 2026-08-02 dead-code diagnosis). Bound in both wiring modes; `InMemoryPolicyParamStore` is no longer referenced by `wiring.ts`. No migration needed — the table already existed.
- This only mattered because of ADR-168. The port's single consumer is `router.ts`'s `resolveGates(await policyParams.get(organizationId))`, and that gate could not run at all while eval history died on restart. With both landed, the promotion gate now reads real eval history AND the Organization's real thresholds — the first configuration in which it can genuinely reject a candidate.
- Two deliberate strictnesses, both in ADR-169: an unrecognised `param_key` THROWS (a typo'd `aqv.gates.qualityMinn` that silently resolves to the default is a governance control that reports as applied and is not — the same failure class as the amnesiac eval store), and values are range-checked at the persistence boundary (a stored `qualityMin: -1` disables the gate while appearing configured). The range checks are intentionally stricter than the in-memory adapter's, because a row crosses a trust boundary while `setOverride` is a compile-checked call — the asymmetry is documented rather than hidden. Per-policy rows are excluded by an explicit `isNull` filter, not silently folded in, with a test pinning that limit.
- No write path was added, deliberately: nothing writes policy params today (the Variance Adjuster only PROPOSES), and shipping unused substrate is precisely the mistake this table already demonstrated. The write lands with the governed nudge that needs it.
- `wiring.test.ts`'s "the fixed lie" test now also pins `evalStore instanceof DrizzleEvalStore` and `policyParams instanceof DrizzlePolicyParamStore`, so a regression to either in-memory fake is a red test rather than a quiet re-opening of the gate.
- Verified: 72/72 turbo typecheck+test tasks green, db 232/232 sharded (8 new policy-param tests), api green including the new binding assertions.

# 2026-08-04 — TASK-034: boot-time env contract closed at its one real gap (ADR-170)
- The ask was "a boot-time assertion listing every required-in-production env var". `assertProductionEnv()` already existed and was substantial, so the work was auditing every `process.env` read across `apps/api`, `packages/*` and `modules/*` against `render.yaml` to find what it still MISSED — rather than bolting on a second guard beside a working one.
- One real gap, with a concrete failure mode: `buildPersistentPorts` registers `LlamaCppProvider` and `OllamaProvider` UNCONDITIONALLY and adds Anthropic/Groq only when their keys are set. Both unconditional providers are Local-Plane runtimes — the deployed image is plain `node:22-bookworm-slim` with no llama.cpp binary and no Ollama daemon, and `OllamaProvider` defaults to `http://localhost:11434`. A public-cloud container with neither key therefore booted successfully, passed `/health/ready`, and failed EVERY Agent Run at its first model call. `render.yaml` already declared both keys as `sync: false` secrets; nothing enforced the contract.
- Fix: in production **public-cloud mode only**, require `ANTHROPIC_API_KEY` or `GROQ_API_KEY` (either alone suffices — the requirement is one provider that can answer). Deliberately NOT asserted off the public cloud, because a self-hosted production host may run a real local Ollama and asserting there would refuse a valid deployment; the test pins that non-assertion too, so a future tidy-up that widens the check fails loudly.
- Checked and found correctly NOT required: `SUPABASE_JWT_SECRET` (optional by design — the already-asserted `SUPABASE_URL` selects remote-JWKS verification, so a verifier always exists in production and SEC-1 fail-closed holds), the `API_RATE_LIMIT_*` trio (real defaults, pinned in `render.yaml`), and the `GOOGLE_*`/`COMMONS_*` vars (absence disables an optional integration rather than breaking a required path).
...
# 2026-08-04 — Branch sweep: five stale branches landed, one rejected as superseded
- Merged into main after per-batch preflight: `claude/panda-avatar-oldplatform-48112d` (Zazoo character art, 0 conflicts), `copilot/create-requirements-doc`, `codex/zazoo-alt-home-creative-direction`, `copilot/add-md-instruction-files` (six `.github/instructions/*.instructions.md` files), and `claude/harness-architecture-research-a617d6` (Zazoo v1 status wiring + governed per-session companion memory).
- **REJECTED — `manishsbhoopalam8498-relationship-module-consolidation` (2026-07-17, 36 files, 27 conflicts).** Not merged, and the merge was aborted rather than resolved, on three pieces of evidence that it is already superseded: (1) main ALREADY contains the branch's own output doc `outputs/2026-07-16-task-008-relationship-module-consolidation.md`, so the work landed by another path; (2) TASK-008 (Relationship module consolidation) is `done` on main; (3) its migration journal head is idx 13 against main's 36, so merging would reconcile a 23-migration-old chain. It also modifies four files main deliberately DELETED in the TASK-031 bloat cleanup (`built-in-packages.ts`, `data/tools.ts`, `lib/moduleRoutes.ts`, `lib/pins.ts`), so merging would resurrect removed dead code to re-land finished work. Recommend deleting the branch rather than merging it.
- Merging six long-lived branches replayed ledger appends main had already published. Four byte-identical APPROVALS rows were dropped as duplicates; six ids were genuine COLLISIONS (different approvals/tasks sharing an id) and the ARRIVING side was renumbered per this repo's existing precedent: AP-090/091/092 (harness branch) → AP-099/100/101, TASK-024 (Copilot instructions) → TASK-039, TASK-029 (LA3 SearchProvider) → TASK-040, TASK-030 (Pending Tests) → TASK-041. An earlier dedupe pass had removed the copilot branch's two approvals as "duplicates" when they were in fact DIFFERENT approvals reusing AP-040/041 — caught by re-reading the rows, and restored as AP-097/098.
- Conflicts resolved by keeping real work from both sides rather than picking a winner: `ZazooLab.tsx` keeps the harness branch's companion-face preview AND the panda branch's inspect-mode hints; `.claude/launch.json` was rebuilt from both sides by config name (8 configs, no port clashes) after a first regex-join silently dropped `oldplatform-preview`. `avatar-overlay.test.mjs` stays DELETED: their branch modified it, but their version is still `readFileSync` + string matching, the cannot-fail category removed on 2026-08-03 with evidence.
...
- Commons archetype aggregation (supersedes ADR-163's first-writer-wins): a same-name re-publish is another organization corroborating the pattern — contributions += 1 (anonymous, inside the SIGNED content so tampering fails verification), supportBand = max band seen, tags union; each aggregation is a freshly signed superseding revision, and the publish race re-reads and aggregates instead of erroring. `learning.archetypes.seed` now ranks archetypes most-corroborated-first so the annoyance cap spends its budget on patterns many organizations converged on.
- ADR-172 records the batch (rationale, rejected alternatives, consequences). Verified: core 517/517, db 233/233, commons 25/25, models 48/48, api 408 (407 pass + 1 pre-existing skip; new promotions e2e incl. executor draft-refusal through the real executor), web 116/116 + typecheck. Open: draft activation flow (list/activate with its own governance), stale-embedding-space reclamation policy.

# 2026-08-04 — TASK-032 eighth landing: draft activation flow, usage-mined retrieval evals, embedding-space reclamation (ADR-173)
- Draft activation: `AutomationRegistry.listByStatus` added to both registries as the review surface while `load` stays active-only forever — one invariant ("load = startable"), no per-caller status discipline. `learning.promotions.drafts.{list,update,activate}`: update fills steps through the canonical `parseAutomationSteps` and refuses skills that don't resolve in the pipeline's own registry (exposed read-only as `Wiring.skillRegistry`); activate is the Human-explicit governance step with two static gates (non-empty steps, resolvable skills) and typed refusals — everything deeper (agent allow-list, taint, Goal/Task, approvals) still binds at run time through the unchanged pipeline. Full lifecycle proven e2e at API and Drizzle levels.
- Retrieval eval automation over real usage: cases are MINED from the organization's own prose Memories as self-retrieval pairs (honestly labeled `origin: "mined"`; machinery rows excluded; under 3 cases = skip, nothing persisted) and scored through the LIVE `fusedChatMemory` path with the active embedder. Runs persist in the platform eval store under `platform.learning.retrieval-fusion` with the embedding-space id as capability_version, so runs across embedder switches compare side by side. Scheduled 6-hourly + 2 minutes post-boot behind the retrieval-fusion flight, reentrancy-guarded like the other maintenance loops.
- Stale-space reclamation: `VectorIndex.listModels` (in-memory + Drizzle selectDistinct); the indexer pass clears every non-active `memory` embedding space AFTER the active space is backfilled — an embedder switch never has a queryless moment, and one deployment keeps exactly one active space per entity type (deliberate; recorded in ADR-173).
- Verified: core 517/517, db 234/234, commons 25/25, api 412 (411 pass + 1 pre-existing skip), web 116/116 + typecheck clean.

# 2026-08-04 — TASK-032 ninth landing: draft-editor UI, eval-dataset refresh policy, honest metric labeling (ADR-174)
- Eval-dataset refresh policy: runs score STORED immutable dataset versions (`retrieval-usage:<org>:vN`) for run-over-run comparability; dead cases (source Memory deleted) are pruned before scoring — corpus drift is not a pipeline regression; a new version is minted only on none/too-few-live/Jaccard<0.5 drift, and old versions stay so historical runs keep pointing at exactly what they scored. Time-based TTLs rejected (they reset baselines while nothing changed).
- Honest metric labeling moved INTO the API contract: `learning.retrieval.evals` responses carry `metric: "self_retrieval"` and the verbatim metricNote ("Self-retrieval consistency … Not human-judged relevance"), so a mislabeled dashboard is a contract violation rather than a copy-review hope. `learning.retrieval.status` always answers; the evals procedure fails closed while the fusion flight is off.
...
# 2026-08-08 — Task Manager TM3/TM4 slice 4: approval materializes, and the model call is governed (ADR-199, AP-119)

- User directive: "Close the next open items" — the two ADR-198 named first.
- **The bigger one: approval materialized nothing.** A reviewer could approve a decomposition and no child Tasks appeared; approving a scan finding created no candidate Task. Draft-then-approve had a draft and an approve and no third step, which made the loop decorative.
- Closed with a new pure `packages/core/src/task-materialize.ts` — `applyApprovedPlanningProposal` takes the current queue plus an approved payload and returns the queue that should replace it. **Both** `decideProposal` implementations call that one function, the same shape `applyApprovedTaskProjectionReconciliation` already uses, so two durability backends cannot materialize an approved plan differently.
- **No migration was needed**: `TaskProposalKind` already carried `candidate` and the `kind` column is `text`. Only `stageProposal`'s narrowed union needed widening — a good reminder to read the existing type before inventing a new name for the same idea.
- The procedures now compute the proposal id BEFORE the Automation run and pass it as the pipeline `proposalId`, so the ledger proposal and the Task-Manager `candidate` row share one id and a single human decision resolves both. That is the exact pairing `projection_reconcile` and `archive_sweep` already use; getting it wrong the first time (staging with a fresh id after the run) would have left `pipeline.decide` unable to find anything.
- **The two rules the materializer is built around.** Generated Tasks land as `status: "candidate"` — approving a decomposition means "these are worth having in the tree", not "start them", and committing is a separate later act. And dot-paths are RECOMPUTED against the current queue, never taken from the payload: the path a Skill drafted was correct at draft time, by approval another Task may hold it, and the path is what the queue's identity and ordering are built on.
- **Other limits, chosen rather than inherited**: an approved pre-mortem writes nothing, and that is right — its product is what the reader now knows, and turning mitigations into Tasks would manufacture work nobody chose. Approved outcomes never arrive as the north star. Each scan candidate lands under the Task its own finding named, not under the proposal's subject, and a finding whose source row has since been deleted is dropped rather than re-homed, because that row was its whole justification. Generated Tasks are owned by the APPROVER, never the drafting Agent. Drafting caps are re-applied at materialize, because the payload is editable before approval and an edited payload is a human's text, not validated Skill output. An empty scan raises no proposal at all — staging one would put "approve this nothing" in the review inbox.
- **The second residual: the model call was ungoverned.** The planning Skills called the resolved provider directly and returned a `modelReceipt` for nobody to record, because the skill-registry closure has no request context and `createGovernedModelProvider` needs one. `RunCtx` gains an optional `modelProvider` — the model this run is authorized to use, already wrapped in whatever governance the caller owes it. `runPlanningPlaybook` resolves a local provider, wraps it, and passes it through; the wiring closure prefers it and falls back to its own local resolution for registry-level calls. The procedure returns `modelReceiptLedgerId`.
- **Plane deliberately unchanged.** Now that the call is audited it would be easy to let the binding reach the Cloud Plane. It does not: governance is not consent, the manifests say `plane: "local"`, and the user's private queue stays local.
- Verified: core 583/583, with the candidate-status, north-star and finding-provenance invariants mutation-checked one at a time. api 450 pass / 1 skipped / 0 fail, including a full end-to-end test — a local model double drafts two children, the governed Run halts for review, approval creates them as candidates under the right parent with the right exit tests, and a veto writes nothing. Seen RED twice: once with the materialize branch disabled (2 failures), once with the governed provider dropped (the receipt assertion). db 234/234. turbo typecheck 44/44.
- `runPlanningPlaybook`/`runOpportunityScan` now require `expiresAt`, matching the other two Task Manager Automation procedures — proposals expire.
- **Recorded as NOT delivered**: `decideProposal` still refuses `edit` for every kind but `projection_reconcile`, so a planning draft can be approved or vetoed but not edited in place. The materializer already re-validates the payload as untrusted human text, so the gap is API surface rather than safety. Also still open: `standup-brief` needs a Chief of Staff runtime Agent; nine declared Automations remain unbound; dependency-aware sequencing still needs the missing schema Relations; TM5/TM6 untouched.

# 2026-08-08 — Task Manager TM3/TM4 slice 3: the catalog stops echoing, and the Automations get built (ADR-198, AP-118)

- User directive: "implent automation and skills".
- **Where slice 2 left it**: eight of the eighteen `task-manager.*` Skills still fell through to `return { proposedOutput: inputs }`, and nothing in the product had ever INVOKED the planning ones — they were reachable through the registry and on no Automation or tRPC path.
- **Four of those eight had no logic anywhere**, and are now real in a new pure `packages/core/src/task-execution.ts`: `verifyTaskEvidence`, `synthesizeProgress`, `scaffoldHabits`, `scanForOpportunities`. Deterministic, because whether a Task carries evidence, what changed in the last week, whether a goal has a cadence, and whether a parent's children are all done are facts about rows — a model there would narrate data the reviewer can already read, and would go silent with no provider configured.
- **Five more had complete, tested implementations sitting in `@bridge/core` that had simply never been bound to a Skill id** — `applyTaskRestructure`, `routeTaskByRequiredSkill`, `classifyTaskChangeBand`+`calibratedTaskChangeDecision`, `emitTasksMarkdown`/`detectTaskProjectionDrift`, `planCompletedBaySweep`. That was the cheapest half of the gap and the easiest to have missed.
- `exit-test-authoring` joins the model-backed Playbook Skills. It earns **no sixth Playbook**: the plan caps the library at 5 in v1 with additions gated on observed demand, and `measurable-review` already asks "restate this so a stranger could tell whether it happened" and "what evidence would you show" — which is the job. It and `evidence-verification` live in different modules that do not import each other, honoring the plan's rule that authoring and checking never share one prompt context.
- **The echo fallback is now a THROW.** A registered `task-manager.*` id with no dispatcher fails loudly. That fallback is the exact mechanism that let TM3 ship declared-but-not-built — from outside, a registered manifest with an echoing `run()` is indistinguishable from a working Skill.
- **Two Automations, built rather than declared**: `taskManager.runPlanningPlaybook` (new `planning-playbook`) and `taskManager.runOpportunityScan` (`proactive-scan-cadence`, in the manifest since TM0 with no runtime id and no procedure). Both run through `automationExecutor` as attributable Internal Strategist Runs whose proposals halt at `pending_review`. The planning one is human-triggered on purpose: the other Task Manager Automations fire on a cadence or a Task Event, but these Skills answer a question somebody asked, so the trigger is a person and the Automation exists to give that invocation an attributable Run.
- **Where the Skills deliberately stop**: `verifyTaskEvidence` proposes a verification with **no `verifiedBy`** — `TaskVerification` requires one and the only honest value is whoever accepts the proposal; filling it would put an Agent's name on a verification record, exactly the self-approval the agent floor exists to prevent. It also does not claim the evidence SATISFIES the exit test, only that a Human now has both halves. `scaffoldHabits` refuses a non-goal Task with a stated reason rather than inventing a recurring review. `scanForOpportunities` says in `scope` that it read the queue: the plan describes cross-Module Signals and evidence, those live in stores a pure Skill does not reach, and claiming findings came from them would be fabrication.
- **A compromise, recorded as one**: `completed-bay-sweep` and `ledger-projection` accept either an authorized queue or the pre-computed plan their existing certified procedures already pass. Forcing one shape would have broken `runCompletedBaySweep`/`runLedgerDriftDetector`; leaving them echoing was the thing being removed. The caller-supplied branch re-derives the projection content hash and rejects a mismatch, and rejects a sweep plan missing a record version for any Task it would archive — so it validates rather than echoes, and `source` states which shape answered.
- **`standup-brief` deliberately got NO runtime binding.** It is Chief of Staff-owned and CoS has no governed runtime Agent identity (`resolveModuleAgentRuntimeId` resolves only `internal-strategist` and `governance-agent`). Binding it to Internal Strategist would make it run at the cost of ADR-107's deliberate routing/planning ownership split. Its Skill is built and callable regardless.
- **Agent authority**: Internal Strategist gains exactly six allow-listed Skills — the five a Playbook run invokes plus the scan. It owns more `task-manager.*` Skills than that; granting ones no procedure calls would widen authority with nothing reachable behind it. **Capability scope is unchanged** (`record:read`/`record:write` already covered them). The list moved to one shared `INTERNAL_STRATEGIST_ALLOWED_SKILLS` in `@bridge/db`, imported by the in-memory seed rather than restated — a drift between the two backends would let a Skill run in one mode and be denied in the other, which only ever surfaces in production.
- Verified: core 574/574, with the two load-bearing new invariants mutation-checked one at a time (naming a verifier in the proposed verification, and scaffolding habits for a non-goal each made their test fail). api 447 pass / 1 skipped / 0 fail — four new Automation tests plus a catalog-wide test asserting NO registered `task-manager.*` Skill echoes, seen RED against a restored echo fallback. db 234/234. turbo typecheck 44/44. Two pinned manifest-version assertions updated deliberately (`1.1.0`→`1.2.0`), not loosened.
- **One pre-existing failure, checked rather than assumed**: `@bridge/web`'s "the in-app browser companion uses the shipped Zazoo renderer" fails in a clean `git worktree` at the same base commit, so it is not from this change set. Nothing here touches `apps/web` beyond the regenerated pending-work projection.
- **Recorded as NOT delivered**: approving a planning proposal materializes nothing — a decomposition does not create the child Tasks and a scan finding does not create the candidate Task, both stop at the Review inbox; the planning Skills still do not route through `createGovernedModelProvider` (now that an Automation invokes them, this is the natural next fix); nine further declared Automations have no runtime binding; dependency-aware sequencing still needs the missing schema Relations; TM5/TM6 untouched.

# 2026-08-08 — Task Manager TM3 slice 2: the four model-backed planning Skills, and Playbooks with something in them (ADR-197, AP-117)

- User directive: "finish the model-backed planning Skills task" — the residual slice 1 named as open.
- **The two open items were one gap.** The plan calls `goal-outcome-framing`, `candidate-task-generation`, `premortem-scenario` and `task-decomposition` "methodology-parameterized", and `TASK_MANAGER_PLAYBOOKS` was five ids with no content. So there was nothing to parameterize the Skills *with*, and nothing consuming the Playbooks. Each half explained the other's absence.
- **Closed together**: new `packages/core/src/task-playbooks.ts` carries `TASK_PLAYBOOKS` — the same five ids, now with a methodology, an intent, the questions the technique asks, Bridge-authored system-prompt guidance, and the Skills each supports — plus the four Skills themselves (`frameGoalOutcomes`, `generateCandidateTasks`, `runPremortem`, `decomposeTask`). `TASK_MANAGER_PLAYBOOKS` is now DERIVED from that content rather than restated, so the roster and the methodologies behind it cannot drift. `wiring.ts`'s `runTaskAuthoringSkill` makes the four execute through the governed registry instead of echoing, declared `executionClass: "authority_bearing"` because a Skill that calls a model is not pure data and must reach the pipeline's `skill_execution` taint sink.
- **Why these four take a model when slice 1's three must not.** Slice 1's Skills run on every Task create, decide where work lands, and must show the reviewer the exact terms behind a finding — determinism was the requirement. These four are invoked deliberately, produce prose a human reads and edits, and there is no deterministic function that writes a useful pre-mortem. The split is along "does this decide placement": the model never touches the path that decides where a Task sits.
- **The load-bearing decision is what happens with NO model.** They return the Playbook's own questions with `source: "playbook_scaffold"`, every item list empty, and a `note` stating why. Not fabricated content — and equally, not a bare empty result, because `failureModes: []` on its own is indistinguishable from "the methodology considered this and found nothing". The questions are real content the technique supplies; the answers are the model's job and their absence is stated. The same degrade catches a model returning prose, malformed JSON, or entries that fail validation — the shape `classifyIntent` already uses when it degrades to clarify rather than inventing a route.
- **Local Plane only.** The four manifests already declared `plane: "local"`, so the binding is `planeDefault: "local"` — which the model router FAILS rather than falling through to a cloud provider. A planning Skill can never quietly ship the user's private queue to a cloud model to write its plan, and that failure is exactly what produces the scaffold. Resolved per call, never snapshotted at boot: on desktop the managed local model only becomes healthy ~30s after wiring runs, so a boot snapshot would leave these permanently scaffolded on the machines that actually have a local model.
- **Structure the model is not allowed to author**: child dot-paths are computed from the parent path and the highest existing DIRECT sibling (an invented "2.3.5" could collide with a live Task or silently reorder the queue — the path is what `compareTaskPaths` sorts on and what restructuring recomputes atomically); generated options are pinned to `status: "candidate"` whatever the model claims, so it cannot promote its own suggestion into live work; outcomes carry no `id`; at most one north star survives; a decomposition with no `parentPath` throws rather than guessing a placement; entries missing a required field are dropped, and if none survive the result degrades to the scaffold rather than shipping a half-invented plan.
- IP care: the techniques are public-domain planning concepts, named as techniques and written from scratch. The two commonly associated with trademarked book brands are deliberately not named after them and none of their text is reproduced (plan §7).
- Verified: core 563/563. The four load-bearing new assertions were mutation-checked one at a time — making the scaffold guess `isGoalProposed`, trusting the model's north-star claim, letting the model choose `status`, and letting a model-supplied path through each made its test fail. api 442 pass / 1 skipped / 0 fail, with the four new registry tests seen RED against the restored echo fallback. turbo typecheck 44/44; `check:agent-context` clean; `check:vocabulary` clean for every file in this change set (one string of mine did trip it and was rewritten) — the remaining failures are the pre-existing `modules/whatsapp` + `modules/manifests` "Tool" terminology already tracked under TASK-036/AP-096 and confirmed on `origin/main`.
- **Recorded as NOT delivered rather than quietly skipped**: nothing invokes these four yet — they are reachable through the governed registry but sit on no Automation or tRPC path. They return a `modelReceipt` for a caller to ledger but do not route through `createGovernedModelProvider`, which needs an `ApiContext` the skill registry does not have; that is acceptable only because the binding cannot egress, and wiring it belongs with the Automation that invokes them. Eight `task-manager.*` Skills still echo. Dependency-aware sequencing still needs the `depends_on`/`blocked_by` Relations that do not exist in the schema. TM5 Second Brain/date wiring and TM6 Commons packaging are untouched.

# 2026-08-08 — Task Manager gap audit + TM3 slice 1: the planning Skills get real logic (ADR-196, AP-116)

- **The gap, found by auditing the shipped Module against its own plan**: TM0/TM1/TM2 and half of TM4 were built and externally certified, but TM3 ("planning intelligence — Playbooks propose, humans decide") was declared and not built. All 18 `task-manager.*` Skill ids were registered as `SkillManifest`s with correct owners/permissions/plane/risk — and `wiring.ts` gave every one of them a `run()` that fell through to `return { proposedOutput: inputs, diff: { to: inputs } }`. An echo. Only `task-manager.create-task` had real behaviour. From the outside, a registered manifest with an echoing `run()` is indistinguishable from a working Skill, which is why this survived shipping.
- Second half of the same gap: `draftTaskCreate` DID raise an `impact_fit` proposal on a populated queue (so "nothing settles silently" held), but its entire analytical payload was `requiresResequenceReview: true` — a flag asking a reviewer to review something the system had not worked out.
- **Closed for three Skills**: new pure `packages/core/src/task-planning.ts` — `findDuplicateTasks` (reconciliation at intake, matching on the plan's own keys: title + outcomes + exit test), `proposeQueueSequence` (in-progress head, then priority, then canonical path; blocked work sinks below ready work), `analyzeTaskImpactFit` (the composite). The `impact_fit` payload now carries duplicates, verdict, placement-with-reasoning, a real proposed order and queue findings; pre-existing keys are preserved (`proposedStatus` is read by the approval path) and the flag became honest — `resequence.changed`, not a constant `true`.
- **Deterministic on purpose, no model call** — same rule that made `suggestTaskParent` deterministic (ADR-183) and kept promotion out of an Agent's hands (ADR-177): it runs on EVERY Task create, the reviewer must see the exact terms that produced a finding, and with no local model configured it must degrade to "nothing found" rather than to a confident guess. The duplicate bar is set HIGHER than parent suggestion's (0.6 score / 3 shared terms vs 0.34 / 2) because a false duplicate argues against work the user just decided to do — a false positive costs more than a miss.
- **Skills stay pure**: `runTaskPlanningSkill` takes the queue as an authorized *input* rather than reaching into `TaskManagerStore` (the registry is built in `buildWiring` before either port set exists, and keeping the Skill in its `pure_data` class leaves authority scoping in the caller where the pipeline can see it). A planning Skill invoked with no `queue` **throws** — a fabricated "no duplicates found" is indistinguishable from a real clean answer, which is the exact failure class this slice exists to remove.
- Verified: core `task-planning.test.ts` 5/5, seen RED first — the `draftTaskCreate` assertion failed on "duplicates must reach the proposal" against the old payload before the wiring landed. api `task-planning-skills.test.ts` 2/2 (Skills execute real analysis through the real registry; missing-queue refusal asserted). Full turbo typecheck+test:coverage+build 70/72 — the two non-green were `@bridge/db` failing with `spawnSync ... ETIMEDOUT` under load; re-run in isolation it passes in 637ms, and nothing in this change touches schema or migrations.
- **Recorded as NOT delivered rather than quietly skipped**: dependency-aware sequencing (the plan specifies `depends_on`/`blocked_by` Relations; no such edge exists in the schema, so the `blocked` status is honored and the missing edges are named as the next slice); the four model-backed TM3 Skills; `TASK_MANAGER_PLAYBOOKS`, still an id/version/owner list with no content and no consumer; `proactive-opportunity-scan` with no Automation behind it; TM5 Second Brain/date wiring; TM6 Commons packaging.

## 2026-08-08 — Task Manager TM3/TM4 slice 5: a reviewer may correct a planning draft (ADR-200, AP-120, TASK-021)

- The residual ADR-199 named first. `decideProposal` refused `edit` for every kind but `projection_reconcile`, so one bad child title in an eight-child decomposition cost the whole draft — the only choices were "build all of it" and "build none of it".
- **The edit is an allow-list MERGE, and that is the whole design.** New pure `mergeEditedPlanningPayload` takes only the ONE content key the STAGED kind materializes from (`children`/`candidates`/`opportunities`/`outcomes`). `kind`, the run id, the model receipt and the Playbook's questions carry over verbatim. `kind` is absent from the wire schema entirely because it selects which branch of the materializer runs — an edit able to carry a new one could turn a reviewed pre-mortem into an unreviewed decomposition.
- **Three layers of validation, because a human's edited text is exactly as untrusted as a model's output**: `.strict()` `EDITED_PLANNING_ITEM` at the API boundary (an edit cannot introduce a key at all), the merge against the staged kind, then `applyApprovedPlanningProposal` per entry on the way to the queue.
- **Two refusals, both deliberate.** An edit that materializes nothing is rejected, not accepted: an approval that writes nothing is a veto wearing an approval's clothes — the ledger would record consent to a plan and the queue would show no plan, so the reviewer is made to say which they mean. A pre-mortem refuses an edit outright, because approving one writes nothing by design and accepting an edit would imply otherwise.
- **Provenance survives the edit**: the Agent's original draft stays on the proposal row under `agentDraft`. The pipeline ledger holds the pre-edit proposal too, but the queue-side row is where a reviewer actually looks.
- Two smaller gaps found behind it and closed in the same slice: nothing could READ a Task-side proposal over tRPC (an edit surface with no read surface is unusable), and a planning Run's recorded output ended at "a draft was produced" rather than at what the Human decided — both now match what `archive_sweep` already did.
- `archive_sweep` and restructure operations stay approve-or-veto **on purpose**: they are computed plans over specific rows and versions, checked for staleness at decision time, so an edited one is a different unchecked plan rather than a corrected draft. Editability follows from "a proposal about content" versus "a proposal about state".
- Verified: core 588/588 with all four new invariants mutation-checked one at a time (dropping the empty-edit guard, the pre-mortem guard, the `agentDraft` provenance, and letting the edit supply the kind each failed exactly one test, then restored green); api 453 total / 452 pass / 1 skipped / 0 fail with the two new API tests seen RED twice — restoring the ADR-199 refusal fails 2, ignoring the edited payload fails 1; db 234/234; turbo typecheck clean.
- **Two gate failures reproduced at the unmodified base commit `dc46636` and therefore not from this change set**: `check:vocabulary` (WhatsApp Module + `modules/manifests` "Tool" terminology, already tracked under TASK-036/AP-096) and one `@bridge/web` test ("the in-app browser companion uses the shipped Zazoo renderer").
- **Recorded as NOT delivered**: no UI reaches the edit — `TaskManagerPage` renders approve/veto for an `impact_fit` proposal only, so this decision is API-reachable and not yet human-reachable. A planning-proposal review surface belongs with the TM5 slice.

## 2026-08-08 — Task Manager TM3/TM4 slice 6: Chief of Staff gets a runtime Agent (ADR-201, AP-121, TASK-021)

- The blocker ADR-198 recorded and refused to work around. Four Automations and three Skills are declared under Chief of Staff by ADR-107's deliberate design; **none of them could run**, because `resolveModuleAgentRuntimeId` resolved only Internal Strategist and Governance and a TASK-007-era comment said CoS deliberately has no physical identity.
- **An owner that cannot act owns nothing that can run.** An Automation starts an Agent Run and only an attributable Agent may invoke a Skill, so the ADR-107 ownership split was decorative. The glossary line that comment cited ("its routing role is a product composition, not an architectural requirement") is about how chat routing is *composed*, and the same entry's first clause already calls Chief of Staff a "default coordinating Agent". The comment is amended rather than deleted, so both readings stay legible.
- **Narrow on purpose**: `signal:write`/`record:read`/`record:write`, deliberately no `record:archive` — Chief of Staff coordinates and reports, Governance archives. Exactly one allow-listed Skill (`progress-synthesis`); `agent-task-routing` and `habit-scaffolding` are CoS-owned in the manifest and deliberately not granted, because authority with no reachable behaviour behind it is authority for nothing.
- **The two briefs REPORT and stop**: no Task-Manager proposal, step action `read`, no proposed status. Making a Human approve being told about their own Tasks would be governance theatre, and what a rotting Task needs — finish it, re-scope it, park it, drop it — is a judgement, not a default.
- **One Skill behind both, and one definition of "stale".** `progress-synthesis` already answers "what moved and what did not"; its `stalled` list IS the staleness question over a longer window, so a nineteenth Skill would have been a roster entry, not a capability. `evaluateTaskGuards` gained the `stale_task` finding it never had, and both now share `isStalenessEligible` — narrower than `taskIsOpen` because a `candidate` is an option nobody committed to and a `parked` Task is a decision to not do it now; `blocked` IS included, since blocked work going quiet is the thing that rots. Staleness is **skipped rather than guessed** with no clock: a pure function must not report findings nobody can reproduce.
- **Two mutation attempts survived first, and each exposed a real defect** — worth recording because the check earned its keep twice. The staleness guard *inlined* the status check instead of calling the shared helper (duplication that would have drifted); and an API-level candidate-exclusion assertion was vacuous, because every row created through the API is new, so that assertion moved to core where `updatedAt` can be set.
- Verified: core 592/592, api 455 total / 454 pass / 1 skipped / 0 fail, db 234/234, turbo typecheck+build 48/48. Manifest `1.2.0`→`1.3.0`; two pinned version assertions updated deliberately; the 1.2.0 comment corrected, since it wrongly claimed `standup-brief` gained a runtime id under ADR-198.

## 2026-08-08 — Task Manager TM3/TM4 slice 7: the guard and the gate become Skills (ADR-202, AP-122, TASK-021)

- Four Governance Automations declared since TM0 had nothing to run. The computations all four need — `evaluateTaskGuards`, `classifyTaskChangeBand`, `calibratedTaskChangeDecision` — have existed as core code the whole time with **no Skill id**, and an Automation step needs one. `evaluateTaskGuards` was reachable only as read-only data hanging off the `projection` query.
- **A gate that trusts the caller's account of its own track record is not a gate.** `taskManager.approvalBand`, the query that predates this, takes `approvals`/`vetoes` as *client inputs* — anything calling it could hand itself a calibrated verdict. `runChangeGate` counts real approve/veto decisions from the ledger, matching on `skill === "task-manager.change-gate"` (the capability-attribution key) and the change kind, bounded to 200 rows. Only the gate's own prior decisions count: a decision on some other proposal says nothing about whether this Human's reschedules have been sound.
- **Two Skills, not one.** The state of the queue and whether one specific change may proceed are different questions with different inputs; a single Skill with a mode switch would have made its manifest permissions describe two things at once. And each Automation reports **only its own finding kind** — one evaluator with several callers is not the same as one Automation dumping everything under whichever name you invoked it by.
- **Least privilege made a real mismatch visible.** Both new Skills only read, so both declare `record:read` alone. The first run REJECTED them: Governance has no `record:write`, and the generic Task Manager manifest shape was handing every Skill both. That rejection was the boundary working — the fix was narrowing the Skills to what they use, not widening the Agent.
- **A finding worth recording**: `unverified-done-challenger` has no reachable path through `taskManager.transition` — the store already refuses `done` without verification evidence. The guard covers rows that reach `done` another way, chiefly an approved `tasks.md` projection reconcile, which is exactly why it exists. Its API test asserts an honest empty result rather than manufacturing an impossible row.
- Verified: api 457 total / 456 pass / 1 skipped / 0 fail, both new tests mutation-checked (unfiltering the guard findings, and swapping the ledger history for caller-supplied numbers, each failed exactly one test); db 234/234; turbo typecheck+build 48/48. Manifest `1.3.0`→`1.4.0`, two pinned assertions updated deliberately.
- **Not covered by a test, recorded rather than implied**: the router's one-line `ctx.identity.type`→`actorType` mapping is not separately exercised (every test caller is a user), though the rule it feeds — an Agent-proposed change always needs a Human — is asserted directly in core; the positive calibration direction (three vetted approvals ⇒ `auto_apply`) is likewise core-only, not end-to-end through the gate.

## 2026-08-08 — Task Manager TM3/TM4 slice 8: Internal Strategist's Automations get bound (ADR-203, AP-123, TASK-021)

- Three of the four were the subtle case. `task-created-impact-analysis`, `task-tree-restructure-proposal` and `target-change-reopen-prompt` ALREADY raised a governed proposal that halted for Human review — nothing settled silently, and the governance story was true. **What was not true was the attribution.** Each proposal's actor was `ctx.identity`, the Human who happened to trigger it, and its skill was `KERNEL_PASSTHROUGH_SKILL`.
- The wiki's own load-bearing rule reads "every Task/Goal create triggers Internal Strategist impact-fit-analysis". The analysis was real — `analyzeTaskImpactFit` computed it and it rode the payload — but there was no Agent Run, no Skill invocation, and the Agent named in the rule was a string label on a row.
- **This is more governance, not a convenience.** An Agent actor is subject to the Skill allow-list and capability scope a Human actor bypasses, so routing these through the executor means the analysis now has to pass the agent floor to happen at all. That is why the three Skill grants had to be written down, and why the pinned `allowedSkills` deepEqual was updated deliberately rather than loosened.
- **A moved target is an impact-fit question.** `target-change-reopen-prompt` needed no Skill of its own: "should this Task reopen because what we want changed" is exactly "does this Task still fit". Running `impact-fit-analysis` makes the Skill's output the reasoning attached to the reopen prompt instead of a prompt with nothing behind it; the store still computes the reopen proposal and the Run's proposal shares its id.
- **`goal-review-cadence` stays Internal Strategist's** even though `queue-guard` is Governance-owned by default. Whether a goal is due for review is a planning question, not a control question — ADR-107's split applied to a guard rather than to routing — and one Skill resolving for two eligible Agents is the shape ADR-104 was built for. The alternative was a second evaluator computing the same finding.
- Verified: api 460 total / 459 pass / 1 skipped / 0 fail with three new tests; both attribution claims mutation-checked — reverting the impact analysis to a kernel-passthrough proposal failed 4 tests, running `goal-review-cadence` as Governance failed 1. core 592/592; db 234/234; turbo typecheck+build 48/48. Manifest `1.4.0`→`1.5.0`; two pinned assertions updated deliberately.

## 2026-08-08 — Task Manager TM3/TM4 slice 9: dependency Relations (ADR-204, AP-124, TASK-021, migration 0039)

- The residual every slice since ADR-196 recorded in the same words. `proposeQueueSequence` honoured the `blocked` STATUS and said so in its own doc comment — a status records that someone *believed* a Task was blocked, not by what, so nothing could ever tell them it had stopped being true, and `dependency-unblock-notifier` had no edges whose clearing it could notice.
- **ONE edge kind, not two.** `blocked_by` is not a second relation: it is the same edge read from the other end. Storing both directions would let them disagree, and a queue whose two halves disagree about what blocks what is worse than one that models no dependencies at all.
- **A cycle is refused at write time, because it is unsatisfiable rather than unwise.** Every Task in a cycle waits forever, and no amount of finishing work clears it. The API answers 409, not 500 — the request was well-formed and the answer is no. The one-hop case is a CHECK constraint; the Drizzle store takes `FOR UPDATE` on the edge set before validating, because two concurrent edges could each look acyclic alone and close a loop together.
- **What stops blocking**: `done`, `abandoned`, or gone. `abandoned` counts as satisfied deliberately — a blocker nobody is going to do can no longer block, and leaving the dependent Task waiting on it forever is exactly the rot this edge exists to expose. A deleted blocker likewise; the composite FKs cascade for the same reason.
- **"Unblocked" means the wait ended, not that there never was one.** A Task with no dependencies is not newly startable, and reporting it would bury the handful of rows that actually changed. Each finding names what cleared it, so the notice is checkable.
- **The notifier notifies.** A blocker landing does not make the dependent Task started; flipping its status would decide for the Human that the work is now theirs to pick up.
- **Opt-in and mandatory, deliberately different.** `proposeQueueSequence` takes the blocked-id set optionally — a caller with no dependency store to read gets the status-only ordering rather than a silently wrong claim of awareness. `dependency-analysis` THROWS without an edge list, because a missing input and a genuinely clean graph must not produce the same answer. An Organization with no edges is a common legitimate state; a dependency question asked with no edges supplied is always a mistake.
- Migration 0039 is purely additive with no backfill and no change to `tasks`, so `DROP TABLE "task_dependencies"` reverses it exactly. Composite FKs on `(organization_id, id)` make a cross-tenant edge inexpressible; RLS uses the standard tenant predicate.
- Verified: core 597/597 with all four invariants mutation-checked one at a time; api 462 total / 461 pass / 1 skipped / 0 fail with two new end-to-end tests; db 234/234 including the drizzle-kit no-op metadata check; turbo typecheck+build 48/48. Three pinned assertions updated deliberately — migration head 0038→0039, manifest `1.5.0`→`1.6.0`, and the catalog-size floor 18→21.

## 2026-08-08 — Task Manager TM5 slice 10: the queue reaches Second Brain (ADR-205, AP-125, TASK-021)

- Three things were wrong, and only the first was the one anyone had written down.
- **Second Brain had no Tasks.** ADR-110 settled that Second Brain IS Graph view at `scope: full`, and `graph.full` composes Records, People, Communities, Events, Module nodes and Agent nodes. Tasks live in their own table rather than in the graph store's Records, so the execution queue — the one Database the user works out of daily — was the one thing the full-scope graph could not show. A "full" graph that silently omits the queue is not full.
- **Graph view was structurally impossible on the Task Page, not merely unbuilt.** `computeEligibleKinds` requires a relation column that is not the parent, and the Task Database had only `parentTaskId`. Until ADR-204 created `depends_on` there was no second relation to be eligible on — which is why this had to follow the dependency slice rather than precede it. Calendar was already eligible (`scheduledFor` is a `date` column) and Tree already was too; Graph is the one that needed the edge.
- **Nine Automation manifest entries were lying.** Each entry's `automationId` came from a four-branch ternary listing the Automations bound as of ADR-198. Every binding added since — nine across ADR-201 through ADR-204 — shipped a manifest entry claiming no runtime Automation stood behind it, which is exactly the "declared, not built" signal this whole workstream exists to make trustworthy. It now derives from `resolveModuleAutomationRuntimeId`, so a new binding cannot land without the manifest following.
- **Both edge kinds, because they are different questions.** The tree edge says where work SITS; the dependency edge says what it WAITS ON. Collapsing them would leave Second Brain able to show that two Tasks are related but not why.
- **The queue honours the same node cap as everything else and drops CLOSED work first.** A queue reliably outgrows every other Database; letting it ignore the cap would make a large queue crowd out everything Second Brain exists to relate it to, and an archived Task is history rather than context. `hasMore` accounts for what was left out, so the surface never claims completeness it does not have.
- **`dependsOn` is read-only in the grid.** An edge can close a cycle, and a cycle is refused server-side with a reason (ADR-204); an editable cell would put that refusal behind a control that silently reverts.
- Verified: api 463 total / 462 pass / 1 skipped / 0 fail with a new end-to-end graph test, mutation-checked twice — omitting Tasks from the graph and collapsing the two edge kinds each failed exactly one test. Web 134/135, the single failure being the pre-existing avatar test reproduced at base. Turbo typecheck+build 48/48. Manifest `1.6.0`→`1.7.0`.

## 2026-08-08 — Task Manager TM6 slice 11: Playbooks declared, Commons entry findable (ADR-206, AP-126, TASK-021)

- **This deliberately diverges from TM6's own wording.** The deliverable reads "Module + Playbooks as signed Commons **capabilities**". ADR-180 established what a capability IS — the thing that is governed, holding permissions and a trust lifecycle — which is precisely why `view` stopped being one. A Playbook holds no permissions: the Skills it names hold them all, and giving a Playbook its own would create a second place authority could widen without anyone noticing. TM6's line predates that boundary, and following it literally would have re-opened the question ADR-180 closed.
- **But a Playbook still has to be DECLARED.** It is shipped, signed, versioned content that shapes what a model is asked to do. A manifest that omits it hands a fresh Organization five methodologies its own manifest never mentioned, so nothing it installed could be audited against what actually arrived — which is the entire point of signing. New `ModulePlaybookBinding` on the Module surface gets the audit trail without the authority claim.
- **Validated, not just recorded.** A Playbook must name at least one Skill — one that may run nothing is a description, not a methodology — and every Skill it names must be one this Module declares, or a fresh install runs a methodology whose Skills never arrived. Derived from `TASK_PLAYBOOKS` rather than restated, the same anti-drift rule ADR-197 set for `TASK_MANAGER_PLAYBOOKS`.
- **The Commons entry becomes findable.** Commons exists so someone with a NEED can find the capability that meets it; an entry tagged only `built-in` and its kind is present but unfindable — shelfware in a registry. Task Manager now carries capability tags plus three `need:` tags, each naming something it actually ships.
- **The no-personal-data audit is over the serialized entry, not hand-picked fields.** It rejects e-mail markers, the pilot identity, and **any UUID at all** — every id in a published package is a stable kebab-case manifest id, so a UUID there is a runtime row that escaped. A leak arrives in whatever field nobody thought to check, which is exactly why a per-field check would not be an audit.
- Verified: manifests catalog 10/10 with both new tests mutation-checked (shipping no Playbooks, and stripping the `need:` tags, each failed exactly one test); api 463 total / 462 pass / 1 skipped / 0 fail; core 597/597; turbo typecheck+build 48/48. Manifest `1.7.0`→`1.8.0`.
- **Recorded as NOT delivered**: TM6 also names a per-repo agent-ledger template and a live fresh-Organization Commons install. Neither happened — this slice audits what *would* be published, not a performed install.

## 2026-08-08 — CI repair: the in-app companion uses the shipped renderer (AP-127, TASK-021/TASK-036)

- `apps/web/test/avatar-prototype.test.mjs` asserts "the in-app browser companion uses the shipped Zazoo renderer". It was **added RED** in commit `42ff30f` earlier the same day by another author and had been failing on main ever since — `AvatarOverlay.tsx` still drew its own ~190-line SVG figure (per-style ears, snouts, gradient head, its own eye geometry) while `OverlayApp` already drove Zazoo through the same director. The product shipped two avatars that could disagree about what "listening" looks like.
- `AvatarFigure` now renders `ZazooAvatar`. Its props are unchanged, so every call site keeps working. The status→pose map is defined once, in `zazoo/status-performance.ts`, so a new status cannot be expressive on one surface and blank on the other. `director.triggerBlink()` — the capture-tell that same commit added — finally reaches this surface ("Avatar blink is the tell", CLAUDE.md). Reduced motion gets the static `ZazooCompact` head; the figure it replaced hardcoded motion ON, which is the inconsistency `CompanionZazooFace`'s own doc comment says it was written to stop copying. `AvatarStyle` names an animal, a Zazoo species has a name and a `kind` that is the animal, so the map is on `kind` with the default as fallback — a missing match degrades to Zazoo, never to nothing.
- **Two `check:vocabulary` findings were this session's own prose** and were rewritten: a bare `Brain` at a line break (the allow-phrase is "second brain", and the test NAME split it), and `published package` in the TM6 audit test (`package` is retired; the canonical noun is Module). The gate catching my own writing is it working.
- **One allowlist entry added**: `element` in `grid-theme.ts` is `document.documentElement`, the DOM's own name for the root node. That is a foreign contract Bridge does not own — precisely the allowlist's stated criterion — so it is exempted with a reason rather than renamed.
- **The remaining 90 findings are the WhatsApp Module's retired "Tool" primitive, and this session deliberately did not decide them.** They cannot be allowlisted: the allowlist's own contract says "Never exempt Bridge's own vocabulary: rename that instead." The rename is blocked on a real contradiction, now written into `docs/BUGS.md` so nobody re-derives it — the manifest already models each one as a **Skill** capability, while the behaviour is a named UI surface with view modes invoked by a human click, and the glossary says a Human never invokes a Skill directly. Deciding it is a Tier C canon call about another Module's vocabulary, needs its own ADR, and changes persisted `whatsapp.tool.*` capability ids across ~90 sites. Guessing the noun would ship a rename its owner has to undo. Tracked under TASK-036, the designated absorber for CI blockers found by other tasks.
- Verified: `@bridge/web` 135/135 (was 134/135 with exactly this failure); full `turbo run typecheck test:coverage build` **72/72** — the first fully green turbo gate this session; `check:agent-context` exits 0. `check:vocabulary` still exits 1, on the tracked `tool` family alone.

## 2026-08-09 — AI Harness canonized: the K-ladder is the next workstream (ADR-210, AP-131, TASK-044..TASK-054)

- The 2026-08-08 session record (`docs/Progress from Manish/ai-harness-plan-2026-08-08.md`) explicitly withheld itself from canon pending the user's gate. The gate arrived — "start on AI harness" — and this change performs exactly the record's own "next step when development resumes": full raw plan with task mapping (`docs/raw/ai-harness-plan-2026-08-09.md`, wiki companion `docs/wiki/ai-harness.md`), the APPROVALS row (AP-131), TASK rows for the phases.
- Eleven new rows, TASK-044..TASK-054, one per phase K0–K9/K11. K10 is NOT a new row — it is TASK-043, which gains the gate sentence: nothing in K11 turns on until its mechanisms are enforced. Each new row carries a falsifiable prototype test authored now, not deferred (e.g. K2's "with the toggle off, no signal is emitted — asserted, not assumed"; K11's "a password-field keystroke fixture yields at most 'typed in an unknown field'").
- Two stale references corrected rather than copied: the session record's "TASK-040 = morning brief" (that ID has named LA3 Phase 2 since the 2026-08-04 renumbering wave — the brief is TASK-050), and the unified-learning spec left "parallel" (now folded in: the harness plan orders, the spec remains a design source, approving its designs stays a separate user gate).
- The KG technology decision is now an ADR, not session memory: Postgres substrate, LangGraph rejected as a category error, Graphiti's bi-temporal/invalidation ideas behind our own port, Kùzu only ever as a derived rebuildable index.
- No code in this change — it orders what gets built. TASK-044 (K0 spine) starts immediately after.

## 2026-08-09 — AI Harness K0: the flights run live and there is one context door (ADR-211, AP-132, TASK-044)

- The three learning flights (`BRIDGE_LEARNING_OBSERVATION`, `BRIDGE_RETRIEVAL_FUSION`, `BRIDGE_COMMONS_ARCHETYPES`) are ON for both pilot surfaces: `render.yaml` for the hosted deploy, `api_sidecar.rs` for the desktop. Each carries a failing-on-removal test, mutation-checked — the flags were muted, the test went RED, the flags came back. A dropped flag is the worst kind of failure here: every `learning.*` procedure fails closed on purpose, so the harness would simply not exist and nothing would ever look broken.
- `chiefOfStaff.converse` no longer assembles model context by hand anywhere. `invokeAgent` and `classifyIntent` take `{ provider, runCtx }` — the determinism seams travel WITH the model, so the type system refuses a model call that cannot assemble a `ModelRunContext`. `buildAgentSystemPrompt` and `buildCommunicationsSystemPrompt` are deleted; the "answer plainly" line became `DIRECT_REPLY_OUTPUT_CONTRACT`, rendered as the output-contract section so it only exists via `projectToSystemPrompt`. The Communications and classifier prompts now carry the kernel-invariants layer the hand-rolled strings never had. `invokeAgent` gained the `memory` slot — K4's feed point, asserted present-and-rendering.
- **The first live boot with the observation flight ON crashed the API.** `LEARNING_DIGEST_AUTOMATION_ID` was the dotted key `platform.learning.observation-digest`; the durable `automations` table's id column is uuid-typed; pglite refused the seed (22P02) and the process died. In-memory mode accepts any string as an id, which is why the whole suite stayed green for two months. One push later this would have been the hosted pilot down at boot — Render deploys on push regardless of CI (ADR-195). Now `b0000000-…-000000000201` with a seen-RED regression test. Lesson for every remaining K-phase: **a flight that has never run live is not "built" — the flags-on boot is the test.**
- Adding eleven K-rows blew the agent-context activeTasksBytes budget (4953 > 4096). Fixed in the renderer, not the budget: the active-task index now extracts dependency TASK-ids (canonical prose stays in TASKS.md), landing at 3208 bytes. The parser test was seen RED first.
- Verified live on a durable Local Plane: the previously-crashing boot serves `/health`; `learning.status` and `learning.retrieval.status` return `{"enabled":true}`; `learning.archetypes.preview` passes its gate with an honest empty list; a real @communications converse turn flows through the assembled context with the offline fallback intact. Full turbo gate green; Rust sidecar 15/15; core 616/616; api learning 8/8. Honest residual: no local model here, so the model-backed converse paths are proven by unit captures of the projected system prompt, not a live completion; the hosted flags take effect on the next deploy.

## 2026-08-09 — AI Harness K1: the ledger becomes the learning input; the DealPilot mapping is deleted (ADR-212, AP-133, TASK-045)

- New `@bridge/core` `learning/ledger-miner.ts`: a Human's approve/veto/edit on any governed proposal becomes a generic observed signal attributed by `skill`. The privacy boundary is structural — the mapper receives an envelope type that cannot express `inputs`/`proposedOutput`/`diff` — and the one place the full row is in scope was deliberately mutated to leak `row.inputs`; the privacy test caught it. `auto` decisions are never mined: an auto-applied decision is the machine echoing its own calibration, and learning from it amplifies the machine instead of the Human. Deterministic ids make re-mining a no-op with no cursor to lie.
- Deleted, not deprecated: `modules/dealpilot/src/learning.ts` (`dealDecisionSignal`, `sdeBand`), its test, and `learning.recordDealDecision` — which, it turns out, no web surface ever called. The learning loop's entire input side was a mapping one Module carried and no human could reach. A regression test asserts the router path never returns.
- The quiet half of the mapping also died: eight learning procedures carried `moduleId .default("dealpilot")`, so the generic Settings surface had been silently showing ONE Module's learning while presenting itself as "your decisions". Lists now show every Module when none is named; digest and promotions fan out across every Module with signals; archetypes make the Module explicit and required.
- The scheduled digest Skill mines first, digests per discovered Module — and the fan-out set includes already-mined modules, because a pattern that crosses the threshold after mining goes idempotent must still reach the digest (a bug the first design had; caught while writing the result shape).
- **Verified live over HTTP on the durable Local Plane**: three `automation.runById` triggers each halted `pending_review` (agents draft, humans approve); three `action.decide` approvals; `learning.digest` mined 3 signals and proposed "You have chosen 'approve' 3 times when skill is 'learning.observationDigest'. Remember this as a preference?" with its evidence signal ids; the suggestion is on the generic list; a second digest mined 0 and proposed 0. Core 6/6 with three seen-RED mutations, api learning suites 21/21, full turbo gate 72/72, agent-context clean, vocabulary unchanged at the 90 tracked WhatsApp findings.
- Honest residuals: mined decisions attribute to the mining owner (the ledger's decision row inherits the proposer's actor — single-tenant-correct, named gap); K9 rung 3 is unlocked by this slice, not built in it.

## 2026-08-09 — The semantic-embedder seam stops lying about reachability (ADR-213, TASK-032/TASK-044)

- Found by reading, not by a crash, while answering "what is required for semantic embeddings?": `resolveSemanticEmbedder` returned an embedder whenever an `OllamaProvider` was REGISTERED, and persistent mode registers one unconditionally. Constructing that adapter proves a base-URL string exists — it says nothing about a daemon listening on `localhost:11434`.
- Two failures followed on any persistent-mode boot without Ollama: the scheduled memory indexer called `embed` unguarded, so **every 15-minute pass threw and no vector index was ever built**; and chat's query-embed catch produced an **empty vector lane**. Fusion silently ran on structured + graph recency with nothing saying so — a capability claim derived from an adapter existing rather than from the adapter working.
- Latent until K0 (ADR-211) turned `BRIDGE_RETRIEVAL_FUSION=1` on. Blast radius stated honestly: **NOT the hosted pilot** — Render sets `BRIDGE_LOCAL_RESIDENCY=public-cloud`, so `publicCloudOnly` gates the indexer off there — but every local/self-hosted persistent-mode boot, which is the desktop pilot's direction of travel.
- Fix (ADR-213): `withReachableEmbedder` asks the reachability question at each point of USE. A boot-time probe was rejected as the frozen-value defect class this repo has now hit three times (ADR-205/209/211) — a daemon started after boot would never be noticed — and the attempt IS the probe, so it costs no extra round trip.
- Four invariants: the WHOLE embedder swaps (id names the space; lexical vectors must never land in the semantic space); indexer and query degrade by the SAME rule (falling back on one side only leaves a permanently empty lane); **a degraded pass never reclaims** stale spaces, because "did not answer this minute" is not evidence a space is superseded and a recovery must not find its index already deleted; a failing LEXICAL embedder throws rather than being retried against itself, since pure computation cannot be down. The downgrade now warns with the configured and active space ids — a silent drop from semantic to token-overlap is a dishonest surface.
...
- The emission moment is POST-APPROVAL materialization, not sync time: the human's approval of an intake row is the warrant for learning from it, and a vetoed record never becomes a signal (asserted). `emitGoogleCaptureSignals` hooks both `google.onApproved` call sites; ids are deterministic per SOURCE record, so decide replays and reconciles never duplicate; emission failure logs and never turns an applied approval into an error.
- Metadata-first, structurally: the core envelopes (thread → sender/subject/time; event → summary/attendees/time) cannot express `snippet`, `bodyText`, or `description` — the API test drives the REAL pipeline over a fixture gateway carrying distinctive content strings and asserts they never reach a signal row. Two seen-RED mutations at the API layer (consent gate stripped: consent-off test fails; snippet spliced into the envelope call site: leak test fails) plus two in core (google-defaults-on; subject-dropped).
- The staged calendar payload now carries the invite-list emails (additive; every consumer `.passthrough()`s) so the approved graph row and the signal both know who the meeting was with. New `BuildWiringOptions.googleGateways` seam keeps runtime env-only and fail-closed while letting composition tests source fixture data.
- Live durable-boot walk: fresh plane shows google OFF beside chat/whatsapp → Human flip records who/when → `google.syncGmail` fails CLOSED with the no-fake-gateway message (no OAuth on this machine) → pause silences with consent kept → resume restores verbatim → restart proves the consent row durable. Full `pnpm verify` green; one UNRELATED culture-research socket-abort test pair timed out at 120s under the fully-parallel 36-minute run, passed 57/57 in isolation and green on the cached rerun — recorded here, not hidden.
- K6 (TASK-050) is now unblocked on both dependencies (TASK-047, TASK-049). The standing blocker for real Google data anywhere remains ADR-215's OAuth client registration (user-owned).
- Collision + forward-fix at push time, same pattern as the last three nights: the parallel UI session's `f8be8d6` took ADR-221/AP-141 first (this rung renumbered to ADR-222/AP-142, later-arriving side renumbers per precedent) and left `check:vocabulary` red on main (a retired word in a new ui-conformance test title — that session still does not run the gates before pushing; reworded to canonical vocabulary here, web tests green).
...
- Repair (this commit): upstream canon restored (ADR-197..216 intact, all K-ladder rows back); the branch's novel content kept, not discarded — its companion ADR grafted as **ADR-217**, its six companion tasks renumbered **TASK-055..060** (graft note in TASKS.md), its AP-135 allowlist row kept as pushed; the OAuth session's colliding AP-135 renumbered **AP-137** with all cross-references updated (later-arriving side renumbers, the TASK-029/032/038 precedent).
- Consequence of its CI fix landing: `check:vocabulary` now passes on main, so the `platform` job can go green and `supabase-migrate` runs again — meaning migration 0040 (claims substrate) applies to hosted Supabase on THIS push rather than waiting, and the K2-era BUGS entry about migrations-not-applying is resolvable pending one green Actions run.
- Second collision, same evening: while this repair was in flight the cloud branch pushed `b9f673b` (TASK-038 M1/M2/M4) — whose M1 is an INDEPENDENT oauth-token encryption (per-install key file stored beside the database, transparent plaintext fallback, no AAD). Superseded at merge by the ADR-215/AP-137 implementation (operator-held vault key, per-Integration-per-field AAD, fail-closed wrong-key, explicit forward migration) — strictly stronger on all four properties; `b9f673b`'s M2 (blueprint membership) and M4 (OTP proof gate) are kept verbatim. Noted: that commit shipped without tests; M2/M4 remain untested on main.
- `b9f673b`'s M4 also broke a pre-existing api test (`security-hardening.test.ts` "the honest paths (null / phone) are still accepted") — that test asserted `phoneVerified: true` from a bare client declaration, which is precisely the hole M4 closed. Test updated to the new contract (unproven phone records the method but stays unverified; a consumed OTP proof verifies): 12/12, and M4 thereby gains its first test coverage.
- Post-push, the cloud branch kept landing: `14d44d2` deliberately set `BRIDGE_COMMONS_ARCHETYPES: "0"` in production (TASK-034(d)) without seeing the brand-new blueprint flight drop-test, leaving main's `render-blueprint.test.mjs` red. Test updated from "all four ON" to pinning each flight's DELIBERATE state ("1","1","0","1") so drift in EITHER direction is caught — a quiet re-enable is as much a defect as a quiet drop.
- And `cec9e01` (childRun cancel button) shipped calling `trpc.childRun.cancel` — the procedure lives at `agentOrchestration.childRun.cancel` — leaving main's web typecheck red (that session evidently runs neither web typecheck nor the blueprint test before pushing). Call path corrected.
- Named gap, not fixed here: three sessions were writing one working tree and one remote concurrently this evening (K3 local, OAuth local, CI-fix cloud); TASKS.md's id-allocation collisions keep recurring at merge time. The renumber precedent handles it, but a durable fix (id reservation or per-session id namespaces) belongs to a future task row if the user wants one.

## 2026-08-09 — AI Harness K3: the claim substrate — one store, two projections (ADR-216, AP-136, TASK-047)

- K3 shipped in the user-directed minimal second-brain cut: `claim_entities` + `claims` (migration 0040; bi-temporal `valid_from/valid_to` + `recorded_at/invalidated_at`, `superseded_by` lineage, `decision_ref` NOT NULL), a pure suggestion lifecycle in core riding the preference/promotion Memory-CAS machinery, Human-only `learning.claims.*` where acceptance traverses the governed pipeline (`pipeline.decide(approve)` IS the recorded Human decision — which the K1 miner reads back as learning input), the fusion graph lane retrieving live claims, and the Second Brain page rendering the SAME rows (entity/claim nodes in `graph.full` + a Claims panel: propose, accept/reject, supersedence history, forget). What the user sees is literally what the model retrieves — one substrate, projections that cannot diverge.
- The build collided with canon in exactly the way the guards were built for: `check:vocabulary` retires the whole `knowledge` family, and the first implementation's `knowledge*` identifiers added ~150 findings. Renamed the engine vocabulary to **Entity + Claim** end to end (tables, store, router namespace, flight `BRIDGE_CLAIM_SUBSTRATE`, resourceType `claim`); "knowledge substrate" survives only as ADR-210 plan prose; findings returned byte-identical to main (exactly the 90 tracked WhatsApp/manifests rows). The guard did its job on day one of the surface, which is the cheapest day.
- Structural guarantees in the K2 style: red claim classes are not members of the proposable union (type + zod, ADR-176 invariant 15); no claim row exists without a governed proposal id; contradiction supersedes in-transaction and never deletes; `forgetClaim` is the only true delete and clears inbound lineage pointers; sensitivity is raise-only with `unknown` resolving UP; only the Human principal holds the `claim:write` grant (persistent `ensureClaimUserGovernance` + in-memory seed) — the RED probe for this was real: the pipeline REJECTED materialization outright until the grant existed.
- Verified: core 7/7 (three mutations seen RED), db 4/4 (0040 proven on pglite by `createLocalDb`), api 8/8 (a rejecting pipeline leaves the suggestion PENDING and writes nothing), migration no-op pin advanced to 0040/0041 and seen RED first, hosted blueprint gained flight drop-tests for ALL FOUR flights (K0 had shipped render.yaml flags with no drop-test — closed), turbo 72 tasks green, and the flags-on durable Local-Plane boot walked 10/10 over HTTP: propose → accept (decisionRef = proposal id, ledger row exists) → `graph.full` shows the claim → contradiction supersedes with history intact → forget deletes → reject suppresses forever; a flight-off boot answers `enabled:false` and fails closed. The Second Brain UI was walked in a real browser against the live plane (propose → accept → claim row + `claim of` edge rendered next to tasks/modules/agents).
- Honest limits: v1 entity identity is exact normalized name per kind (no fuzzy refusal band yet — that clause of the prototype test moves to the pre-agreed dedupe follow-up); the distiller/decay loops are deferred per AP-136; hosted Supabase gets 0040 when `supabase-migrate` goes green — TASK-036's allowlist landed in parallel (5d93778), so that is expected on this very push; the browser walk needed a dev-only auth-injecting proxy (the desktop sidecar surface is where this UI actually authenticates).

## 2026-08-09 — oauth_tokens encrypted at rest, closing the plaintext-token gap Option B depends on (ADR-215, AP-137, TASK-006/TASK-038)

- ADR-215/AP-137 chose Bridge's own Google OAuth app + the existing local vault over a hosted OAuth broker (Composio) for TASK-006. Reading the already-built Google OAuth pipeline (`google-oauth-routes.ts`, `packages/integrations-google/src/oauth.ts` — PKCE consent, callback, token exchange, local persistence, all pre-existing on `main`) to confirm "vault-backed" was actually true surfaced that it was not: `packages/local/src/stores/pglite.ts`'s `oauth_tokens` table stored `access_token`/`refresh_token` in plaintext columns — exactly `docs/BUGS.md`'s OPEN 2026-07-08 finding, unfixed since filing.
- Fix: new `packages/local/src/stores/oauth-token-crypto.ts` (AES-256-GCM, one random IV per encryption, AAD bound to `<integrationId>:<field>` so a ciphertext can never be replayed into a different Integration or the other field). `oauth_tokens` gains `access_token_encrypted`/`refresh_token_encrypted` JSON-envelope columns; a new migration (`oauth-token-encryption-migration.ts`) upgrades any pre-existing plaintext install in place (encrypt-then-drop-column) and is a no-op on a fresh install or an already-migrated one.
- The one real design snag: compare-and-swap previously matched columns via raw SQL equality (`access_token = $4`), which cannot work against ciphertext that differs on every write (fresh random IV by design). Reworked to decrypt-then-compare in application code — safe because every mutation on a given `integrationId` was already serialized through the store's own per-key exclusive lock, inside a Local Plane a single process owns at a time (file-lock-enforced for `dataDir`-backed instances).
- Key source: reuses the SAME `BRIDGE_CREDENTIAL_VAULT_KEY_ID`/`BRIDGE_CREDENTIAL_VAULT_KEY` pair already used for the DealPilot encrypted-file credential vault — one operational secret, not two. `apps/api/src/wiring.ts` now fails closed at boot if a durable `BRIDGE_LOCAL_DIR` and real Google OAuth (`GOOGLE_CLIENT_ID`/`SECRET`) are both configured without that key. An in-memory/ephemeral Local Plane (tests, no `dataDir`) gets an auto-generated ephemeral key from `createPgliteLocalPlane` itself — nothing there survives restart regardless, so there is nothing to fail closed on.
- Verified: `@bridge/local` 41/41 (new test proves the raw table never contains the plaintext token substring and that a wrong key fails closed with an authenticated-decryption error, not silent garbage or a plaintext fallback), `@bridge/integrations-google` 39/39, full `apps/api` suite 501/501, all three builds/typechecks clean.
- Not claimed: no live boot against a real, verified Google Cloud OAuth client — that half of TASK-006's gate is unchanged and requires the user's own Google Cloud Console registration (external account action, outside what this session can perform).

## 2026-08-09 — AI Harness K2: the local stores emit, and consent is the only door in (ADR-214, AP-134, TASK-046)

- Chat threads and the WhatsApp store now emit learning signals — behind a NEW consent surface, because a new USE of already-held data is a new grant, never an inheritance. Every source defaults OFF, and the parser fails CLOSED: the only stored value that reads as consent is a well-formed boolean `true` written by the consent surface itself.
- Envelope-only by construction, K1's discipline one rung up: the mappers receive types that cannot express a turn's `content` or a message's `body`. Inbound WhatsApp maps to null (someone else's act — learning the owner's rhythm from it would attribute another person's behavior to the owner); the assistant's chat reply never emits (machine output); cloud-plane turns map to null before any consent question.
- The kill switch (`paused`) silences every source WITHOUT rewriting per-source choices — a circuit breaker, not a bulk consent edit. Flips are Human-only (agent/team identities FORBIDDEN) and inspectable (changedAt/changedBy on every toggle). Settings gains the CaptureConsentCard so the decision exists for the person, not just the API (ADR-208's rule).
- Attributes are deliberately two low-cardinality facets with a real consumer: `timeOfDay` (K6's brief rung) plus `surface`/`chatKind`. Deterministic signal ids make replayed turns and re-ingested windows no-ops.
- Seen RED: five core mutations (default-on, truthy parse, pause bypass, inbound admitted, cloud admitted) and two API mutations (consent gate bypassed at both emission points; message body leaked at the one call site with the full row in scope). Core 9/9, api 7/7, turbo 72/72.
- Live over HTTP on the durable Local Plane: default all-off → 3 outbound messages under consent-OFF produced no pattern → Human flip recorded → 3 outbound + 1 inbound under consent-ON produced exactly "You have chosen 'send' 3 times when chatKind is 'direct'" with 3 evidence ids and no message text anywhere → one evidence signal deleted over HTTP → pause silenced an enabled source with consent intact → chat consent + three turns produced "converse × 3 when surface is chat_panel". Live-boot footnote: the durable boot requires `BRIDGE_DEALPILOT_CREDENTIAL_VAULT=encrypted-file` + a vault key — `disabled` is public-cloud-only (fail-closed working as designed).
- Honest residuals: live chat turns' model step failed (no local model on this machine; the user turn and its signal persist before it); the Settings card is typecheck-verified, not browser-walked; WhatsApp signals attribute to the ingesting identity (ADR-212's named gap); timeOfDay uses the API's local clock. Per the user's directive, the session stops after K2 — K3 (TASK-047) is the next ready phase.

## 2026-08-10 — UI standardization: root-cause post-mortem, conformance gate, TASK-061

User challenged why previously-shared UI rules kept disappearing and why "every page still looks different". Audited all 21 pages in `platform/apps/web/src/app/pages/` by import analysis.

**Finding (root cause).** The rules were advisory only — nothing in `pnpm verify` failed when a page ignored them. Two toolbars coexist: `StandardToolbar.tsx` implements §5's canonical order and has ONE consumer (ApprovalsPage), while the real Module pages render `<DataViews>`'s own inline toolbar (view Select · search · filter · Columns · ControlPanel) which has no List dropdown, no Insights chevron, and Add-column outside the 3-dots. 5 of 21 pages are fully conformant. The `ArtifactsPage` built earlier in this same session is a live instance of the failure mode, not a historical one.

**Landed.**
- `platform/apps/web/test/ui-conformance.test.mjs` — the corrective measure. Fails the build when a page bypasses `ModuleSurfaceLayout`+`DataViews` without a written `EXEMPT` reason, or hand-rolls a dropdown/search slot/raw table. Pre-existing divergences sit in a RATCHET that can only shrink: a new offender fails immediately; a fixed entry left in the list also fails. Asserts specific canon directly (dropdown delegation, red-flag click-to-clear, keyboard parity). Seen failing unfixed before the ratchet was added (2 of 5 tests red, naming the exact 10 divergences).
- `components/shared/StandardDropdown.tsx` — §5e, the one dropdown primitive: selected-first, type-to-filter search (auto past 6 options), pinned "＋ Add" that never scrolls away. `ListDropdown` and `<DataViews>`'s View switcher converted onto it; the bespoke Radix Select in DataViews removed.
- `RedFlagControl.tsx` — clicking a red flag now REMOVES it (plain click is a toggle). Inspect/edit/reason/correction moved to right-click / Shift+Enter, keeping §5d's keyboard-parity requirement.
- `docs/raw/ui-architecture-rules-2026-07.md` — §0 (rules are generic, no per-Module standard), §5 additions (Form is a view not an Add-row alternative; no Build/Preview split; click-outside autosave; Sort inside 3-dots), §5e (standard dropdown), §5f (three menus, one definition each), §5g (shell invariants: hover-revealed panel controls, `h-14` header alignment, Zazoo present every launch), §10 (post-mortem, corrective measures, adoption audit, honest open-gaps list). Wiki companion updated.
...
**Cell right-click (33d15f69).** New `StandardCellMenu` — one definition for every Module. Cell-scoped commands (Edit cell / Copy value / Clear cell / Flag) above a row-scoped half that REUSES `StandardRowMenuItems`, so caret and right-click cannot drift. Delete row present, disabled with its reason pending a governed delete with dependency preview + undo. **The red flag moved off hover into this menu** per user directive; `RedFlagControl` gains `affordance` defaulting to `"menu"` — the flagged STATE is still always drawn (§5d is about the indicator, not the invitation) and focus still reveals the glyph, so keyboard parity holds. `useOptionalRedFlagContext` added so the menu renders on Modules without flag support instead of throwing.

**Capability audit** (`docs/raw/capability-audit-notion-airtable-evernote-2026-08-10.md`, wiki companion + index). Bridge measured FROM CODE, not docs; declared-but-unimplemented scored as absent. Headline findings: 11 of ~26 benchmark field types; 8 view kinds (`DashboardView.tsx` exists but is **not registered**, so it cannot render); **`formula` is declared in the ColumnKind union with no evaluator anywhere — a phantom capability**; and the largest structural gap, **`ViewConfig` is never persisted**, which alone gates saved views, saved filters/sorts/columns, Lists that survive reload, personal-vs-collaborative views, linked views and view sharing. Bridge-only strengths recorded as the moat to protect: governed writes, provenance on a value, red-flag→learning, Skill columns, local-first residency, Graph/Tree/Second Brain, signed Commons.

**Canon added:** §3b — Notes and Governance are now MANDATORY Sections on every Record, not Module-optional ("Notes without Governance is Notion; Governance without Notes is a compliance tool; the pair is the product"). §3c — Helpdesk is a Database, Custom Helpdesk is an associated Database rendering as a sibling toggle, and "Public" is a property-level feature (Record Detail checkbox + right-click command), never a database type.
...
## 2026-08-13 — K7: the first ambient sensor is live — app-focus capture through the @bridge/sensors hub (ADR-230; AP-148; TASK-051 done)

**The kernel-side SPI finally has a host.** `@bridge/sensors` had enforced the capture contract against a fake provider since it shipped; the Tauri shell had been buffering real NSWorkspace frontmost-app observations nobody drained. K7 is the connective tissue: "apps" as the fifth consent source (K2 machinery untouched), one SensorHub per capturing user hosted by `buildWiring` (capability-manifested provider, computed risk, blink DomainEvent per ingest), a `PushContextProvider` for the process boundary that DROPS fail-closed while stopped, and a learning-loop consumer that writes the one observed-signal Memory per focus — the "Learning Agent consumes context, not screenshots" seam running for real.
...
## 2026-08-13 — K9 rung 3: the Builder drafts automation steps from ledger evidence (ADR-231; AP-151; TASK-053 partial — rung 4 blocked on TASK-042)

**Derivation, not generation.** The promotion machinery deliberately stopped at empty-steps drafts ("a fabricated step would be a lie"). Rung 3 fills them the strictest way possible: no model call — `draftStepsFromEpisodes` reads the modal governed shape (action/resourceType/dataScope) out of the affirming HUMAN ledger episodes behind the accepted pattern (approve/edit only; a veto is evidence AGAINST repeating), constrained to the live skill registry, one step per draft (multi-step chains are rung 5, behind the K10 gate). The episode envelope has no payload fields — content is unrepresentable in the drafting path, the K1 miner's own posture reused. The API lane validates through the same `parseAutomationSteps` every registry write gets and saves the draft STILL as a draft; activation is untouched.

**Refusals over guessing.** Structured, specific, and they write nothing: a K7/K8 behavior rhythm has no skill to bind (`pattern_not_skill_shaped` — rendered verbatim in the drafts card's new "Draft steps from my decisions" control), an unregistered skill proposes nothing (the prototype test's core), zero remaining episodes proposes nothing, an unrecognizable action refuses rather than widening. The empty draft still cannot activate.

**Honest scope split.** Rung 4 (structure synthesis from K3 claims) declares TASK-042's per-class input packs as its dependency — and TASK-042 has never been executed. TASK-053 now records rung 3 DONE + rung 4 blocked instead of a blanket "ready". **Verified**: core 6/6, api 4/4 over real buildWiring, four mutations seen RED, verify 77/77 (one earlier gate run flaked on the pre-existing culture-research cancel test under full-machine load — green in isolation, filed); live durable-boot walk 9/9 on real captured K7 focus data including restart-stable refusal; real-browser drafts-card walk.

## 2026-08-13 — K10: the five ADR-176 constitution mechanisms become executable obligations (ADR-234; AP-152; TASK-043 done)

**Five mechanisms, one checkpoint each.** E1 is a build-time structural scan (`port-allowlist.json`, approval-gated) pinning exactly what `core/learning` and `core/memory` may import outside their own scope — adding a port fails until the allowlist changes, and unused allowlist entries fail too (drift in either direction is caught). E2 stamps every acceptance with a sha256 of the exact text the client rendered, so a bulk-accept audit can tell REVIEWED (stamp matches the canonical suggestion) from UNVERIFIED (no stamp, or a stale-tab mismatch) without ever blocking the acceptance itself. E3+E4 share one proposal-time gate in `claims.ts`: red content (health, protected characteristics, psychological/relationship conclusions, financial distress) is refused outright regardless of the claimed class; amber content renders as an observed fact with its evidence shown, never as a conclusion; every proposal needs >=1 evidence reference, checked per clause so a compound statement can't smuggle an unevidenced clause past a evidenced one. E5 fingerprints a rejection's semantic content and suppresses paraphrased repeats for 30 days, then 90, then permanently on a third strike — deleting the fingerprint Memory is the un-suppress affordance.
...
- **2026-08-16 — Stage 1 of the Accounting/D2C module merge: canon only, no code moved (ADR-246/231/232, AP-162/149/150)**:
  ...
  `CLAUDE.md` rule additions, AP-162/149/150, ADR-246/231/232 + wiki one-liners, TASK-074 (Stage 2, code)
...
## 2026-08-14 — TASK-042: per-class input packs validated and canonized; K9 rung 4 unblocked (ADR-238; AP-156; TASK-042 done)

The user chose TASK-042 over deciding K11's keystroke question ("start on next task for AI harness" → "Defer — do TASK-042 instead"), so the K-ladder holds at K10 and the never-executed pack-validation task finally ran. Method replicated the ADR-175/177 series per artifact class: for each of Skill, Module, and Integration, one isolated agent got the full six-layer pack and one got only the platform-neutrally-reworded class harness — the harness-only arm IS the user's boundary-conditions hypothesis, run per class. Ground truths were the shipped TM3 planning Skills, the WhatsApp Module, and the Google Integration; rubrics were pinned before any output was generated and are preserved verbatim in the report this time (`outputs/2026-08-14-regeneration-test-per-class-packs.md`), correcting the 2026-08-04 report's own noted gap. The packs carried the five fixes all three original agents had unanimously requested — concrete taint lattice, Proposal/Decision shape, MemoryStore ops, consent contract, and an HONEST budget-primitive inventory (no general per-port meter exists; `RunBudgetMeter` is an ADR-177 aspiration, not code).

**The headline replicates at every class: boundary conditions recover the safety architecture; what they cannot recover is composition and purpose.** All six round-1 arms produced structural containment, fail-closed durable caps, staged-then-approved writes, deterministic-id idempotency, and slice-1 injection gates. Three harness-only inventions converged exactly with shipped decisions the agents never saw — capability-exclusion webview containment (ADR-158), apply-time staleness re-validation (ADR-199), sensitive-class unrepresentability (K10-E3). But every harness-only arm also rebuilt installed machinery under invented vocabulary (the ADR-175 "well-built island", now measured at three scales), and the Integration harness-only arm delivered the series' sharpest finding: a design satisfying every stated obligation that quietly deleted the product, by quarantining the exact metadata attributes (subject/counterparty/participants) the learning lane exists to mine. The full arm differed by one mandate sentence — "metadata is the product here". A harness can pin safety; it cannot pin purpose.

**Every round-1 WORSE traced to exactly one missing pack sentence, and one amendment round closed every gap.** Both Skill arms shipped construction-time provider resolution — the precise defect ADR-197 fixed — because the contracts layer never said provider health is dynamic; amended, the rerun resolved per call and pinned it with a dedicated late-provider test. Both Module arms read "consent-shaped gate" as OWNER consent, missing recipient-wrote-first/near-identical-refusal/warm-up; both missed trade-dress; both missed ADR-159's key-space-lookup principle — all three amended with shipped ADR language, all three regenerated exactly on rerun, including "severing is a delete, not a migration". The Integration full pack needed no amendment at all (10/10 round 1, with metadata-only OAuth scopes making bodies unfetchable at the credential level — beneath our type-level ban). Verdicts canonized in `docs/raw/capability-input-packs-2026-08.md`: Skill packs may run on harness + contracts/registry; Module and Integration packs carry all six layers with the mandate load-bearing.

**Honesty rows.** All eight runs finished with 0 tool calls, but one rerun cited "the user's pending keystroke decision (K11)" — absent from every prompt — proving subagents inherit the host session's instructions and memory index; neither contains ground-truth design content (verified by inspection), so the grading stands, and the 2026-08-04 runs ran under this same envelope. Ten adopt-candidates were recorded with repo-verified status and NOT enacted (two turned out already shipped in substance; one — scope narrowing — would change approved product behavior and is the user's call). The roster-cleanup line in TASK-042's scope was found already resolved by ADR-176. TASK-053's rung 4 blocker clears; it starts on its own explicit pull-forward, and K11 stays where the user left it.

## 2026-08-16 — K11a: the input-capture boundary ships before its producer (ADR-239; AP-157; TASK-054 in progress)

The user resolved the harness plan's last open decision in favour of FULL CONTENT keystroke capture — the more invasive of the two options the plan offered, against its own recommendation of event-only — which makes the fail-closed boundary the entire product promise rather than a detail. So this slice built the boundary, its API lane, and its consent surface, and deliberately built NO producer: there is no event tap yet, and the lane is inert until the Rust provider lands. That ordering is the K10 gate working as designed — the guarantees are executable tests before raw capture flows, not after.

**One module owns every safety decision, and both sides of the process boundary run it.** `input-capture.ts` is pure — no ports, no I/O, no clock — so the desktop provider and the API lane distil through the same code and the whole thing is testable with no OS, no event tap, no model. The field-role gate treats `undeterminable` exactly like `secure`, and does so structurally: `CONTENT_ALLOWED_ROLES` is a single opt-in set, so a FieldRole added later without being opted in suppresses by default rather than capturing by default. A denylisted app emits nothing at all — not the text, not a marker, because a bank must not reveal that the user was typing. The denylist parse fails closed to a seed floor of password managers and banks that the core builder re-merges on every save, so a human cannot edit their way into capturing 1Password. And the API lane re-distils what arrives rather than trusting the shell's verdict: the shell is the component most likely to be tampered with, re-running the gate costs one function call, and the wire schema has no field a raw keystroke stream could arrive in.

**The adversarial pass found a real leak that review did not.** The first implementation's suppressed summary read `"Typed 8 characters in a secure field in Mail"` — publishing the exact length of a password into a durable, inspectable Memory row. Password length materially narrows a brute-force space, so a boundary whose whole promise is "never the characters" was quietly handing over the count. Suppressed bursts now carry no count at all, not even a coarse band, and the test asserts that two different-length secrets in the same field distil to byte-identical events. The rule it generalizes to is worth keeping: **for a field we refuse to read, the metadata about what we refused is itself sensitive.**

The Settings card gives typing capture a severity treatment and a confirm-to-enable step the other five sources do not get, because it is the only source that stores what the user WROTE rather than a metadata envelope — turning it on is deliberate, turning it off stays one click. Verified: core 37/37 across the contract suite and a dedicated red-team suite written as attempted bypasses (denylist evasion by case/whitespace/subdomain, rogue field roles, redaction evasion across card/SSN/token forms, oversized bursts, the length-inference above), every safety test removal-fails; `@bridge/api` green over the real composition root; `pnpm verify` green. Honest residual: macOS `EnableSecureEventInput` is what actually stops the tap seeing a password field in the common case — our gate is the second line for when it does not, and the product promises a fail-closed boundary, not omniscience.

### 2026-08-16 — K11a, second pass: what running it found (ADR-240, TASK-054)

The first pass shipped the boundary and proved it with unit tests. This pass stood the lane up for real — a persistent Local Plane, the learning flight on, and the API reached over an inherited-fd loopback listener, which is the transport the Tauri shell actually uses and the only one `listenServer()` will accept alongside a sidecar token. Satisfying that guard properly rather than routing around it mattered: what rendered and what answered was the real persistent deployment, not a test-mode approximation of it.

**Two defects, both found by exercising the thing rather than reading it.** The first: `learning.capture.input.burst` returned `captured: true` for a SUPPRESSED burst, because a no-content marker Memory had in fact been written. Any caller branching on that field — the Rust provider's telemetry, an Avatar blink, an audit surface — would have reported a password field as captured. That is the one claim this boundary exists to never make, and it is ADR-239's password-length leak one level up: it is not enough for the boundary to withhold the data, it must also not claim to have taken it. `captured` now means content was stored and `recorded` means a row exists, and a suppressed burst is `{ captured: false, recorded: true, verdict: "suppressed" }`.

The second is larger and is not fixed here. AP-157 recorded the user's explicit choice of FULL CONTENT keystroke capture over the harness plan's recommended event-only shape, and the code does not deliver it: `recordSignal()` persists `{ anchor, recordKind, recordId, attributes, observedAt }` and has no body field, so the distilled, redacted `content` is computed, handed to the envelope, and dropped. What actually persists is app name, bundle id, a bucketed keyCount, a disposition, and a time-of-day bucket — functionally the event-only design that was rejected. It is fail-safe rather than leaky, and the live walk shows why the on-disk sweep was so clean: a burst carrying `pay with 4111111111111111 today` leaves no trace of that string anywhere in the Local Plane, because no path stores it. Closing it means widening `ObservedSignal`/`recordSignal`, which chat, WhatsApp, Google, browser and app-focus all share and whose ADRs promise metadata only — a cross-lane privacy decision with its own blast radius, and the safe direction is the status quo, so it stops here for the user rather than being widened silently. The Settings card's "the only one that stores what you type" is false as a result, and is left unchanged on purpose: correcting it to describe facets-only would quietly reverse an approved decision.

**The lane also had no API-level tests at all.** ADR-239's "`@bridge/api` suite green" was accurate and did not mean this lane was covered — the suite simply had nothing for it. It now has eight tests over the real `buildWiring()` root, pinning what the SERVER does rather than what the pure module does: a caller that reports `content_ok` for a denylisted app still stores nothing, two secrets of very different lengths persist byte-identically, the write is idempotent per burst id, and only a Human identity may report a human's own typing. Live walk over the real sidecar: consent-off refusal, denylisted silence, secure and undeterminable suppression, redacted capture, idempotent replay, and a sweep of the whole Local Plane finding zero occurrences of five planted secrets.

### 2026-08-16 — K11a, third pass: the tests were green and the card was lying (ADR-241, TASK-054)

The first two passes proved the boundary's logic. Neither looked at the surface a human actually consents through. This pass ran a harsh critic against the card as rendered — real computed colours, real contrast ratios, real bounding boxes — and it found something no unit test could have: the card promised a protection the code does not provide.

"Password managers and banking sites are always included," said the never-capture list. `SEED_DENYLIST_DOMAINS` holds `1password.com`, `bitwarden.com`, `lastpass.com`, `dashlane.com` — four password managers and no banks at all. The claim appeared in four code comments, three UI strings, and ADR-239's own summary; it had been written once and then propagated by everything downstream that trusted it, including me. A user reading that sentence has no reason to add their bank, and would then have their online-banking typing captured while believing the product had already handled it. That is worse than a missing feature: it converts the user's caution into misplaced trust.

The fix was to make the claim true rather than the list longer. Seeding banks was tempting and wrong — there is no bounded, maintainable list of the world's banking domains, so any list gives false comfort to every user whose bank is missing, which is the identical failure one layer down. The seed floor now promises exactly what it enforces, and the card says plainly that banks are not on it, why, and that the user should add their own.

Three more from the same pass were real rather than cosmetic. "Pause all" disabled the toggle outright, so while paused a user could not turn typing capture **off** — directly contradicting the confirm dialog's "you can turn this off at any time". An off-ramp is never disabled; pause may block enabling, never revoking. The denylist editor rendered only after capture was already on, while the consent dialog told the user capture happens "in apps you have not denied below" — asking them to rely on exclusions they had never been shown. And the seed floor rendered as ordinary editable text, so deleting an entry appeared to work and silently reverted on save, because the core re-merges it; a control that accepts an edit and discards it lies about system state. Seed entries are locked chips now.

The rest was craft: the armed state for the most invasive source was pixel-identical to the five benign ones (an amber "Capturing" pill now, not a steel "On"), the disclosure carrying every safety promise sat at roughly 2:1 contrast, the severity badge wrapped mid-word on mobile, and the editor's amber made it read as a seventh capture source so amber meant both "danger" and "sub-panel".

The lesson is the one worth keeping. Every test on this boundary passed — 37 core, 8 API, a red-team suite written as attempted bypasses — while the sentence above the toggle was false. Logic correctness and consent correctness are different properties, and only one of them had been checked.

### 2026-08-16 — K11a, fourth pass: two of three regressions came from the fixes themselves (ADR-242, TASK-054)

The critic was run a second time, told to verify each claimed fix empirically and to hunt for regressions. It confirmed the real wins — the bank falsehood gone, review-before-consent, the armed state distinguished, the disclosure at 10.38:1 — and then found that the fixes had introduced three new defects of their own.

The worst was mine and it was subtle. Renaming the armed label from "On" to "Capturing" made the invasive source stop reading as a peer of the benign five, which was the point. But the label keyed only on whether the source was consented, so while "Pause all" was active the row sat there in full-saturation amber saying **"Capturing"** about a sensor that was capturing nothing. The old wording had been accidentally safe: "On" describes the consent switch, and a switch really is on even while the system is paused. "Capturing" describes live behaviour, and live behaviour is exactly what the kill switch changes. The rule worth keeping is that a control's label may describe its own state or the world's, and if it describes the world it has to answer to every switch that changes the world.

The second regression: the denylist editor had been moved to render before consent, but was still gated behind `!status.paused`, so pausing removed the exclusion list altogether — the user could neither review nor edit it. A denylist is a policy editor, not a live control, and it now renders regardless.

The third was invisible in the most literal sense. The "locked" seed chips were styled with `bg-[var(--color-mist)]`, and `--color-mist` does not exist anywhere in the app: one usage, zero definitions. It computed to transparent, so the entries a user cannot remove rendered as bare text on the card background, distinguished from ordinary text only by a padlock emoji that then orphan-wrapped onto its own line on mobile, detaching the lock from the thing it locks. An invented design token does not throw; it just quietly renders nothing, which is a good argument for measuring computed styles rather than trusting the class list.

Two smaller ones, both about failing loud. A fetch error in the denylist editor called `setLoaded(false)` and returned `null` — silently deleting the exclusion list that the consent dialog explicitly points at — and it now shows a warning telling the user not to enable capture until the list loads. And the new `aria-label` no longer contained the button's new visible word, which breaks WCAG 2.5.3 for anyone driving the app by voice.

The pass also caught two stale claims that survived the bank correction: a test still *named* "denies password managers and banks out of the box" whose body asserts no bank at all, printed on every CI run, and a source comment still calling the seed floor "removable-with-a-warning" after it had been locked. Both fixed, and there is now a removal-fails test pinning the honest contract — `chase.com` and `hsbc.co.uk` are not denied by the floor, `1password.com` is — so seeding a bank later breaks the test and forces the copy to be revisited alongside it.

The verdict is still AAA: NO, and that is fair. But the remaining blockers have moved: `window.confirm` as the consent gate, the absence of heading and grouping semantics, and the app-wide warm-gray token near 2:1 are all app-level decisions rather than this card's. What was K11's own to fix on this surface is fixed. The lesson distinct from the previous pass is simply that a fix is a change, and changes deserve the same adversarial reading as the code they replace — two of these three came from the edits that made the card more honest.

### 2026-08-17 — K11b: the producer's pure core, and proving the guard by deleting it (ADR-243, TASK-054)

The boundary has been built, hardened, and argued with for three passes; nothing has ever fed it. This slice starts the producer, and lands the half that can actually be proven: burst aggregation and the AX-role mapping. The event tap itself needs a signed build and a human granting Input Monitoring before it can be exercised at all, so building the testable half first is the same ordering the K10 gate imposed on the boundary — guarantees executable before raw flows.

The design question worth recording is how much the Rust side is allowed to decide. The answer is almost nothing. `input-capture.ts` owns every safety rule, Rust cannot call TypeScript, and a second opinion in a second language is a second thing to drift. So the producer aggregates keystrokes and hands them over, and the shell distils before anything is sent — with exactly one exception, which only ever withholds more: a field that is not positively identified as ordinary text never has its characters accumulated at all. The core gate would suppress them a moment later regardless. Doing it here anyway means a password never sits in this process's heap for the length of a burst waiting to be thrown away, and redundancy in the safe direction costs nothing. Structural absence beats prompt deletion.

That has a pleasing consequence for the leak the red-team found back at ADR-239. The count of characters in a secure field *is* a password length, and the fix then was to withhold it at distillation. Now no counter runs in the first place, so two secrets of very different lengths are byte-identical *before* they reach the distiller rather than after — the property holds one layer earlier.

One ordering detail is load-bearing enough to name. macOS reports a password box as role `AXTextField` with subrole `AXSecureTextField`. Check the role first and you classify a password field as ordinary text and capture it. So the secure check runs across both slots before the ordinary-text allowlist is consulted at all, and a test pins that exact combination rather than the roles in isolation. The content roles are an opt-in allowlist for the same structural reason core uses one: a role macOS adds later falls through to `undeterminable` and suppresses, instead of arriving as a capture-by-default.

The part I am most willing to stand behind is not the code but the check on it. The claim "a secure field accumulates nothing" is only worth as much as the test that would break if it stopped being true, so I deleted the guard and ran the suite. Four tests went red — a secure field accumulating characters, an undeterminable field doing the same, two different-length secrets becoming distinguishable, and text crossing from an ordinary field into a password field — and the eight non-safety tests stayed green, which is exactly the signature you want: the safety tests are load-bearing and they are not merely restating each other. Guard restored, 12/12, full desktop lib 176/176.

Still honest about the state: the `CGEventTap` and the AX focused-element read that would feed real strings into that mapping are not built. The lane remains inert.

### 2026-08-17 — K11c: the keystroke tap, and the id that was being thrown away (ADR-244, TASK-054)

The producer can now produce. A `CGEventTap` on key-down, an Accessibility read of the focused element's role per keystroke, and the hub wiring that lets `sensor_start("input")` actually run the thing.

The posture that matters most is the dullest line of code in the file: the tap is created `kCGEventTapOptionListenOnly` and the callback returns the event untouched. A capture sensor that could swallow or rewrite what the user typed would be a keylogger with a bug budget. The untouched return is kept even though listen-only makes it advisory, so that if someone later flips the option the behaviour is still a no-op rather than a silent change in what the sensor can do to the user's typing.

Two smaller decisions in the same spirit. macOS's own secure-input flag is checked before any Accessibility call, because ADR-239 already recorded `EnableSecureEventInput` as the real first line of defence and our field-role gate as the second — this just makes the ordering explicit, and it also covers the case where the flag is set by an app that is not frontmost. And for a field that is not positively ordinary text, the keystroke is never even decoded into a character: the accumulator would throw it away a moment later, but not decoding means a password's characters never exist as Rust values at all.

The role is read fresh on every key-down rather than cached, which is the more expensive choice and the right one. A cache would mean that tabbing from a note into a password box could, for the length of the cache window, attribute password characters to the previous field's role. The cost is bounded instead by pinning the Accessibility messaging timeout at 25ms, so an unresponsive app degrades to "undeterminable" — which suppresses — rather than stalling the tap; and a tap that macOS disables for slowness re-arms itself instead of dying quietly, because a capture lane that stopped without saying so is exactly the dishonest state this workstream keeps refusing to ship.

Wiring the emission is where a real gap turned up. Typed text belongs in the bounded raw ring buffer, never in the `Observation` — the observation is what fires the `sensor:capture` blink event and is the thing a future consumer is most likely to log wholesale. But `pump_channel` pushed the raw payload, took the id the ring assigned it, and dropped it on the floor. A drained observation announcing a captured burst therefore had no handle to its own payload: the lane would have looked perfectly alive and delivered nothing. The id is now carried through as `raw_id`, which incidentally closes the same latent hole for the clipboard provider, which has had it all along.

The last decision is about honesty in the UI. This lane needs two separate grants and they fail differently, so collapsing them into one "needs permission" would be a lie of omission. Without Input Monitoring macOS delivers no keystrokes at all, so the provider refuses to start rather than run deaf. With Input Monitoring but without Accessibility, keys arrive and no field can be identified — everything fails closed to undeterminable and nothing is ever stored, which is usable but useless. `sensor_list` now says which one is missing, in those terms.
...
I didn't fix that when I found it, because fixing it meant putting a text field on the primitive that seven capture lanes share, six of which have ADRs promising they will never carry message text, page content, or window contents. That is somebody's privacy decision, not an implementation detail I get to settle at 2am. So it sat as an open bug with the honest note that either the copy or the capture had to move.

Given the choice today, the user took the shared field — over my recommendation of a separate input-only path that would have left the other six structurally unable to express content. Their call, and it is recorded as their call, with the option I preferred written down next to it so a future reversal has something to reverse to. What I did do inside that choice was make the promise enforceable rather than aspirational: their envelope types still admit no text, and a guard now asserts all six leave the field undefined. I broke it deliberately to check — made the chat lane populate `content`, watched the guard go red naming the leaked string, put it back.

Then shipping the storage showed me two things that storing nothing had been hiding.

The first: the card-number regex had been eating the space after the number this whole time. `"pay with 4111111111111111 today"` came out as `"pay with [redacted:card]today"`. It removes more than intended, never less, so it was never a safety problem — but the moment this text is something you read back, quietly deleting a character out of your own sentence is not a thing a redaction should do. It survived because all three redaction tests only checked that the placeholder appeared, never what sat around it. One of them had the corruption right there in its own fixture and asserted past it.

The second is the one I'd have preferred to notice on purpose. Retrieval fusion, retrieval-eval and the router's memory search all filter learning-machinery rows out of what reaches a prompt. That filter was written for relevance. As of this commit it is the thing standing between your typed prose and a model's context window, because those rows have a body for the first time. It was already correct; nothing tied it to this. It's tied now.
...
The interesting part is a gate that looks redundant and isn't. The typed text doesn't ride in the Observation — ADR-014 keeps raw capture out of anything that crosses the sensor gate, so it sits in the local ring and the drain has to ask for it by id. The obvious implementation asks whenever there's an id to ask with. I made it check the field role first, which means for a password field the text is never even *requested*. Three layers now refuse the same thing independently: the Rust accumulator won't collect the characters, the drain won't fetch them, the server won't store them. Any one of those is sufficient today. The point is that none of them is load-bearing alone.

I wrote the first test against the drain module directly and it blew up on `import.meta.env` — the tRPC client dragged in Supabase config just to reach a pure function. That failure was the useful part. A safety property you can only reach through React and a network client is a property that gets tested through mocks, and mocks are where "the gate is still there" quietly stops being checked. So the decision layer is now its own module with no imports at all, and the planner's return type has no `text` field on it — it can't emit content even in principle; only a caller holding a non-null ring handle can fill that in. Broke the gate deliberately to confirm: swapping it for a bare id check turned exactly the two suppressed-role tests red.

Two things I got wrong along the way, both mine. I ran `tsc -b --noEmit false` in the web app to typecheck it, which is not what that package's typecheck script does, and it emitted 154 `.js` files into the source tree. The vocabulary gate caught it — it started scanning `WhatsAppPage.js`, a file that has no business existing. Deleted, after checking every one had a `.ts`/`.tsx` sibling so I wasn't removing anything real.

The other: `pnpm verify` came back 82/83 with a socket-cancel test in `jobpilot-culture-research` timing out at 120s — having run for 123. Nothing I touched goes near agent-orchestration cancellation, but "not mine" is a claim that needs evidence, so I ran it three times in isolation: 57/57 each time. It's load. Which makes it the same shape as the bug the other session just fixed in the supervisor yesterday — a wall-clock deadline that in practice measures how busy the machine is rather than the thing it claims to measure. Filed as evidence rather than shrugged off, because "just re-run it" is a mitigation, not a fix, and this is the second one this week.

The lane is now connected end to end in code. It still has not captured a single real keystroke, and it won't until there's a signed build with both grants on it.
...
## 2026-08-18 — three shipped Modules a reader couldn't find
...
The user's answer to the seven strains was short: do 1, 3, 4, 5, 6, 7; on 2, drop the claim rather than change the code; and "why do we even require built in module? correct it."

The router split was the one I did not want to hand to an agent. A 22k-line file moved by hand is a 22k-line diff nobody can review. So it became a script: find the 35 `name: t.router({` keys at indent two, cut each into `routers/<name>.ts`, export every top-level declaration of the preamble from `router-shared.ts`, and compute each router's import list from the identifiers it actually uses. Four defects surfaced in order, each a lesson in what "mechanical" hides: keys with digits (`d2c`), comments inside import braces swallowing the neighbouring binding, doc comments that belong to the *next* router, and — the good one — `t` never counting as used because `t.router({` is the key line itself, so a global `t` of type `any` silently took over and turned the whole API type into `any` with zero compile errors and 119 implicit-any errors in tests. That one is why "typecheck passed" is not evidence until you know what the type *is*.

The organization guard fold is a tightening, and I recorded it as one: 286 handlers asserted the pilot org, 163 also asserted membership, and the middleware does both for any procedure whose input carries an `organizationId`. Three tests asserted the looser world and were updated, not deleted.

Two things the agents got wrong that the verify caught. The catalog inversion rewrote `modules/d2c/package.json` after the storage move had added `drizzle-orm` to it, so 52 API test files failed on a missing package; and WhatsApp kept a dependency line on the catalog with no import behind it, which turbo reported as a build cycle before running anything. Both were one-line fixes; both would have been invisible to a green-looking partial run.

`drizzle-kit generate` reproduced 0044 and 0045 exactly from `schema.ts`, so the missing snapshot was regenerated rather than hand-written, and the metadata test now pins head 0052 after the renumber below.

Landing it on main was its own lesson. Main had already split the router on 2026-09-03, on a branch this one never saw, and had built its own Module Builder and its own notch behaviour since. So the merge kept main's split and re-applied only the guard fold to its 39 routers; dropped the branch's authoring stack and notch fix as superseded; renumbered the migration to 0052; and rebuilt the six per-Module catalog entries from main's newer manifests rather than the branch's. Two sessions solving the same strain a week apart is the cost of a 93-commit gap.

The user's report that the New dialog still lacks Modules turned out to be the seed guard: it filled only an empty registry, so a registry seeded before a built-in was curated never learned about it. It now publishes what is missing.

Landing it took two more passes. The guard fold had asserted membership for Agents as well as people — `assertMembership` takes a userId, so every Agent-callable procedure was refused with "not a member" before the agent floor could say why — and it compared the organizationId before zod had lowercased it, so an upper-case uuid the handler would have taken was refused as non-pilot. Eight tests named the first, one named the second. Both are the same mistake: middleware runs earlier than the parse, and folding an assert outward moves it earlier too.

The user asked again why the New dialog is short. Measuring their own registry settled it: 13 entries, not 15. Relationship and DevPilot are refused by the publish scan as the lethal trifecta, and the union is real — Relationship reads private data on every Page, ingests mail and web, and can send. Exempting first-party built-ins at the publish route turned out to be dead code, because `verifyCommonsEntryContent` independently rejects any scan-failed entry on every read. That is the Capability Trust Model's own definition of a valid entry, so the change was reverted and the decision written up for the user rather than taken quietly.
## 2026-08-27 — the colleague was right: main could not build a working desktop app (TASK-078, ADR-256, AP-166)

A full fresh-machine reproduction — uninstall, wipe all app data (user portion backed up first), pull `main`, delete every build artifact, rebuild, launch — found three independent breakages stacked in the order a fresh machine meets them: the bundler (correctly) refusing the accounting/d2c merge's native addons in Resources; the TASK-077 updater demanding a CI-only signing secret from local builds; and the accounting migrations folder missing from the deploy whitelist, which booted a dead API behind a live-looking shell. All three fixed the repo's way — keyring-pattern signed Framework + fail-closed loader for better-sqlite3, canvas pruned as the optional dependency it is, updater artifacts skipped without the key, migrations shipped and guarded in `verifyInputs`. The rebuilt app now boots healthy from nothing: sidecar port reported, accounting migrations applied, webview mutations 200. CI never saw any of this because the full bundle job runs only on tags — the colleague was simply the first truly fresh build since ac6de169.

## 2026-08-27 — every issue from the fresh-machine run closed; main's gates green again (TASK-079, ADR-257, AP-167)

"Can you fix all these issues?" — yes: the model download was never broken (human-triggered by design, closed as not-a-bug); the dead-end updater is disabled by the user's explicit choice until distribution exists (BRIDGE_UPDATER=1 re-enables); the false desktop SEC-1 warning became a truthful per-deploy-shape notice (unit-tested red-first); the 16 unclassified procedures (13 accounting/d2c plus 3 jobpilot.onboarding nobody counted) are classified on store-residency evidence; and the retired-vocabulary gate is green through migration, not amnesty — Zazoo's egg became shell, the governance copy dropped its retired noun, TASK-078's own accidental "artifact" findings were caught and renamed, and only Tauri's unrenameable schema key got reviewed allowlist entries. Three BUGS.md entries resolved, including the 2026-08-17 one that warned every session about red that was not theirs.

## 2026-08-29 — every UI rule in one rulebook; the consolidation found the docs describing an app that no longer exists (TASK-080..084, ADR-258, AP-168)

"Can you layout an executive style clear rulebook" — the rules lived across `ui-architecture.md`, the imported `ui-conventions-cvn.md` (C-1..C-34), `design-system.md` and the one thing that actually enforces any of them, `ui-conformance.test.mjs`. Consolidating them meant checking each documented claim against the running code rather than against the other docs, and that is where the value was: three of the user's questions turned out to be documentation describing surfaces that do not exist. **Module Detail** was deleted 2026-08-10 on the user's own directive yet is still named as the destination for admin in four places — so admin (mount/unmount, scopes, versions) has had **no surface at all** since, and `ModuleGovernanceSection`'s "Edit in Module Detail" link redirects the user back to the page they were on. **`module.yaml`** is named in the always-loaded `CLAUDE.md` and in `TASKS.md`; no such file has ever existed, manifests are TypeScript. The **Governance Section** the user thought was missing is in fact built and correctly placed at all 14 call sites — but enforced at exactly one, and declared by exactly one Module of thirteen. Five doc/code contradictions were catalogued with an explicit precedence rule (a passing gate beats the latest directive beats the doc text), including the one the user found unaided: the rail-scope rule still listed Second Brain after ADR-224 removed its entry. On the rules themselves the user rejected the *genre* — backward-facing "X is not Y" rules and meta-rules asserting that examples are not definitions — so ADR-258 makes positive definition the required form and deletes the comparisons outright. Six new rules approved (AP-168) and queued as TASK-080..084 so none of this depends on remembering a chat; two BUGS entries filed. One reversal recorded loudly rather than folded in: **C-33 is overridden**, New opens an element page everywhere. Two things were narrowed against the literal directive and the reasoning written down: the add-row gate is rewritten rather than deleted, and enabled-with-warning stops short of the 13 column-menu items that have no capability behind them, because enabling those would ship a menu that looks live and fails at the server.

## 2026-08-30 — the UI rules stop being advisory: four enforcement layers, one implementation (ADR-260/261, AP-170/171)

"Bring in hooks, governance policies and code level, test level and other enforcement mechanisms for the UI." Built and verified: a **Claude Code PostToolUse hook** (fires as a `.tsx` is written), a **tracked `.githooks/pre-commit`** (fires on commit touching UI or the rulebook), **`pnpm check:ui-rules` wired into `pnpm verify`** (local + CI), and **two new gates in `ui-conformance.test.mjs`** (15 tests now). The first three run the SAME script — an agent-level check that drifts from the CI check is worse than none. The counted-rule checker covers eight rules as a **ratchet**: a count going up fails (you introduced one), and a count going down ALSO fails, demanding the fix be recorded, because an unrecorded fix regresses silently. It froze **395 pre-existing violations** — 334 hardcoded hexes, 29 sub-12px type sizes, 17 native dialogs, 10 `vh`-not-`dvh`, 2 `absolute top-full`, 2 bare `<select>`, 1 `overscroll-contain` — turning invisible debt into counted debt. Three doc gates hard-fail with no baseline: one canonical rulebook (the four retired canons may not regrow rules), no `module.yaml`, no rule directing a reader to the deleted Module Detail. Two findings came out of building it. The doc gate **immediately caught a live Module Detail reference in the rulebook** — which on inspection was a correct line saying the route *was deleted*, so the gate was tightened rather than the prose edited: a gate that fires on correct code is one people learn to ignore. And proving the new conformance gates red-first **found a defect in the gate itself** — the regex prefix-matched, so `<ModuleGovernanceSectionREMOVED` satisfied the check that a Governance Section exists; a tag boundary fixed it. That is the entire argument for CLAUDE.md's rule that a test proves nothing until seen failing unfixed. Documented in `docs/wiki/ui-enforcement.md`, including what is deliberately NOT enforced: the judgement rules (Hick's Law, "a collapsed panel shows its finding") stay reviewer-enforced and are tagged CANON rather than GATED, so the distinction is visible rather than assumed.

## 2026-08-30 — the rail gains right-click, drag-order and rename; Organization admin gets a surface for the first time since Module Detail was deleted (TASK-081, TASK-089, ADR-262)

"start implementing the tasks with multiple subagents." First wave landed. Right-clicking a Module in the rail now opens Hide/Rename, hidden Modules come back from rail **View options** in their previous position, and Modules drag-reorder — all persisted per Organization in `rail-module-presentation.ts` (10 unit tests, pure model, no React). And admin has a home again: `/organization/admin` behind the rail's Organization control, Modules and Members tabs — the gap TASK-080 recorded when it found ADR-180 had put admin inside Module Detail and ADR-224 had then deleted that page, leaving mount/unmount/scopes/versions with **no surface at all** since 2026-08-10.

Both tasks stay `in_progress`, and the reason is the same in both cases: half of each exit test is genuinely not met and marking them done would be a lie the ledger keeps. **A renamed Module's local Files folder does not follow the rename** — the rename is client-side presentation, the folder path is derived server-side, and closing that needs a per-Organization `displayNameOverride` on the installation record plus a folder migration, which is Tier C. **Unmount is explained rather than offered, and there is no invite/accept flow** — both are governed writes, and a button that looks live and fails at the server is precisely what the canon forbids. Named as ceilings in ADR-262, not smoothed over.

Two findings from merging rather than from building. The conformance §5 search-slot regex was **firing on compliant code**: `searchPlaceholder="Search Modules…"` is a page handing `<DataViews>` its placeholder, which is the correct path, and the gate called it a divergence. Anchored, and `JobPilotPage.tsx` came off `KNOWN_DIVERGENCES` as a real burn-down — the second time in two days that building enforcement found the enforcement wrong. And the merge caught a `max-h-[min(70vh,420px)]` in `Layout.tsx` — `NEW VIOLATION — vh-not-dvh (1, was 0)` — because the agents' worktrees branched off `main` and so did not carry the gate they were being measured by. That is a dispatch defect owned here, not an agent defect: agent worktrees must branch off the branch that carries the gate. Central gate green after the fix: 395 violations held at baseline, 3 doc gates pass, 25 tests pass.

## 2026-08-30 — governance stops being read-only and the Chat composer stops being half-disabled (TASK-088, TASK-082, ADR-263)

Wave 2 of the parallel run landed. **Governance is editable.** `module.governance.userEdited` has been parsed by the manifest reader and rendered by the UI since ADR-248 with nothing in the repo able to set it — the flag was written for an overlay nobody built. It exists now: an engine-held overlay keyed by Organization + Module, on the Local Plane state store that learning-consent already rides (no new table, no migration), resolved into the same `governanceVerdict` the enforcement path always used. Accounting's one real governed call site now answers to the **resolved** policy rather than the manifest, and the Section's "Edit in Module Detail" link — which returned the user to the page they were already on — is an in-place editor. That closes the BUGS 2026-08-29 dead end.

The overlay **replaces** the declared policy rather than merging with it, and that is a reading of the TASK row rather than a transcription of it: merge semantics mean the user deletes a manifest deny, saves, and watches it come back. `reset` is the only road back to the default. Written down in ADR-263 as chosen, not read, because the implementing agent's worktree branched off `main` and could not see the row it was building against.

**The paperclip uploads and the mic works everywhere.** Uploads route through `modules.addFile` — the one existing Module File path, not a second uploader — and transcription moved off the Tauri-only Rust command into `chat.voice.transcribe`, same model and same 24 MB ceiling, audio forwarded once and never persisted. Both controls now report their refusals *ahead of time* through `chat.model.status.composer`, so a control that cannot run is visible, disabled, and carries the server's own reason instead of dying after the user has already recorded. The Avatar shortcut still auto-sends and the panel mic still fills the draft for review — that distinction is pinned by test now rather than by convention.

Both rows stay `in_progress`, and for the same reason as yesterday's pair: **neither has been watched in a browser**, and the exit tests say a human sees it. One clause is not merely unverified but currently unrunnable — "a second Organization still sees the shipped default" cannot execute while every `moduleGovernance` procedure calls `assertPilotOrganization`; the test asserts that pin rather than routing around it and proves the Organization key one layer down at the store. It unblocks with TASK-089.

Merging found more than the agents did. Reviewing TASK-088's own suite against its exit test showed two clauses unproven, so **reset** and **Organization keying** got tests written centrally — the reset test proven to bite by mutating `reset` into a no-op. And `git stash` cost real work: it is shared across every worktree of one repository, two agents stashed, one popped and took the other's entry and dropped it. It survived only because the popper noticed and recovered it from the dangling commit. That also left one worktree carrying a sibling's changes, so TASK-082 had to be separated from TASK-088 by diffing the two agent worktrees file-by-file rather than trusting either agent's own file list — four governance files came back byte-identical, which is what made the separation safe. Standing rule now demonstrated rather than asserted: **no agent uses `git stash` in this repo.**

Three red tests were found on `main` that nobody's change caused — a missing `avatar/zazoo/parts` module, an unawaited nested test in `modules.test.ts:386`, and an `input-capture` assertion that only passes in UTC-ish timezones — filed in BUGS.md so the next session does not re-litigate whose failure they are. Central verification after both merges: typecheck 57/57, web 189/190 (the one is the pre-existing avatar failure), API subset 37/37, `check:ui-rules` OK at the 395 baseline with 3 doc gates passing.

## 2026-08-30 — the column menu stops lying and tables gain multi-select; and a "pre-existing failure" turns out to be my own bad command (TASK-084, TASK-086, ADR-264)

Wave 3. **13 of `StandardColumnMenu`'s 17 items had been permanently disabled** — not for risk, but because no governed schema-mutation capability existed behind them. The capability exists now, as a per-Organization column overlay resolved over the shipped `TableSpec`, riding the same Local-Plane seam ADR-263's governance overlay established. Nine items are live and warn before acting through a real dialog carrying a dependency preview; eight stay visible and disabled, each with a stated reason. Three of those eight are the honest ceiling: **the overlay describes columns the Database already has, so an added column has nowhere to store its values** — enabling Add/Duplicate would ship exactly the menu-that-fails-at-the-server the task forbade. And fx binds to Accounting's existing validator rather than growing a second engine; the web app never parses or evaluates, asserted by test.

**Every table now has multi-select**, long-press on touch, and a shared selection bar. The Constraint that a bulk delete be governed identically to three single deletes is met *structurally* rather than asserted: `onDeleteRows(ids)` is the only delete prop and `archiveRelationshipRecord` the only server helper, so the caret, the cell menu and the bar all route through one path. That was proven the right way round — giving the bulk loop its own inline `pipeline.propose` made the ledger test fail on a differing `action`, `skill` and `userDecision`.

Two pieces of engineering worth keeping. The gesture logic went into a **pure reducer** because the web suite has no jsdom, and a swallow written inside an `onClick` closure is behaviour nothing can fail on. Extracting it made the synthetic-click trap provable — and the actual failure mode was worse than the one the task row anticipated: without the swallow the finger-lift toggled the row straight back **off**, emptying selection so the next tap opened a Record. Separately, the dependency preview **distinguishes "inspected, none found" from "never looked"**: Automations and Skills carry opaque `inputs` with no column-level reference in the contract, so a step naming a column cannot be found, and the preview says so in danger colour instead of showing a reassuring blank.

Both rows stay `in_progress`. TASK-086's touch clauses **cannot** be verified here — the reducer is proven, but nothing proves the DOM fires those events at the right moments, and no synthetic dispatch was substituted because it would only prove that events can be dispatched. TASK-084's "every dependent cell recompute" is half-runnable: with no imported facts every metric value is `null`, so the new Value column reads blank in every row. No figure was fabricated to make the clause demonstrable — that is "unknown is first-class" costing a visibly empty column.
...
## 2026-09-07 — TASK-112: Add column asks what it is, and Task Manager gets the capability it was missing

Add column used to mint `New column` of type text with no questions asked. It now opens one dialog — name, type over the twenty kinds `COLUMN_KINDS` accepts, and an add/remove option list for `select`/`multiselect`/`status` — shared by both entry points (the toolbar overflow and the column menu's Add left/right), creating nothing until confirm. An empty name is refused rather than defaulted: confirm stays visible and disabled with the reason on it (ADR-247). `options` and `position` now actually reach the add op; both were accepted server-side and silently dropped, which is why "Add column left" had been landing at the end. Retype carries options the same way, stored beside the overlay as `ColumnOverlayOptions` and resolved over the shipped column. The reported Task Manager gap had a plainer cause than expected: `task-manager.tasks` was in neither `SCHEMA_MUTABLE_SPECS` nor a manifest, so rename, retype, lock and delete were dead there too, not just add — the spec joins the list, `canAddColumn: !shipped` gives exactly the asked-for answer, and `TaskManagerPage` now reads the resolved spec from `tableSchema.get` with its constant demoted to a pre-load fallback. Checked before shipping: the overlay is keyed by column id, no server code matches a Task column by label, and the one writer validates `status` server-side, so a retype produces a refusal rather than a corrupt Task. NOT LANDED: the three D2C pages need only client wiring and the ten other hand-written pages need a server spec first — neither was reported, so neither was done quietly.

## 2026-09-08 — TASK-114: the companion sleeps behind the notch and comes out on hover

User directive, verbatim: *"I want the avatar always present behind my notch. But it should listen to me or appear outside notch only when I hover there or when I trigger it with shortcut"*. The 2026-09-04 rule concealed the window entirely at rest and made ⌘⇧Space the only way in, so the notch home showed nothing at launch. Three states replace two: `interactive` (summoned — shown and taking the mouse), `resting` (shown, CLICK-THROUGH) and `concealed`. Resting matters because the docked window covers a strip of the menu bar; a permanently present window that swallowed clicks there would cost the user their own menus, so `overlay_present` takes an `interactive` flag and sets `ignore_cursor_events` from it. At rest the window shrinks to the cutout's width and 16px more, and Zazoo is parked so his CROWN — not his bounding box, whose top quarter the rig leaves empty — sits at the cutout's lower edge; the window itself clips the rest, so what shows is the top of a sleeping head under the notch. Hover restores the full wake performance, and hover summons ONLY in the notch home: the free-floating avatar has no notch to hover, which is what the 2026-09-04 complaint was actually about. Verified in `overlay.html?lab=1` — the first peek rendered as a plain black strip, which is how the empty rig margin was found. Evidence: two new cases in `apps/web/test/notch-home.test.mjs`, each seen failing against unchanged source; web 314/314; `@bridge/web typecheck` and `cargo check` clean; ui-rules OK.

## 2026-09-15 — TASK-095: walkthroughs go the distance, and the companion draws while it talks

User directive, verbatim: *"Build draw on screen with narration capability alongside Guided walkthroughs that detect your click and advance, up to 15 steps, goal kept across a "continue" pause, 89 per-app teaching skills"*, following a changelog comparison against Hey Clicky v1.0.16–v1.0.48.

Found while checking rather than asserting: the 15-step walkthrough budget already existed, but the 150 s hands-mode wall clock also bounded guide runs that wait up to 90 s per user click, so a walkthrough could honestly end at step two. Guide mode now has a 20-minute clock. Landed: `wait_for_user` closed planner action and a click timeout that both `pause` with goal + completed steps kept in `ActState`; `act_start{resume}` / `act_paused` / typed "continue"; 97 per-app teaching packs as embedded data with site-in-title matching; explain asks draw up to four marks and clear when the spoken answer ends. Rust 203+1 ignored (two mutation checks RED-then-green), web 317/317, gates green. Decision: `docs/raw/decisions-log.md` 2026-09-15 "Teaching packs are data, a pause is a state, and drawings live as long as the voice".


## 2026-09-16 — Everything editable (TASK-114)

User directive, verbatim: *"complete task 114"*, closing the 2026-09-11 directive *"Literally I want everything can be edited, editable. Not being able to edit should be an rare exception"*.

Read-only was never decided — manifests are immutable and nothing held a user's edits, so every intelligence surface rendered prose. Landed the governance overlay's own shape (ADR-263) for Agents, Skills, Automations and Integrations: Local-Plane, per Organization + Module, resolved once in `modules.list` so no surface still shows the shipped name of something renamed. The editability rules are one table in `@bridge/core` that the server refuses writes from and the page disables controls from — the client ships no copy, so a lock cannot exist on one side only, and a lock with no reason fails a test. Skill/Automation/Integration each open `/module/:name/intelligence/:kind/:entryId`; Agents keep their room page, now editable. Found while verifying live and fixed before landing: a cadence on an unscheduled Automation was accepted as a no-op, and object-valued locked fields rendered `[object Object]`. Decision: `docs/raw/decisions-log.md` 2026-09-16 "Editing is the default; a locked field is data that carries its own reason" (ADR-283, AP-188).

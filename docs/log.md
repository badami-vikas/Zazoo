...
## 2026-07-07 — Foreign-capability importer: Pi package / MCP server / Activepieces piece / OSS integration -> CapabilityManifest (ADR-026)
New `platform/packages/core/src/capability/foreign-import.ts` (types: `ForeignCapabilityImport`, source union `pi-package|mcp-server|activepieces-piece|oss-integration`, exact-pinned `ForeignVersionPin`, `ForeignPermissionDeclaration`, `SandboxPolicy`) + `capability/importer.ts`'s `translateForeignCapability()` translating into the existing `CapabilityManifest` (never forked). Per-source mapping: pi-package extension->tool+connector/skill+prompt->skill/theme->view; mcp-server tools->connectors + resources->read permissions (capabilityType tool); activepieces-piece actions+triggers->one skill capability with a connector per step (always sandbox-required); oss-integration is a generic passthrough with permissions read ONLY from caller-supplied `permissionDeclarations`, never auto-inferred from the descriptor. Every import forced to `origin: "community"`. Sandbox-required guard returns a typed `{ok:false, error: ForeignImportSandboxRequiredError}` result (not a throw) when an executable import (activepieces always, pi-package extensions) lacks a real `sandboxPolicy` — every other malformed-input case still throws `ForeignImportValidationError`. New `test/importer.test.ts` (18 tests, `test_fixture_` naming). `platform/packages/core/src/index.ts` gained two additive export blocks. Note: the task's cited source doc `docs/raw/execution-plan-2026-07.md` and a pre-existing `foreign-import.ts` did not actually exist in this worktree — built from first principles against `capability/types.ts` + `package/types.ts`'s conventions instead; flagged in BUGS.md for reconciliation once/if that doc lands. Gates: `pnpm --filter @bridge/core build` clean, `pnpm --filter @bridge/core test` 181/181 passing. ADR-026 in decisions-log.

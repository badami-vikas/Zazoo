...
- **Status**: gate ✅. **plane gate ✅ — structural in core** (`Actor.plane local|cloud`; `planeGate()` Layer 0.5: local→egress DENY, cloud ceiling-clamped to public; `external:fetch` resource added; 38/38 tests incl. 3 gate invariants; enforced over HTTP). local store 🟡 — **first local adapter LANDED (2026-06-20)**: **`LocalMediaStore`** port (core) for camera blobs, two adapters one port (platform `PgliteMediaStore` bytea + browser IndexedDB), bound in API wiring next to ledger. Governance ledger/people still cloud Supabase (next slice). E2EE-at-rest = Phase 6.

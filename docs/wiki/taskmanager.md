...
- **Automations built, 2026-08-08 (ADR-198)**: `planning-playbook` (NEW, human-triggered — these Skills answer a question someone asked, so the trigger is a person; the Automation exists to give it an attributable Run) and `proactive-scan-cadence` (declared since TM0 with no runtime id). Procedures `taskManager.runPlanningPlaybook` / `runOpportunityScan`; both attributable Internal Strategist Runs halting at `pending_review`. Manifest `1.1.0`→`1.2.0`. **`standup-brief` deliberately NOT bound**: Chief of Staff-owned, and CoS has no governed runtime Agent identity — binding it to Internal Strategist would make it run by discarding ADR-107's ownership split. **Approval materializes nothing yet**: accepting a decomposition creates no child Tasks, accepting a scan finding creates no candidate Task. Nine other declared Automations still unbound.

...
- **F4 Consent+Visibility** (P1, deepest): F4a two-tier Person view (`people_canonical` public read-only vs `people` private warmth/notes, E2EE-bound; overrides shown both); F4b `people.visibility` private/team/workspace (default = `workspace_settings`); ~~F4c both-party-consent intro state machine~~ **F4c REVERSED 2026-07-06 (user call, ADR-026): both-party consent NOT required. Data owner controls own data. Intro = normal governed proposal (draft-then-approve by SENDER only), no awaiting_both state.** NO silent enrichment/auto-send stands.
...
- New files: `data/governance.ts` · `pages/ApprovalsPage.tsx` · `components/ExecutionLedger.tsx` · ~~`components/PersonTiers.tsx`~~ (DELETED 2026-07-06 — orphaned; ItemDetail's inline two-tier view superseded it; its unique F4b visibility setter moved into ItemDetail's Relationship section).

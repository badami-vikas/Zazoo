...
- Capability archetypes (roadmap-v2 Phase 4, TASK-033/ADR-163/ADR-172): `GET/POST /v1/archetypes`. Generalized preference patterns only (domain/action/attribute + banded support; deterministic name = many-workspace identity). Same privacy gate + Ed25519 signing. Client drops unverified entries. Re-publish of a known name AGGREGATES (ADR-172): contributions += 1 (anonymous, inside signed content), max support band, tag union — freshly signed superseding revision. Contribution = per-archetype Human action behind `BRIDGE_COMMONS_ARCHETYPES`; consumption = seeds PROPOSED suggestions most-corroborated-first on the learning lineage (never auto-applied).

...
- **OPEN — No real Brokerage entity or per-brokerage credential/connect flow in DealPilot.** User asked mid-playtest where to add a brokerage and log into it — the answer is: nowhere yet. "Brokerages" is only a `List` filter label applied to generic sourced listings (`data/dealpilot.ts`'s `LISTINGS`), not a real entity (name, portal URL, contact, credentials). The only connect/login pattern in the app is the generic `ConnectAppFlow` wizard (Apps — API-first, falls back to guided scrape/bot/browser). If a real Brokerage entity is wanted, it would need: a `Brokerage` type + store (mirroring `ToolList`'s shape), a per-brokerage `ConnectAppFlow` instance for credentials, and a link from `DealListing.source` to a specific brokerage record instead of a free-text union.

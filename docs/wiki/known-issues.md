...
- **RESOLVED (2026-07-04) — Brokerage entity built under Intelligence → Apps.** New `data/brokerages.ts` (`Brokerage { id, name, portalUrl, status }`, reactive store) surfaces as a real "Brokerages" list under the Apps tab (`IntelligencePage.tsx`) — "Add Brokerage" creates one, clicking a disconnected brokerage opens the same governed `ConnectAppFlow` wizard every app uses (no public API for a brokerage portal, so it goes straight to the scrape/bot/browser waterfall), and connecting sets its status. Ships with the same 2 seed brokerages (BizBuySell, BusinessBroker.net) DealPilot's listings already cite as sources. **Still not done**: linking `DealListing.source` to a specific `Brokerage.id` (source remains a free-text union) — that's the next step if DealPilot's sourcing should actually pull from a selected brokerage rather than just categorizing by name.

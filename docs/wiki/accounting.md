...
- Local Plane, but not PGlite like the rest of Bridge: `platform/commons/accounting` opens its own `better-sqlite3` file inside the host-granted `ModuleContext.dataDir` — the `module-host` contract was built for exactly this, so no kernel change was needed. Two embedded engines coexist and never meet.
- `router.ts`'s `accounting` namespace runs real queries against the imported Drizzle schema (`platform/commons/accounting/src/schema.ts`) — no fabricated rows, honest `0`-row empty states.

...
- **ADR-175** (2026-08-07, REVERSES AP-101/TASK-006's manual-deploy gate) — `render.yaml` `autoDeploy: true` on both hosted services (user directive); a new `supabase-migrate` CI job applies pending Drizzle migrations to the hosted Supabase project on push to main, gated on CI passing first. Residual, stated plainly: Render's git-trigger fires independent of GitHub Actions' outcome, so this is a practical timing gap (migration job seconds, Docker build minutes), not a transactional guarantee (AP-102).

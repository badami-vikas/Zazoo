...
- D2 (TASK-071): three governed Skills — `devpilot.reviewPr`, `devpilot.suggestPractice` (over a tracked PR's live diff), `devpilot.analyzeIssue` (over a tracked Issue's live body) — draft through the normal proposal/Approvals pipeline. Every draft needs a Human approve/edit/veto; nothing is ever posted back to GitHub. No model configured → an honest "not drafted" scaffold, never a guess.

...
- **Capability Build Chain** — the fixed order in which a machine-proposed Capability comes into being (ADR-181): Internal Strategist recommends it, Capability Builder drafts a Manifest, Governance computes the risk verdict, a Human approves. Each junction belongs to exactly one actor and none can be skipped; the chain's last machine state is "reviewed", never "approved".

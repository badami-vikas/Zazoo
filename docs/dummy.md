...
its real data source exists, and an empty state would hide the thing being reviewed): append a row below
with the file, the reason, the real element it stands in for, and the removal condition; mark the row
done when the real source lands. That is the whole protocol (AP-182 retired the four-step ritual and the
`check:no-dummy-runtime` grep). Test fixtures may use a `test_fixture_` prefix for grep-ability; nothing
enforces it.

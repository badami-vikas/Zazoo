...
resolved_since_last_report:
  - Full live Skill catalog is fail-closed with no allowlist; every call is manifested, structurally Agent-floor-denied, or the Human-only KERNEL_PASSTHROUGH_SKILL reservation.
  - Goal/Task/SkillManifest/ChildAgentRun are restart-durable through real Drizzle stores + post-release migration 0014.
  - Source branch was reconciled, then integrated and security-hardened on main with zero remaining conflicts.
---

## Integrated outcome — 2026-07-17

TASK-007 is integrated on `main`. This section supersedes historical branch-status notes below.

- Five foundational Agents have persistent least-privilege identities and assignable boundaries.
- Workspace Skill manifests require an active matching Goal/Task, its assigned active Agent, sufficient authority, matching Plane, and contained data scope. Missing/cross-workspace state fails closed.
- `stageMutation` is Human-only kernel passthrough, not an Agent bypass. Learning, Outreach, capture, Helpdesk, DealPilot sourcing, and Google sourcing use dedicated governed Skills.
- Server-owned child Agent Runs intersect parent authority, Skills, data, review, taint, depth, deadline, and budget. Public creation is absent. Usage/lifecycle updates are guarded; authenticated members can inspect/stop; transitions are append-only audited.
- Migration `0014_task007_goal_task_skill_manifest_child_run` follows released `0013`, so fresh and already-migrated databases both receive the schema, constraints, tenant foreign keys, and FORCE-RLS policies.
- Security review closed membership-before-provisioning gaps, lifecycle audit races, blocked Task persistence, and migration high-water ordering.

Release evidence: core 421/421, DB 106/106, API 161/161, Google 35/35, web 43/43; monorepo typecheck/build; 38 changed TypeScript files lint clean; no-dummy-runtime, migration no-drift, whitespace, and conflict-marker gates green. Full-repo lint is blocked only by unrelated untracked `ZazooAvatar.tsx` referencing an unavailable lint rule.
...
At that historical point, commits `5723337` and `59faf0b` were pushed to the TASK-007 branch for coordinator review. They were later integrated and hardened on `main` as recorded in the current outcome above.
...
2. **Restart-durable Goal/Task/SkillManifest/ChildAgentRun persistence** — DONE. Real Drizzle-backed stores now exist and are wired into `buildPersistentPorts` (bound to Postgres when `DATABASE_URL` is set):
   ...
   - **Migration**: final integration regenerated the schema as **`0014_task007_goal_task_skill_manifest_child_run.sql`**, strictly after released `0013`; the snapshot chain and journal high-water ordering now support both fresh installs and upgrades.

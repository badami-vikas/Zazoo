...
function dormantSignals(): Signal[] {
  ...
  return cands.slice(0, 3).map((p, i) => ({
    ...
    actions: [
      ...
      {
        label: 'Add to a check-in ritual', channel: 'Internal', action: 'Add to check-in ritual',
        proposed: `Add ${p.name} to the "Monthly inner-ring check-in" ritual so this tie doesn't slip again.`,
      },
    ],
    detectedAt: 'Today',
  }));

...
export const tools: Tool[] = [
  ...
  },
  {
    id: 'calendar',
    name: 'Calendar',
    description: 'Your Google Calendar in Bridge — see every event, and create / move / delete events that round-trip to Google through the governed pipeline.',
    category: 'Coordination', status: 'Live', list: 'My Tools', icon: CalendarClock, color: '#4D7EA8',
    overview: 'A native, pinnable Calendar — the time-axis view over your graph. Today its source is Google Calendar (read + write); conference, ritual, and initiative events are future adapters that emit the same event shape, so the surface never changes. Every create / edit / delete is drafted, approved, and synced to Google through the Universal Action Pipeline — append-only audited in Approvals, never a silent write. Rendering is fully in-house (date-fns + Bridge tokens) so it is free and endlessly modifiable.',
    capabilities: ['Month / week / day / agenda views', 'Create, modify & delete events', 'Two-way Google Calendar sync (gated egress)', 'Draft-then-approve — every write audited'],
  ...
  },
  {
    id: 'dealpilot',
    name: 'DealPilot',
    description: 'Thesis-scored triage over sourced business-for-sale listings — sourcing, dedupe, and fit-scoring, composed from Bridge\'s shared engine.',
    category: 'Sourcing', status: 'Live', list: 'My Tools', icon: Briefcase, color: '#4D7EA8',
    overview: 'An external tool: a kanban triage surface over listings sourced from BizBuySell (the user\'s own Gmail saved-search alerts, via the governed Google integration) and BusinessBroker.net. Each candidate is deduped against existing deals and scored against your thesis (industry / geo / SDE / revenue) into green / yellow / red. DealPilot composes company-sourcing, people-sourcing, and recorder rather than owning sourcing/dedupe/facts logic itself.',
    capabilities: ['ThesisFit triage (green/yellow/red)', 'BizBuySell alert-email sourcing (Gmail compose)', 'Dedupe against existing deals', 'Composes shared sourcing/dedupe/facts engine'],
    watches: ['BizBuySell saved-search alerts', 'Thesis profile (industry/geo/SDE/revenue)'],
    lastUsed: 'Live',
    route: '/dealpilot',
    native: true,
    source_repo: '@bridge/dealpilot (composes company-sourcing, people-sourcing, recorder)',
  },
  {
    id: 'reconnect',
    name: 'Reconnect',
    ...
    capabilities: ['Rank ties by the trust↔warmth gap', 'Draft a personal reconnect note', 'Hold 20 minutes on the calendar', 'Add a person to a check-in ritual'],
  ...
  },
  {
    ...
    capabilities: ['Detect inbound with no reply', 'Draft a reply in your voice', 'Snooze a thread', 'Route to the right ritual'],
  ...
  },
  {
    ...
    overview: 'A ritual template that rotates through your inner-ring ties on a monthly cadence and drafts a light touch for each.',
  ...
  },
  {
    id: 'sys-api',
    name: 'API Connector',
    description: 'REST / GraphQL bridge for rituals and external systems.',
    category: 'Integration', status: 'Available', list: 'Systems', icon: Cable, color: '#2E4057',
    overview: 'Lets a ritual call an external system as a step, and lets external systems trigger a ritual — always through the governed action pipeline.',
    capabilities: ['Call REST / GraphQL from a ritual', 'Map fields to the canonical model', 'Sign & log every call to the ledger', 'Inbound webhook triggers'],
    watches: ['Ritual steps', 'Governance ledger'],
    lastUsed: '—',
  },
];

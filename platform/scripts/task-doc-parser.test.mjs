...
test('on the committed ledger, no reported duplicate is the value the parser kept', () => {
  ...
  // The ledger was clean of stray blocks by 2026-09-11; the parser contract
  // itself is pinned on synthetic fixtures above, so a clean document passes
  // vacuously here instead of failing the gate for being tidy.

...
// + kanban tracker by `applications.status`). Columns-as-data on @bridge/tables — the same engine
// DealPilot's triage feed and the prototype's People/Communities tables use — so JobPilot gets
// kanban/list/gallery views for free instead of a bespoke tracker UI.
...
export function jobsTrackerView(): ViewConfig {
  return { ...defaultViewConfig("jobpilot.jobs.tracker", "kanban"), groupBy: "stage" };
}

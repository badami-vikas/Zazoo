...
export class InMemoryDealPilotStore implements DealPilotStore {
  ...
  async createThesis(input: CreateThesisInput): Promise<ThesisRecord> {
    ...
    this.theses.set(record.id, record);
  ...
  async link(input: CreateRelationInput): Promise<DealPilotRelation> {
    ...
    const key = `${input.workspaceId}:${input.kind}:${input.fromId}:${input.toId}`;
    const existing = this.relationRows.get(key);
    if (existing) return { ...existing, evidenceRefs: [...existing.evidenceRefs] };
...
export async function proposeThesisSourceDiscovery(
  ...
  const sources = await store.list("sources", workspaceId, { limit: 200, offset: 0 });
  return {
    kind: "thesis_source_discovery",
    workspaceId,
    thesisId,
    relations: sources.items
      .filter((record): record is SourceRecord => record.kind === "source" && record.rightsState === "attested")
      .map((source) => ({
        sourceId: source.id,
        thesisId,
        confidence: 0,

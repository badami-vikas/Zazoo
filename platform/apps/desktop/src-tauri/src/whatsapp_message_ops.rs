...
//!
//! WIRING (one line, deliberately not applied by this change): the allowlist in
//! `whatsapp_webview::script_for_op` must delegate its fallthrough arm here —
//!
//!     _ => crate::whatsapp_message_ops::script_for_message_op(op, arg),
//!
//! Until that line lands the shell refuses `list_chats` and `list_messages`
//! with `WHATSAPP_OP_REFUSED`, which the web surface reports honestly rather
//! than presenting an empty store as a synced one.
//!

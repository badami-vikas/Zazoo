...
import { useRecords, updateRecord } from '../data/records';
...
export function RecordDetail() {
  ...
  const stored = records.find(x => x.id === iid);
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden" style={{ backgroundColor: 'var(--color-background)' }}>
      ...
      <div className="border-b px-6 py-4 bg-white" style={{ borderColor: 'var(--color-border)' }}>
        <Breadcrumb className="mb-3" segments={[{ label: 'Work', to: '/work' }, { label: name || 'Record' }]} />
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex-1 min-w-0">
            <Editable text={name} onSave={v => stored && updateRecord(stored.id, { name: v || 'Untitled record' })} className="text-2xl font-bold" />
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem asChild>
                <Link to={`/record/${encodeURIComponent(iid)}/control-panel`}>
        ...
        <div className="flex gap-1 border-b -mb-4" style={{ borderColor: 'var(--color-border)' }}>
          {RECORD_PAGES.map(page => (
            <button key={page.slug} onClick={() => selectPage(page)} className="pb-3 px-4 text-sm font-medium transition-colors relative" style={{ color: activeTab === page.label ? 'var(--color-steel)' : 'var(--color-warm-gray)' }}>
              {page.label}
              {activeTab === page.label && <motion.div layoutId="recordTabIndicator" className="absolute bottom-0 left-0 right-0 h-[2px]" style={{ backgroundColor: 'var(--color-steel)', boxShadow: '0 0 8px rgb(from var(--color-steel) r g b / 0.3)' }} />}
            </button>
          ))}
        </div>
      </div>
        ...
        <div className="max-w-4xl mx-auto">
          {activeTab === 'Overview' && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
              {/* Goal — what done looks like (front and center, from the record store) */}
              <div className="border rounded-xl p-6 bg-white" style={{ borderColor: 'var(--color-border)' }}>
                ...
                <Editable multiline text={stored?.goal || ''} placeholder="What does done look like? Click to write…" onSave={v => stored && updateRecord(stored.id, { goal: v })} className="text-sm leading-relaxed block w-full" />
              </div>
              ...
              {/* Boundaries — what this record explicitly will NOT do */}
              <div className="border rounded-xl p-6 bg-white" style={{ borderColor: 'var(--color-border)' }}>
                ...
                <p className="text-xs mb-2" style={{ color: 'var(--color-warm-gray)' }}>What this record will NOT do — keeps scope honest.</p>
          ...
          {activeTab === 'Files' && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
              <ModuleFilesSection moduleName={`record-${iid}`} />
            </motion.div>
          )}
        </div>
      </div>

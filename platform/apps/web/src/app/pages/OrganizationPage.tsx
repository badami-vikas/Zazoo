...
import { compileBlueprint, type CompiledOrganization, type OrganizationBlueprint } from "@bridge/core";
...
async function fetchRowsFor(nodeType: string): Promise<DataRow[] | null> {
  switch (nodeType) {
    ...
    default:
      return null; // not yet wired — OrganizationPage says so plainly, no fallback data
  }
}

type BlueprintResult = Awaited<ReturnType<typeof trpc.organization.blueprint.get.query>>;

/**
 * OrganizationPage (/organization) — the P1 Organization Generator's frontend proof:
 * fetches the active organization_definition, compiles it CLIENT-SIDE with
...
export function OrganizationPage() {
  ...
  const compiled: CompiledOrganization | null = useMemo(() => {
    const definition = blueprintResult?.definition;
    if (!definition) return null;
    try {
      return compileBlueprint(definition.blueprint as OrganizationBlueprint, REGISTERED_NODE_TYPES, RELATIONSHIP_NODE_TYPES);
    } catch (e) {
    ...
    return (
      <div className="p-6 space-y-3 max-w-xl">
        ...
        <div className="border rounded-md p-4 text-sm text-muted-foreground">
          ...
          <code className="mx-1 px-1 py-0.5 rounded bg-muted">organization.blueprint.propose</code>

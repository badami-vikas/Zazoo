// Organization + Team management, available from every standalone tool shell (StandaloneLayout).
// Real backend-persisted (Supabase, via apps/api's organization.* tRPC procedures) — not a local
...
import { API_ENABLED, apiListOrganizations, apiCreateOrganization, apiListMembers, apiInviteMember, type OrganizationDTO, type OrganizationMemberDTO } from '../../data/api';

type Tab = 'organization' | 'team';

export function OrganizationTeamModal({ initialTab, onClose }: { initialTab: Tab; onClose: () => void }) {
  ...
  async function refreshOrganizations() {
    ...
    setError(null);
    try {
      const rows = await apiListOrganizations();
      if (rows) {
  ...
  async function refreshMembers(organizationId: string) {
    ...
    setError(null);
    try {
      const rows = await apiListMembers(organizationId);
  ...
  useEffect(() => { if (API_ENABLED) refreshOrganizations(); }, []);
  ...
  async function createOrganization() {
    ...
    setError(null);
    try {
      const ws = await apiCreateOrganization(newOrganizationName.trim());
      setNewOrganizationName('');
      if (ws) { setOrganizations((prev) => [...prev, ws]); setSelected(ws.id); }
    } catch (e) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.4)' }} onClick={onClose}>
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex border-b" style={{ borderColor: 'var(--color-border)' }}>
          <button onClick={() => setTab('organization')} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-sm font-medium" style={{ color: tab === 'organization' ? 'var(--color-steel)' : 'var(--color-warm-gray)', borderBottom: tab === 'organization' ? '2px solid var(--color-steel)' : '2px solid transparent' }}>
          ...
          {!API_ENABLED ? (
            ...
            <>
              <div className="flex gap-1.5">
                <input value={newOrganizationName} onChange={(e) => setNewOrganizationName(e.target.value)} placeholder="New organization name" className="flex-1 px-3 py-2 rounded-lg border text-sm" style={{ borderColor: 'var(--color-border)' }} onKeyDown={(e) => e.key === 'Enter' && createOrganization()} />
                <button onClick={createOrganization} disabled={loading || !newOrganizationName.trim()} className="px-3 py-2 rounded-lg text-white text-sm font-semibold disabled:opacity-40" style={{ backgroundColor: 'var(--color-steel)' }}>
              ...
              <div className="flex flex-col gap-1">
                ...
                {organizations.length === 0 && !loading && <span className="text-xs text-center py-4" style={{ color: 'var(--color-warm-gray)' }}>No organizations yet — create one above.</span>}
              </div>
            </>
          ) : (
            <>

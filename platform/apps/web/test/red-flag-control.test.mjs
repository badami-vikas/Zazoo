...
test("RedFlagControl gates visibility on POINTER CAPABILITY (not viewport width), keeps a >=44px touch hit target, aria-pressed, and the popover", async () => {
...
test("RedFlagControl serializes reason-save against clear/reopen/forget and skips saving an unchanged reason (review item 4)", async () => {
  const source = await readFile(redFlagControlUrl, "utf8");
  assert.match(source, /pendingSaveRef/);
  assert.match(source, /reasonDraft === reason\)\s*return;/, "an unchanged reason must not trigger a save");
...
test("RedFlagControl is a context CONSUMER (no per-mount fetch) — the batched query lives in RedFlagProvider (review item 7)", async () => {
  ...
  for (const proc of ["create", "clear", "reopen", "updateReason", "forget", "listForScope"]) {
...
test("TableView wraps eligible cells with RedFlagControl inside a RedFlagProvider, gated by isFlaggableValue, using the real databaseId (not moduleId conflated with it)", async () => {
  ...
  assert.match(tableSource, /kind: "cell", moduleId: moduleIdFromDatabaseId\(spec\.id\), databaseId: spec\.id, recordId, fieldId: col\.id/);

...
export async function buildInMemoryPorts(env: {
  ...
  return {
    ...
    organizationDefinitionStore: new DrizzleOrganizationDefinitionStore(localDb),
...
export async function buildWiring(options: BuildWiringOptions = {}): Promise<Wiring> {
  ...
  const modePorts: ModePorts = url
    ? buildPersistentPorts({ url, organizationRenameCoordinator })
  ...
  const google = new GoogleService({
    ...
    identities: { organizationId: PILOT_ORGANIZATION, egressAgentId: EGRESS_AGENT, intakeAgentId: INTAKE_AGENT, userId: PILOT_USER },
    selfEmails,
    goalTasks,
  });

...
export const onboardingRouter = t.router({
  ...
  saveProfile: procedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const row = {
        ...
        phoneVerified: (input.verificationMethod === "phone" && await ctx.wiring.otpProofs.consume(input.organizationId, ctx.identity.id))
          ? true
  ...
  verifyPhoneOtp: procedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (verified) await ctx.wiring.otpProofs.issue(PILOT_ORGANIZATION, ctx.identity.id, 600_000);
      return {
        verified,
        dummy: true as const,

...
/**
 * Publish the curated built-ins the registry does not have yet.
 *
 * Presence is the guard, per Module. Published versions are immutable, so
 * re-publishing an existing one is an error rather than a no-op, and a version
 * someone pinned deliberately is not a boot's to reconcile. Republishing after
 * a curated manifest changes is `publish-builtins`' job.
 */
async function seedCommonsBuiltins(
  ...
  // Only what the registry LACKS. Versions are immutable, so a Module that is
  // already there is left alone (republishing after a curated manifest changes
  // is `publish-builtins`' job) — but a built-in added after this registry was
  // first seeded must still arrive, or the New dialog never offers it
  // (BUGS 2026-09-11 "I still dont see all module options").
  const existing = await client.listAvailable({ limit: 100, offset: 0 });
  const present = new Set(existing.items.map((item) => item.name));
  let published = 0;
  for (const { manifest: sourceManifest, commons } of COMMONS_BUILT_IN_MODULES) {
    if (present.has(sourceManifest.name)) continue;
    try {
  ...
  if (published > 0) log(`commons: published ${published} curated Modules the registry lacked`);
  return published;
}

...
const reviewModeEnum = z.enum(["auto", "notify", "approve", "quorum"]);
const trustOriginEnum = z.enum(["operator", "user_content", "untrusted_external"]);

const childRunCreateInput = z.object({
  workspaceId: z.string().min(1),
  parentRunId: z.string().min(1),
  parentAgentId: z.string().min(1),
  goalId: z.string().min(1),
  taskId: z.string().min(1),
  delegatedScope: z.array(z.string()),
  selectedSkills: z.array(z.string()),
  budget: z.object({ maxCalls: z.number().int().min(0), maxCost: z.number().min(0) }),
  deadline: z.string().min(1),
  stopCondition: z.string().min(1),
  requestedDataScope: dataScopeEnum.optional(),
  requestedReviewMode: reviewModeEnum.optional(),
  requestedTaint: trustOriginEnum.optional(),
  touchesExternalRisk: z.boolean().optional(),
  /**
   * The PARENT run's current bounds. These are honestly caller-supplied
   * (there is no persisted general "Agent Run" ledger yet — only
   * `ritual_runs`, a narrower primitive — see child-agent-run.ts's module
   * doc comment); `authorityScope`/`eligibleSkills`/`plane`/`dataScope` are
   * still ALWAYS re-derived server-side from `ctx.wiring.agents` +
   * `resolveSkillForTask` below, never taken from the request body, so a
   * caller cannot widen its own parent envelope by asserting a larger one.
   */
  parentBudgetRemaining: z.object({ calls: z.number().int().min(0), cost: z.number().min(0) }),
  parentReviewMode: reviewModeEnum.default("approve"),
  parentTaint: trustOriginEnum.optional(),
...
export const appRouter = t.router({
  ...
  agentOrchestration: t.router({
    goal: t.router({
      ...
      list: procedure.input(z.object({ workspaceId: z.string().min(1) })).query(async ({ input, ctx }) => {
        assertPilotWorkspace(input.workspaceId);
    ...
    task: t.router({
      create: procedure.input(taskCreateInput).mutation(async ({ input, ctx }) => {
        assertPilotWorkspace(input.workspaceId);
        const goal = await ctx.wiring.goalTasks.getGoal(input.goalId);
      ...
      listByGoal: procedure.input(z.object({ goalId: z.string().min(1) })).query(({ input, ctx }) => {
        return ctx.wiring.goalTasks.listTasksByGoal(input.goalId);
      }),
      /** The ONLY thing that changes governed-Skill eligibility for a Task —
       * never a Skill manifest's `defaultAgents` preference list. */
      reassign: procedure.input(taskReassignInput).mutation(({ input, ctx }) => {
        return ctx.wiring.goalTasks.reassignTask(input.taskId, input.assignedAgentId);
      }),
    }),
    ...
    skill: t.router({
      ...
      resolve: procedure.input(resolveSkillInput).query(async ({ input, ctx }) => {
        const goal = await ctx.wiring.goalTasks.getGoal(input.goalId);
        const task = await ctx.wiring.goalTasks.getTask(input.taskId);
        if (!goal || !task || task.goalId !== goal.id) {
          throw new TRPCError({ code: "NOT_FOUND", message: "unknown or mismatched Goal/Task" });
        }
        const agentScope = await ctx.wiring.agents.capabilityScope(input.agentId);
        const agentDataScope = await ctx.wiring.agents.dataScope(input.agentId);
        const candidates = input.skillId
          ? ctx.wiring.skillManifests.forSkill(input.skillId)
          : ctx.wiring.skillManifests.all();
        return resolveSkillForTask(candidates, {
          goal,
          task,
          agent: { id: input.agentId, capabilityScope: agentScope, plane: "local", dataScope: agentDataScope },
    ...
    childRun: t.router({
      /**
       * Derive + persist + audit a bounded child Agent Run. `authorityScope`/
       * `eligibleSkills`/`dataScope`/`plane` are ALWAYS re-derived server-side
       * (never taken from the request body) — see `childRunCreateInput`'s doc
       * comment above. Maps `ChildRunDepthExceededError`/
       * `ChildRunAuthorityExceededError` to 400 rather than a generic 500.
       */
      create: procedure.input(childRunCreateInput).mutation(async ({ input, ctx }) => {
        assertPilotWorkspace(input.workspaceId);
        const goal = await ctx.wiring.goalTasks.getGoal(input.goalId);
        const task = await ctx.wiring.goalTasks.getTask(input.taskId);
        if (!goal || !task || task.goalId !== goal.id) {
        ...
        if (task.assignedAgentId !== input.parentAgentId) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: `Task ${task.id} is assigned to "${task.assignedAgentId}", not "${input.parentAgentId}" — only the assigned Agent may spawn a child Run for it`,
          });
        }
        const agentScope = await ctx.wiring.agents.capabilityScope(input.parentAgentId);
        const agentDataScope = await ctx.wiring.agents.dataScope(input.parentAgentId);
        const resolution = await resolveSkillForTask(ctx.wiring.skillManifests.all(), {
          goal,
          task,
          agent: { id: input.parentAgentId, capabilityScope: agentScope, plane: "local", dataScope: agentDataScope },
        });
        const eligibleSkills = resolution.ok && resolution.manifest ? [resolution.manifest.skillId] : [];

        try {
          return await createChildAgentRun(
            { store: ctx.wiring.childAgentRuns, ledger: ctx.wiring.ledger },
            {
              runId: input.parentRunId,
              agentId: input.parentAgentId,
              workspaceId: input.workspaceId,
              authorityScope: agentScope,
              eligibleSkills,
              dataScope: agentDataScope,
              plane: "local",
              budgetRemaining: input.parentBudgetRemaining,
              reviewMode: input.parentReviewMode as ReviewMode,
              ...(input.parentTaint ? { taint: input.parentTaint as TrustOrigin } : {}),
            ...
            },
            {
              ...
              ...(input.requestedDataScope ? { requestedDataScope: input.requestedDataScope as DataScope } : {}),
              ...(input.requestedReviewMode ? { requestedReviewMode: input.requestedReviewMode as ReviewMode } : {}),
              ...(input.requestedTaint ? { requestedTaint: input.requestedTaint as TrustOrigin } : {}),
              ...(input.touchesExternalRisk !== undefined ? { touchesExternalRisk: input.touchesExternalRisk } : {}),
            },
            ctx.run,
          );
        } catch (err) {
          if (err instanceof ChildRunDepthExceededError || err instanceof ChildRunAuthorityExceededError) {
            throw new TRPCError({ code: "BAD_REQUEST", message: err.message });
          }
          throw err;
        }
      ...
      get: procedure.input(z.object({ childRunId: z.string().min(1) })).query(({ input, ctx }) => {
        return ctx.wiring.childAgentRuns.get(input.childRunId);
      }),

      listByParentRun: procedure.input(z.object({ parentRunId: z.string().min(1) })).query(({ input, ctx }) => {
        return ctx.wiring.childAgentRuns.listByParentRun(input.parentRunId);
      }),
      ...
      cancel: procedure.input(z.object({ childRunId: z.string().min(1) })).mutation(({ input, ctx }) => {

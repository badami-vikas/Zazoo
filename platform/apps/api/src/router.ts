...
export const appRouter = t.router({
  ...
  action: t.router({
    ...
    propose: procedure.input(proposeInput).mutation(async ({ input, ctx }) => {
      ...
      if (input.skill.startsWith("dealpilot.")) {
    ...
    decide: authenticatedProcedure.input(decideInput).mutation(async ({ input, ctx }) => {
      ...
      const dealPilotEffects =
        resolved.status === "applied"
          ...
          : [];
      return { ...resolved, effects, dealPilotEffects };
    }),
  }),
  ...
  dealpilot: t.router({
    ...
    decideSourceDiscovery: dealpilotProcedure
      .input(
        z.object({
          workspaceId: z.string().min(1),
          proposalId: z.string().min(1),
      ...
      .mutation(async ({ input, ctx }) => {
        assertPilotWorkspace(input.workspaceId);
        const original = await ctx.wiring.ledger.get(input.proposalId);
        const originalInputs = original?.inputs as { kind?: unknown; workspaceId?: unknown } | undefined;
        if (
          !original ||
          original.workspaceId !== input.workspaceId ||
          originalInputs?.kind !== "thesis_source_discovery" ||
          originalInputs.workspaceId !== input.workspaceId
        ) {
          throw new TRPCError({ code: "BAD_REQUEST", message: "Proposal is not a DealPilot Source discovery" });
        }
        let resolved;
        try {
          await validateDealPilotDecision(
          ...
          if (error instanceof AgentFloorDeniedError) {
            throw new TRPCError({ code: "FORBIDDEN", message: error.message });
          }
          throw error;
        }
        if (input.decision === "veto") return { proposal: resolved, relations: [] };
        const proposedOutput = resolved.output?.proposedOutput as ThesisSourceDiscoveryProposal | undefined;
        if (proposedOutput?.kind !== "thesis_source_discovery") {
          throw new TRPCError({ code: "BAD_REQUEST", message: "Proposal is not a DealPilot Source discovery" });
        }
        const relations = await materializeDealPilotApproval(ctx.wiring, resolved);
        return { proposal: resolved, relations };
      }),
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        try {
          proposal = await ctx.wiring.pipeline.propose(
            {
              workspaceId: input.workspaceId,
              actor: { type: ctx.identity.type, id: ctx.identity.id, plane: "cloud" },
              action: "read" as Action,
              resourceType: "external:fetch" as ResourceType,
              skill: "dealpilot.source",
              inputs: { workspaceId: input.workspaceId, sourceId: input.sourceId },
            },
            ctx.run,
          );
        } catch (error) {

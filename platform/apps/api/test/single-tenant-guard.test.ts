 ...
 * several workspace-scoped procedures either silently ignored a client-supplied
 * `workspaceId` or never validated one. Every such procedure now runs through
 * `assertPilotWorkspace` (router.ts), which throws a typed `NonPilotWorkspaceError`
 * translated to `TRPCError({code:"FORBIDDEN"})` by the `withPilotWorkspaceGuard`
 * middleware — turning a silent cross-tenant leak into a loud, typed 403.
 *
 * These tests prove: (1) a non-pilot workspaceId is rejected with FORBIDDEN for
 ...
 * pilot workspace's own id still works normally end-to-end.
 */
...
test("dealpilot.list: a non-pilot workspaceId is rejected with FORBIDDEN, not silently ignored", async () => {
  const wiring = await buildWiring();
  try {
    const caller = await makeCaller(wiring);
    await assert.rejects(
      () => caller.dealpilot.list({ workspaceId: NON_PILOT_WORKSPACE, limit: 10, offset: 0 }),
      (err: unknown) => {
...
test("dealpilot.list: the pilot workspace's own id still works normally", async () => {
  const wiring = await buildWiring();
  try {
    const caller = await makeCaller(wiring);
    const page = await caller.dealpilot.list({ workspaceId: PILOT_WORKSPACE, limit: 10, offset: 0 });
...
test("dealpilot.list: omitting workspaceId entirely still works (no caller sends it today)", async () => {
  const wiring = await buildWiring();
  try {
    const caller = await makeCaller(wiring);
...
test("action.propose: a non-pilot workspaceId is rejected with FORBIDDEN (guard applies beyond dealpilot.list)", async () => {
  const wiring = await buildWiring();
  try {
    const caller = await makeCaller(wiring);
    await assert.rejects(
...
test("action.propose: the pilot workspace's own id still works normally", async () => {
  const wiring = await buildWiring();
  try {
    const caller = await makeCaller(wiring);

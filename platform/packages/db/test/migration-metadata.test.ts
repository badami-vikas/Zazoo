...
test("Drizzle metadata is rebased through 0037 and generate is a deterministic no-op", () => {
  ...
  try {
    ...
    assert.deepEqual(last, {
      ...
      version: "7",
      when: 1785867456919,
      tag: "0037_aqv_ledger_attribution",
      breakpoints: true,
    });
    assert.ok(
    ...
    assert.ok(
      ...
      "current AQV ledger-attribution snapshot must be tracked",
    );
    ...
    assert.ok(
      // The NEXT index after the current head (0037). If `generate` allocates
      // this, schema.ts and the committed migrations have drifted apart.
      !readdirSync(probeMigrations).some((name) => /^0038_.*\.sql$/.test(name)),

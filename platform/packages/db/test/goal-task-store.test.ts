...
test("goal-task store: create + get Goal round-trip", async () => {
  ...
  try {
    ...
    const fetched = await store.getGoal(created.id);
    assert.ok(fetched);
...
test("goal-task store: create Task, reassign, and update status round-trip; listTasksByGoal scopes to goal", async () => {
  ...
  try {
    ...
    const reassigned = await store.reassignTask(task.id, otherAgent!.id);
    assert.equal(reassigned.assignedAgentId, otherAgent!.id);

    const updated = await store.updateTaskStatus(task.id, "in_progress");
    assert.equal(updated.status, "in_progress");

    const listed = await store.listTasksByGoal(goal.id);
...
test("goal-task store: getGoal/getTask return null for unknown ids rather than throwing", async () => {
  ...
  try {
    const store = new DrizzleGoalTaskStore(db);
    assert.equal(await store.getGoal("00000000-0000-4000-8000-000000000000"), null);
    assert.equal(await store.getTask("00000000-0000-4000-8000-000000000000"), null);
  } finally {
    await close();
  }
});

...
function fixtureManifest(overrides: Partial<SkillManifest> = {}): SkillManifest {
  return {
...
test("skill manifest store: seedSkillManifests + refresh round-trips a full manifest", async () => {
  ...
  try {
    await seedSkillManifests(db, [fixtureManifest({ budget: { maxCallsPerDay: 10 }, defaultAgents: ["learning"], childRunPolicy: "allowed" })]);

    const registry = new DrizzleSkillManifestRegistry(db);
    await registry.refresh();

    const manifests = registry.forSkill("test.fixture.skill");
...
test("skill manifest store: multiple versions of one skill id are both retrievable via forSkill", async () => {
  const { db, close } = await createLocalDb();
  try {
    await seedSkillManifests(db, [fixtureManifest({ version: "1.0.0" }), fixtureManifest({ version: "1.1.0" })]);
...
test("skill manifest store: all() returns the full seeded catalog across skill ids", async () => {
  const { db, close } = await createLocalDb();
  try {
    await seedSkillManifests(db, [fixtureManifest({ skillId: "test.fixture.a" }), fixtureManifest({ skillId: "test.fixture.b" })]);
...
test("skill manifest store: forSkill on an unregistered skill id returns empty, not an error", async () => {
  ...
  try {
    const registry = new DrizzleSkillManifestRegistry(db);
    await registry.refresh();
    assert.deepEqual(registry.forSkill("nothing.registered"), []);
  } finally {
    await close();
  }
});
...
test("skill manifest store: reading forSkill/all before refresh() fails loud rather than silently returning empty", async () => {
  ...
  try {
    const registry = new DrizzleSkillManifestRegistry(db);
    assert.throws(() => registry.forSkill("test.fixture.skill"), /read before refresh/);
    assert.throws(() => registry.all(), /read before refresh/);
  } finally {
    await close();
  }
});

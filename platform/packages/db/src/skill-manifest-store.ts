...
export class DrizzleSkillManifestRegistry implements SkillManifestRegistry {
  ...
  async refresh(): Promise<void> {
    ...
    for (const row of rows) {
      ...
      const existing = next.get(manifest.skillId) ?? [];
      existing.push(manifest);
      next.set(manifest.skillId, existing);
    }
    this.#cache = next;
    this.#loaded = true;
  }
  ...
  forSkill(skillId: string): readonly SkillManifest[] {
    this.#requireLoaded();
 ...
 * (unique on skill_id+version — re-running on every boot with the SAME
 * declarations is a no-op update, not a growing duplicate list). `workspaceId`
 * omitted = a global/platform-wide manifest (mirrors `skills.workspaceId`'s
 * nullable shape). Call once at boot, before the registry's `refresh()`.
 */
export async function seedSkillManifests(db: Database, catalog: readonly SkillManifest[], workspaceId?: string): Promise<void> {
  for (const manifest of catalog) {
    const existing = await db
      .select({ id: skillManifests.id })
      .from(skillManifests)
      .where(and(eq(skillManifests.skillId, manifest.skillId), eq(skillManifests.version, manifest.version)))
      .limit(1);
    ...
    if (existing[0]) {
      await db.update(skillManifests).set(values).where(eq(skillManifests.id, existing[0].id));
    } else {
      await db.insert(skillManifests).values(values);
    }
  }
}

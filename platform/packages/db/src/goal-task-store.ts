...
export class DrizzleGoalTaskStore implements GoalTaskStore {
  ...
  async listTasksByGoal(goalId: string): Promise<Task[]> {
    const rows = await this.#db.select().from(tasks).where(eq(tasks.goalId, goalId));
    return rows.map(unpackTask);
  }

  async reassignTask(id: string, assignedAgentId: string): Promise<Task> {
    const [updated] = await this.#db.update(tasks).set({ assignedAgentId }).where(eq(tasks.id, id)).returning();
    if (!updated) throw new Error(`tasks: unknown task ${id}`);
    return unpackTask(updated);
  }

  async updateTaskStatus(id: string, status: TaskStatus): Promise<Task> {
    const [updated] = await this.#db.update(tasks).set({ status }).where(eq(tasks.id, id)).returning();
    if (!updated) throw new Error(`tasks: unknown task ${id}`);
    return unpackTask(updated);
  }
}

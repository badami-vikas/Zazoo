 ...
 * stealth "Ox Alpha" preview model as the Chat default (Groq stays the
 * fallback — see resolveChatModel's cloud ordering). Same rules as
 * GroqProvider/AnthropicProvider: NO API key on any manifest/capability, key
 * comes from env or the Settings vault at construction, two-plane gate
 * applies (createModelRouter enforces planeDefault, not this class).
 */
import {

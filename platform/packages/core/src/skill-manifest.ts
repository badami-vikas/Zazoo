...
export class InMemorySkillManifestRegistry implements SkillManifestRegistry {
  ...
  register(manifest: SkillManifest): void {
    const existing = this.byId.get(manifest.skillId) ?? [];
    existing.push(manifest);
    this.byId.set(manifest.skillId, existing);
  }
...
export async function resolveSkillForTask(
  ...
  for (const manifest of pool) {
    ...
    // 4b) Data scope.
    if (!manifest.dataScopes.includes("all") && !manifest.dataScopes.includes(requestedDataScope)) {
      fail(
        "data-scope-mismatch",
        `manifest permits data scopes [${manifest.dataScopes.join(", ")}], request/agent tier is "${requestedDataScope}"`,
      );
      continue;
    }

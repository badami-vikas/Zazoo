 ...
 * manifest EXCEPT the manifest itself. Carried on a `CommonsModuleEntry`
...
/**
 * A published module's signed envelope — the manifest plus a detached
...
export function toSignedEnvelope(manifest: ModuleManifest, signature: ManifestSignature): SignedManifestEnvelope {
 ...
 * external JSON-canonicalization dependency — this module is zero-deps).
 */
export function canonicalizeManifest(manifest: ModuleManifest): string {

...
import { computeModuleRisk, moduleHasLethalTrifecta, type CapabilityManifest, type ModuleManifest } from "../src/index.js";
...
function pkg(overrides: Partial<ModuleManifest> = {}): ModuleManifest {
  return {
    name: "dummy-module",
    version: "1.0.0",
...
test("computeModuleRisk: composite risk = max over bundled capabilities", () => {
  const p = pkg({
    capabilities: [
...
test("computeModuleRisk: a single egress permission alone drives risk to external", () => {
  const p = pkg({
...
test("computeModuleRisk: trifecta assembled ACROSS separate capabilities escalates to external", () => {
  const p = pkg({
    capabilities: [
...
test("moduleHasLethalTrifecta: false when only two of three legs present", () => {
  const capabilities = [
    cap({ id: "a", permissions: [{ resourceType: "person", action: "read", dataScope: "private", egress: false }] }),
    cap({ id: "b", permissions: [{ resourceType: "external_fetch", action: "read", dataScope: "public", egress: false }] }),
  ];
  assert.equal(moduleHasLethalTrifecta(capabilities), false);
});

test("computeModuleRisk: walks dependency module's capabilities into the composite", () => {
  const depPkg = pkg({
    name: "dep-module",
    capabilities: [cap({ id: "dep-cap", permissions: [{ resourceType: "role", action: "write", dataScope: "all", egress: false }] })],
  });
  const p = pkg({ dependencies: [{ manifestId: "dep-module", version: "1.0.0" }] });
  const result = computeModuleRisk(
    p,
    () => undefined,
    (name, version) => (name === "dep-module" && version === "1.0.0" ? depPkg : undefined),
  );
  // "role" is a GOVERNED_RESOURCES write -> operational, pulled in from the dependency module.
  assert.equal(result.compositeRisk, "operational");
});

test("computeModuleRisk: unresolved module dependency escalates conservatively to operational", () => {
  const p = pkg({ dependencies: [{ manifestId: "missing-module", version: "1.0.0" }] });
  const result = computeModuleRisk(p, () => undefined, () => undefined);
  assert.equal(result.unresolvedDependencies.length, 1);
  assert.equal(result.compositeRisk, "operational");
});

test("computeModuleRisk: cycle-safe against a self-referencing dependency", () => {
  const p = pkg({ name: "cyclic-module", dependencies: [{ manifestId: "cyclic-module", version: "1.0.0" }] });
  const result = computeModuleRisk(p, () => undefined, (name, version) => (name === "cyclic-module" && version === "1.0.0" ? p : undefined));
  // Should terminate and just reflect the module's own capability risk (informational/transformational),

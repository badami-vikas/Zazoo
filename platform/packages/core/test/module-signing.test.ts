...
test("canonicalizeManifest: key order does not change the canonical bytes", () => {
  ...
  const b: ModuleManifest = {
    organizationVocab: { domainTerms: {}, alignsToBridgeTheme: true },
    contextProviders: [],
...
function commonsEntry(): CommonsModuleEntry {
  const content = {
    name: "test-fixture-pkg",
    version: "1.0.0",
...
test("Commons hash excludes its own integrity/signature/publish fields and verifies the signed pin", () => {
  ...
  assert.equal(
    computeCommonsContentHash(commonsModuleContent(entry), (value) => `hash(${value})`).value,

 ...
 * (organizationId, ownerUserId, lineageKey), a stale `expectedCurrentId` is
...
test("casSupersede: a second create (expectedCurrentId: null) fails once a row already exists — never forks", async () => {
  ...
  const first = await store.casSupersede({
    organizationId: WS, ownerUserId: OWNER, lineageKey: LINEAGE_KEY, expectedCurrentId: null,
    next: draft("00000000-0000-4000-8000-000000000001"),
  });
  assert.ok(first);
...
test("casSupersede: a stale expectedCurrentId (already superseded) is rejected", async () => {
  ...
  const v2 = await store.casSupersede({
    organizationId: WS, ownerUserId: OWNER, lineageKey: LINEAGE_KEY, expectedCurrentId: v1!.id,
    next: draft("00000000-0000-4000-8000-000000000002"),
  });
  assert.ok(v2);
...
test("casSupersede (review round-7 'durable lineage ordering'): allocates lineageRevision 1, 2, 3... atomically per lineage, never per a caller-supplied value", async () => {
  ...
  const v3 = await store.casSupersede({
    organizationId: WS, ownerUserId: OWNER, lineageKey: LINEAGE_KEY, expectedCurrentId: v2!.id,
    next: draft("00000000-0000-4000-8000-000000000003"),
  });
  ...
  const otherLineage = await store.casSupersede({
    organizationId: WS, ownerUserId: OWNER, lineageKey: "test_fixture_other_lineage_key", expectedCurrentId: null,
    next: draft("00000000-0000-4000-8000-0000000000aa", { subjectRecordId: "test_fixture_other_lineage_key" }),
  });
  ...
  const [winner, loser] = await Promise.all([
    store.casSupersede({
      organizationId: WS, ownerUserId: OWNER, lineageKey: LINEAGE_KEY, expectedCurrentId: v3!.id,
...
test("retrieve with orderBy:'lineageRevision' (review round-7): a legacy write() row with no revision sorts OLDEST regardless of direction, and keyset pagination is stable across the null/allocated boundary", async () => {
  ...
  const v1 = await store.casSupersede({
    organizationId: WS, ownerUserId: OWNER, lineageKey: LINEAGE_KEY, expectedCurrentId: legacy.id,
    next: draft("00000000-0000-4000-8000-000000000001"),
  });
  const v2 = await store.casSupersede({
    organizationId: WS, ownerUserId: OWNER, lineageKey: LINEAGE_KEY, expectedCurrentId: v1!.id,
  ...
  const auth = { organizationId: WS, userId: OWNER };
...
test("currentForLineage scopes strictly by organizationId + ownerUserId + lineageKey — no cross-tenant/owner bleed", async () => {
  const store = new InMemoryMemoryStore();
  await store.casSupersede({
    organizationId: WS, ownerUserId: OWNER, lineageKey: LINEAGE_KEY, expectedCurrentId: null,
    next: draft("00000000-0000-4000-8000-000000000001"),
  });
  const otherOrganization = await store.currentForLineage("99999999-0000-4000-8000-000000000009", OWNER, LINEAGE_KEY);

...
test("Automation/Ritual integration: a Ritual run whose declared actor is a Human (not an Agent) fails closed on a governed step — no second actor-binding path", async () => {
  const h = harness();
  authorizePrincipal(h, "user", "human-1");
  const { task } = await seedGoalTask(h, "internal_strategist");
  const ritualRegistry = new InMemoryRitualRegistry();
  ritualRegistry.register({
    id: "ritual-3",
    name: "test_fixture automation human actor",
    workspaceId: WS,
    steps: [
      {
        skill: "stageStrategicRecommendation",
  ...
  assert.equal(result.status, "halted");
  assert.match(result.proposals[0]!.rejectionReason ?? "", /may only be invoked by an eligible Agent Run/);
});
